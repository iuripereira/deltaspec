...
# A privada pode não estar em disco: quem a guarda num agente SSH (1Password,
# gnome-keyring) fica só com o `.pub`, e esse é o arranjo recomendado, não um
# defeito. `ssh -i <arquivo .pub>` é o modo canônico nesse caso — o `.pub` casa
# com a chave que o agente oferece, e o `IdentitiesOnly` abaixo segue impedindo
# que outra chave entre no lugar dela (DT-092).
CHAVE_PUBLICACAO="${DELTASPEC_SSH_KEY_PUBLICA:-${HOME}/.ssh/deltaspec-publica}"
if [[ ! -f "${CHAVE_PUBLICACAO}" ]] && [[ -f "${CHAVE_PUBLICACAO}.pub" ]]; then
  CHAVE_PUBLICACAO="${CHAVE_PUBLICACAO}.pub"
  echo "==> chave privada fora do disco; usando ${CHAVE_PUBLICACAO} com o agente SSH"
fi
if [[ ! -f "${CHAVE_PUBLICACAO}" ]]; then
  echo "chave de publicação ausente: ${CHAVE_PUBLICACAO}" >&2
  echo "  se a chave já existe num agente SSH, deixe o .pub em disco para o match:" >&2
  echo "    op read 'op://<cofre>/<item>/public key' > ${CHAVE_PUBLICACAO}.pub   # ou o export do seu agente" >&2
  echo "    ssh-add -l   # confere que o agente serve a chave" >&2
  echo "  se ainda não existe, gere o par e registre a pública como deploy key com escrita no derivado:" >&2
...
# O modo de falha comum não é chave ausente — é chave presente que não consegue
# assinar: o 1Password pede aprovação a cada cliente novo e não responde sozinho
# num shell sem ninguém para clicar. A mensagem de "chave ausente" acima não cobre
# esse caso, e mandar gerar um par novo é o pior conselho possível ali: a chave
# existe, está registrada como deploy key, e trocá-la exigiria registrar de novo.
diagnostico_assinatura() {
  local fp agente servido identity_agent
  fp="$(ssh-keygen -lf "${CHAVE_PUBLICACAO}" 2>/dev/null | awk '{print $2}')" || true
  agente="${SSH_AUTH_SOCK:-<vazio>}"
  # o ssh_config vence o SSH_AUTH_SOCK, e é por isso que a chave pode estar
  # disponível mesmo quando `ssh-add -l` não a lista
  identity_agent="$(awk '/^[[:space:]]*IdentityAgent/{print $2; exit}' "${HOME}/.ssh/config" 2>/dev/null)" || true
  ...
  {
    echo
    echo "push recusado ao assinar com ${CHAVE_PUBLICACAO}"
    echo "  A chave existe — o que falhou foi assinar com ela. Em ordem de probabilidade:"
    echo
    echo "  1. Aprovação pendente. O 1Password confirma cada cliente novo e não"
    echo "     responde sozinho: rode com o app destravado e responda o prompt."
    echo "     Sessão sem interface (cron, CI, ssh remoto) não tem quem clique."
    echo "  2. Agente que não serve esta chave. O ssh usa o 'IdentityAgent' do"
    echo "     ssh_config do usuário quando existe, senão SSH_AUTH_SOCK."
    echo "  3. A chave deixou de ser deploy key com escrita no derivado."
    echo
    printf '  %-22s %s\n' "fingerprint" "${fp:-<ilegível>}"
    printf '  %-22s %s\n' "SSH_AUTH_SOCK" "${agente}"
    printf '  %-22s %s\n' "IdentityAgent" "${identity_agent:-<nenhum>}"
    printf '  %-22s %s\n' "esse agente a serve?" "${servido}"
    echo
    echo "  Conferir o que o agente oferece:   ssh-add -l"
    echo "  Conferir a deploy key no derivado: gh api repos/${repo_gh:-<dono>/<repo>}/keys --jq '.[].title'"
  } >&2
}
...
git push --quiet --force origin HEAD:main || { diagnostico_assinatura; exit 1; }
git push --quiet --force origin "${versao}" || { diagnostico_assinatura; exit 1; }
