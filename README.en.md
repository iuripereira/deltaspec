...
[Current version: tags](https://github.com/iuripereira/deltaspec/tags) · 15 skills · MIT License
...
Done: the 15 skills show up under the `deltaspec:` prefix. To update later, `/plugin update deltaspec@deltaspec` — without the marketplace's `@deltaspec`, the plugin is not found.
...
Fourteen cycle commands, in the order you meet them. Click a name to read the skill's details.
...
The fifteenth is not a command: [`eu-tenho-tdah`](skills/eu-tenho-tdah/SKILL.md) is Iuri's writing profile (based on [ayghri/i-have-adhd](https://github.com/ayghri/i-have-adhd)), always on — action before context, ranked lists, tangents become saved pending items.
...
# TRUTH size, task dependencies (checks C1–C13)
...
# CHANGELOG entry shape: category, length, PR reference, ordering
...
| `debts/` | debt, pending items and lessons | Stable `DT-NNN` IDs, one file per item; resolved items **change status, never disappear** — closed items move to `debts/_archive/`; urgency index generated at `DEBT.md`; rules in `debts/README.md` |
