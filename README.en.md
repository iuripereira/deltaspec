...
[Current version: tags](https://github.com/iuripereira/deltaspec/tags) · 23 skills · MIT License
...
Done: the 23 skills show up under the `deltaspec:` prefix. To update later, `/plugin update deltaspec@deltaspec` — without the marketplace's `@deltaspec`, the plugin is not found.
...
Branch protection, CI, Conventional Commits check, assisted review. Running it again breaks nothing — the second run just reports what was already there.

Releasing is two skills, nothing by hand: `/deltaspec:criar-release X.Y.Z` validates the version against the latest tag, cuts the release branch, releases the `CHANGELOG.md` and opens the PR to `main`; `/deltaspec:release` merges on green checks, creates tag and Release in one call on the merge commit and dispatches the deploy from the tag, when the project declares one. Where a `develop` exists, it syncs `main → develop` through a `sync/X.Y.Z` PR whose CHANGELOG is checked as a set of bullets — none lost, none unreleased pushed under the version — and confirms that `main` is an ancestor of `develop`; on the framework itself, it also publishes the public derivative. Stopped halfway? Run it again: it checks what is already done and resumes from the first missing step. The version is the only human decision.
...
flowchart LR
    ...
    r -->|no| l("capture register<br>route from your CLAUDE.md")
...
The **lesson** is the third resident of `debts/`: a post-mortem with no pending action, a `debts/licoes/LICAO_L-NNN-<topic>.md` file with the rule that sticks as the title, what happened, the cause (a gap in the system or the process, never "human error"), the outcome, and the prevention — each item pointing to the check, the `DT-NNN`, or the discipline that keeps it from recurring. It is born via `licao.py nova`, never by hand; `debts/LICOES.md` is the generated index, and CI fails on a divergent index ([ADR-0042](docs/adrs/ADR-0042-licoes-em-arquivo-por-item.md)).

```bash
# registers an already-validated lesson — computes the L-NNN, refuses a missing field
python3 skills/handoff/scripts/licao.py nova . \
  --descricao "the rule that sticks" --familia kebab-name --deteccao gate --prevencao disciplina \
  --origem "delta-NNN · PR #N" --corpo-arquivo body.md

# regenerates debts/LICOES.md from debts/licoes/ (a projection, ADR-0042)
python3 skills/handoff/scripts/licao.py indice .
```

**Outside a project with a reachable register, nothing is lost.** The [`eu-tenho-tdah`](skills/eu-tenho-tdah/SKILL.md) skill registers the pending item as a `DT-NNN` in a **capture register** — a repository with `debts/`, whose **route** you declare in your own `CLAUDE.md`; the framework names routes, never machine paths — with a queue and aging like any other item, closes with a commit and no push when the destination is versioned, and migrates the item to the owning repo's `debts/` as soon as it becomes real work. A repository that wants its own register declares `deltaspec.registro: proprio` in `doc-profile.yaml`, and `debito.py novo` creates it on the first entry. It is the counterpart of [`handoff`](skills/handoff/SKILL.md), which does the same routing at the end of a session.
...
Twenty-two cycle commands, in the order you meet them. Click a name to read the skill's details.
...
| [`project-review-dotclaude`](skills/project-review-dotclaude/SKILL.md) | Measures whether `.claude/` holds only what Claude Code reads, plus `handoffs/`: parked plans and prompts, a legacy `commands/` folder, runtime leftovers with no rule in the versioned `.gitignore`, a hook cited but not committed, a canonical document in the wrong place | Auditing the `.claude/` folder of one repo or a whole workspace |
...
| [`replan-doc`](skills/replan-doc/SKILL.md) | Re-plans a plan already in execution against what the repo learned after it: collection driven by a target map, sourced facts, a new dated plan with checkable tasks — the previous one stays intact | The plan went stale: a dossier, an answer, grilling output, a handoff or a debt arrived after it |
...
| [`inicio-de-sessao`](skills/inicio-de-sessao/SKILL.md) | Opens the day with every delta's state derived from git — never from a status field —, the bench of worktrees and branches holding unsaved or stalled work, and ONE next step | Resuming a configured project and needing to know where it stopped |
| [`criar-release`](skills/criar-release/SKILL.md) | Cuts the release with no hand-written recipe: validates the requested version against the last tag and the commits (`versao_corte.py` — only the immediate successor passes), cuts `release/X.Y.Z` from `develop` (or from `main`, without `develop`), releases the `[Não lançado]` section with `--liberar`, opens the PR and waits for the checks. Hotfix in two phases: `--hotfix` creates `hotfix/X.Y.Z` from `main` and stops; the second call, with the fix, cuts it | When `[Não lançado]` has something to ship and you decided the version |
| [`release`](skills/release/SKILL.md) | Publishes the cut version: merge of the release/hotfix PR (merge commit with `develop`, squash without), tag + Release from the CHANGELOG section, deploy by tag, `main→develop` sync through `sync/X.Y.Z` checked as a set of bullets and `main ⊆ develop` verified by `merge-base --is-ancestor`. Resumable: running it again continues from the first missing step | When the `release/*` or `hotfix/*` PR is green — or to resume a publication that stopped halfway |

The twenty-third is not a command: [`eu-tenho-tdah`](skills/eu-tenho-tdah/SKILL.md) is always on and delivers two axes — it shapes the answer (action before context, ranked lists, a concrete next step) and gives every tangent a destination: a pending item becomes a record in the system from [section 6](#6-the-debt-register), never an invitation at the end of the answer.
...
# a PR that settles a DT with a Ticket carries the link (Closes #N or the Jira key);
# the ci job runs the same command with the PR text (--pr-arquivo)
python3 skills/handoff/scripts/debito.py vinculo . --base origin/main --head HEAD
```
...
python3 skills/spec-feature/scripts/montar_changelog.py --selftest
python3 skills/guarding-doc-integrity/scripts/validate_integrity.py --selftest
python3 skills/guarding-doc-integrity/scripts/ancora.py --selftest
...
python3 skills/handoff/scripts/licao.py --selftest
python3 skills/handoff/scripts/check_handoff.py --selftest
python3 skills/handoff/scripts/md_para_adf.py --selftest
python3 skills/handoff/scripts/projecao.py --selftest
python3 skills/handoff/scripts/integracao.py --selftest
python3 skills/audit-workspace/scripts/audit_workspace.py --selftest
python3 skills/inicio-de-sessao/scripts/fila.py --selftest
python3 skills/inicio-de-sessao/scripts/config.py --selftest
python3 skills/inicio-de-sessao/scripts/contexto.py --selftest
python3 skills/inicio-de-sessao/scripts/nota.py --selftest
python3 skills/inicio-de-sessao/scripts/sessao.py --selftest
python3 skills/git-guard/scripts/segredos.py --selftest
python3 skills/project-review-dotclaude/scripts/dotclaude.py --selftest
...
python3 skills/criar-release/scripts/versao_corte.py --selftest
python3 skills/release/scripts/secao_changelog.py --selftest
...
| `debts/` | debt, pending items and lessons | Stable `DT-NNN`/`L-NNN` IDs, one file per item; resolved items **change status, never disappear** — closed items move to `debts/_archive/`; a lesson is immutable in `debts/licoes/`; generated indexes at `DEBT.md`/`LICOES.md`; rules in `debts/README.md` — overview in [section 6](#6-the-debt-register) |
