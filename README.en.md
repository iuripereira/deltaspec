...
[Current version: tags](https://github.com/iuripereira/deltaspec/tags) · 14 skills · MIT License
...
Done: the 14 skills show up under the `deltaspec:` prefix. To update later, `/plugin update deltaspec@deltaspec` — without the marketplace's `@deltaspec`, the plugin is not found.
...
Twelve cycle commands, in the order you meet them. Click a name to read the skill's details.
...
The fourteenth is not a command: [`eu-tenho-tdah`](skills/eu-tenho-tdah/SKILL.md) is Iuri's writing profile (based on [ayghri/i-have-adhd](https://github.com/ayghri/i-have-adhd)), always on — action before context, ranked lists, tangents become saved pending items.
