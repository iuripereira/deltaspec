...
- **Em curso:** corrigido na delta-027, quita no archive dela — o C2 e o C3 passaram a ter conjuntos próprios (`scan_valores` e `scan_links`), com chave `exclude_links_globs` no manifesto e default nomeado para propagar sem migração. Neste repo o C3 saltou de **105 para 161** links verificados. Dois achados que só a medição deu: o `DEBT.md` tem **19** links no atalho `../../issues/N` do GitHub, que viravam falso FAIL e passaram a ser ignorados como `http://` já era; e `specs/_archive/**` tem 13 links quebrados que ficaram **fora por decisão do usuário** — registro de época (R47), rot que a política proíbe corrigir
