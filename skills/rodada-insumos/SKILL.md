...
2. **Mineração.** Delegar às fases 1–3 da `descoberta`: dossiê datado `docs/discovery/AAAA-MM-DD-<evento>.md` ([template](../descoberta/references/templates/dossie.md)), claim com `confirmado`/`inferido`/`lacuna` + fonte rastreável (`arquivo:linha`, timestamp, id de resposta, frame), GLOSSARY/DATA_DICTIONARY por append/merge. Dossiê do dia já existe → append, nunca sobrescrever. Insumo que chegou **atrasado** (datado antes de eventos já minerados) → "nota de sequência" no dossiê tratando duplicidade com o que outras vias já capturaram, em vez de reabrir questão fechada.
