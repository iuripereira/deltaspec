...
Subcomandos:
  ...
  check     [RAIZ] [--dbml CAMINHO] [--forcar]     M1–M6 (tabela de severidades: references/camadas.md)
  --selftest                                         fixtures co-localizadas (RNF4)

Severidade máxima na v1: ALTO (ADR-0038; M6 é sempre BAIXO). Exit 0 sem achado, 1 com ALTO ou contrato inválido,
...
def checar(raiz: Path, dbml: Path) -> list:
    """M1–M6. Severidade máxima ALTO (M6 é sempre BAIXO — ADR-0038). Devolve [(sev, onde, inconsistência, ação)]."""
    v = []
    dm = raiz / DATA_MODEL
...
def relatorio(v: list, nome: str) -> int:
    ...
    print("\nParcial: cobre M1–M6 (camada conceitual × contrato × dicionário).")
    sevs = {f[0] for f in v}
