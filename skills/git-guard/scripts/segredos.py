...
# Nome do padrão genérico — dono único: `PADROES` o usa como chave e o recorte de bundle
# gerado (DT-162) o usa para desligá-lo; renomear aqui vale para os dois.
PADRAO_GENERICO = "valor-opaco-em-chave-sensivel"
...
# Bundle gerado por ferramenta (página autocontida do archify): o runtime JavaScript
# embutido tem variáveis `token`/`tokenKind` seguidas de valor minificado, e o padrão
# genérico casa o NOME da chave — falso positivo medido 3× por página (DT-162).
# Ali só os padrões de segredo real valem; o genérico fica fora.
#
# Dois reconhecimentos, porque a pasta de saída do archify muda de projeto para projeto e
# o recorte por pasta só cobria a convenção de um deles: o caminho convencional, e o
# sufixo `.archify.html`, que identifica o arquivo pelo que ele É em vez de por onde foi
# parar. Medido num repo consumidor que entrega em `docs/apresentacao/archify/` e ficava
# fora do recorte — três bypass `--no-verify` seguidos, que é o desfecho que este recorte
# existe para evitar.
PREFIXOS_BUNDLE_GERADO = ("docs/diagrams/",)
SUFIXOS_BUNDLE_GERADO = (".html",)
SUFIXO_ARCHIFY = ".archify.html"


def bundle_gerado(nome: str) -> bool:
    """Puro: caminho de bundle gerado, onde só os padrões de segredo real se aplicam."""
    minusculo = nome.lower()
    if minusculo.endswith(SUFIXO_ARCHIFY):
        return True
    return minusculo.startswith(PREFIXOS_BUNDLE_GERADO) and minusculo.endswith(SUFIXOS_BUNDLE_GERADO)
...
def casar(texto: str, incluir_generico: bool = True) -> list[tuple[str, int]]:
    """Puro: devolve [(nome_do_padrao, numero_da_linha)] ordenado por linha.

    `incluir_generico=False` desliga só o padrão genérico (bundle gerado, ver DT-162);
    os padrões de segredo real casam sempre.
    ...
    for numero, linha in enumerate(texto.splitlines(), 1):
        for nome, padrao in PADROES:
            if nome == PADRAO_GENERICO and not incluir_generico:
                continue
...
def achados_no_diff(diff: str) -> list[tuple[str, str, int]]:
    ...
    def fechar() -> None:
        if arquivo and not arquivo_ignorado(arquivo):
            achados.extend(
                (arquivo, nome, n)
                for nome, n in casar("\n".join(adicionadas), incluir_generico=not bundle_gerado(arquivo))
            )
...
def selftest() -> int:
    ...
    assert generico == [(PADRAO_GENERICO, 3)], generico
    ...
    assert [n for n, _ in multi] == ["chave-aws", PADRAO_GENERICO], multi
    ...
    # --- bundle gerado (DT-162): o genérico cala, o segredo real segue acusando -------
    bundle = "diff --git a/docs/diagrams/x/p.html b/docs/diagrams/x/p.html\n--- a/docs/diagrams/x/p.html\n" \
             "+++ b/docs/diagrams/x/p.html\n@@ -0,0 +1,2 @@\n" \
             f"+var token = \"{OPACO_LONGO}\";\n+var tokenKind = \"{OPACO_LONGO}\";\n"
    assert achados_no_diff(bundle) == [], "variável token do viewer não é segredo"
    assert achados_no_diff(bundle + f"+AWS_KEY={AWS_FALSA}\n") == \
        [("docs/diagrams/x/p.html", "chave-aws", 3)], "segredo real dentro do bundle continua acusado"
    assert bundle_gerado("docs/diagrams/a/b.html") and not bundle_gerado("docs/diagrams/a/b.json") \
        and not bundle_gerado("skills/x.html"), "só HTML sob docs/diagrams/ é bundle"
    # ...e o mesmo bundle entregue fora daquela pasta, reconhecido pelo sufixo
    assert bundle_gerado("docs/apresentacao/archify/x/y.archify.html"), \
        "bundle do archify é reconhecido pelo sufixo, em qualquer pasta"
    assert not bundle_gerado("docs/apresentacao/archify/x/y.archify.json"), \
        "a fonte .json do archify continua varrida pelo padrão genérico"
    assert not bundle_gerado("docs/apresentacao/x.html"), \
        "HTML comum fora de docs/diagrams/ não vira bundle por estar perto do archify"
    fora_da_pasta = "diff --git a/docs/apresentacao/archify/p/p.archify.html" \
                    " b/docs/apresentacao/archify/p/p.archify.html\n" \
                    "--- a/docs/apresentacao/archify/p/p.archify.html\n" \
                    "+++ b/docs/apresentacao/archify/p/p.archify.html\n@@ -0,0 +1,2 @@\n" \
                    f"+var token = \"{OPACO_LONGO}\";\n+var tokenKind = \"{OPACO_LONGO}\";\n"
    assert achados_no_diff(fora_da_pasta) == [], \
        "variável token do viewer não é segredo, esteja o bundle na pasta que estiver"
    assert achados_no_diff(fora_da_pasta + f"+AWS_KEY={AWS_FALSA}\n") == \
        [("docs/apresentacao/archify/p/p.archify.html", "chave-aws", 3)], \
        "segredo real dentro do bundle continua acusado fora de docs/diagrams/"
    ...
    print("selftest: OK (segredos.py — 8 padrões, filtro de exemplo e de placeholder em "
          ...
          "o contrato de nunca devolver o trecho casado, bundle gerado sem o genérico, e as "
          "portas --staged/--diff)")
    return 0
