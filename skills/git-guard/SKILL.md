...
5. **Registro.** Dois destinos, conforme a origem do achado:
   ...
   - **Repositório alheio** (varredura de workspace) → o registro de captura, pela rota da `deltaspec:eu-tenho-tdah`, com `Origem: git-guard` e o caminho do repositório auditado. Cadastre fora dele, nunca dentro: escrever `DT-NNN` em N repositórios de uma vez é porta de mão única.
...
| Cadastrar `DT-NNN` em cada repositório da varredura | Achado de repositório alheio vai para o registro de captura; escrever em N repositórios de uma vez é porta de mão única |
