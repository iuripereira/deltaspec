...
4. **Commit, push, PR.** Caminhos explícitos, nunca `git add -A`: `git add -- CHANGELOG.md $(git ls-files -- changelog.d)` (mais o caminho do manifesto, se o repo o espelha) — o `ls-files` pega a remoção dos fragmentos e é vazio no repo sem `changelog.d/`. Confira `git diff --cached --name-only` (só esses caminhos) e então `git commit -m 'chore(release): X.Y.Z'` · `git push -u origin <branch>` · `CORPO="$(mktemp -d)/corpo-pr.md"` · `gh pr create --base main --title 'chore(release): X.Y.Z' --body-file "$CORPO"`. Corpo padrão — sem link relativo: ele resolve contra a URL da PR e quebra, e o repo consumidor nem tem a ADR:
