...
Perfil com `artefatos.modelo-dados.obrigatorio: true` → rode também `python3 ${CLAUDE_PLUGIN_ROOT}/skills/modelo-dados/scripts/check_data_model.py check` e cole a tabela (M1–M3) abaixo da do `check_cycle.py`, que segue dono dos C1–C13 (R12). Categoria opcional ou perfil ausente → o script se omite em 1 linha (delta-073).
