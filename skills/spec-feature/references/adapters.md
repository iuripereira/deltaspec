...
- **Canal humano (verificação pós-fase, delta-026):** além dos ADRs, a fase só fecha com a **trilha do clarify** no cabeçalho do `spec.md` (C12 — sintaxe na tabela de trilha de auditoria do `cycle.md`, dona do formato). Ambiguidade resolvida por exploração do repositório **não conta como resposta do usuário**: o `grill-me` manda explorar em vez de perguntar, e num repo com TRUTH e ADRs isso responde quase tudo — o resultado é uma entrevista que nunca acontece. Nesse caso o relatório de ambiguidade sai marcado `auto-avaliado`, em vez de passar como se tivesse havido conversa.
- **Viés de quem pontua:** quem redige a spec é quem atribui os graus do relatório, e tende a se avaliar bem. Na dúvida entre dois graus, escolha o **mais ambíguo** — regra do próprio `grill-me` ("be honest about your own uncertainty"), repetida aqui porque é onde ela é ignorada.
...
|---|---|---|---|---|
| `max@max4c-skills` | 0.8.0 | **fork deliberado** — pin na testada ([ADR-0012](../../../docs/adrs/ADR-0012-recontratacao-motores.md)) | 2026-08-07 | API contratada intacta no 0.8.0 (`grill-me`/`grill-with-docs`/`write-prd` presentes); o `to-tickets` upstream serve o ecossistema Dahso (`dahso:go`), não projeção Jira/GitHub — reavaliação da delta-017 cumprida, **fork mantido** ([ADR-0024](../../../docs/adrs/ADR-0024-pin-do-max-reavaliado-fork-mantido.md)). **Gatilho novo:** breaking do plugin distribuído ou o upstream ganhar projeção de tickets para Jira/GitHub. Forkável: em último caso copiar a SKILL.md para o diretório de skills pessoais do usuário e apontar este adapter |
