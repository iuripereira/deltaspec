...
| modelo de dados (specify/plan, categoria `modelo-dados` obrigatória no perfil) | `deltaspec:modelo-dados` (própria, delta-073) · modo `auditar` → `mattpocock-skills:grilling` + `domain-modeling` (gatilho vigente, delta-075) | nome fixo `docs/data-model.md`; subconjunto DBML do parser (`camadas.md` da skill) — `.dbml` fora dele cai em M1; gatilho "modelo de dados persistente" reaproveitado fora do clarify |
