...
from projecao import (emitir_sh_acli, ler_destino_tickets,  # noqa: E402 — após o path-fix
                      nome_var)
...
def _carregar_contexto(delta_dir: Path) -> tuple:
    ...
    _, projeto = ler_destino_tickets(root)  # tickets.py só fala Jira; destino não-jira => sem projeto
    if not projeto:
        raise _SemProjeto
...
def selftest() -> None:
    ...
    # ler_destino_tickets: presente, desligado, ausente
    root, delta_dir = _montar_delta()
    assert ler_destino_tickets(root) == ("jira", "SBX")
    root_off, delta_off = _montar_delta(com_jira=False)
    assert ler_destino_tickets(root_off) == ("github-issues", None)
    root_sem = _montar_delta()[0]
    (root_sem / "doc-profile.yaml").unlink()
    assert ler_destino_tickets(root_sem) == ("github-issues", None)
