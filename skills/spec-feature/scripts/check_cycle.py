...
continuam com o modelo — são juízo, não regex.
  ...
  C15 índice do TRUTH — coluna 'Requisitos' da tabela confrontada com '### R' no disco (delta-093)
  C16 HANDOFF × git — versão citada no topo do 'Agora' e 'Atualizado em' confrontados
      com git describe/git log; informativo, nunca bloqueia (delta-095)
  C17 âncora × plugin em execução — versão declarada em doc-profile.yaml confrontada
      com $CLAUDE_PLUGIN_ROOT; ALTO, reprova (delta-104)
  C18 promoção de perfil (topologia) — delta toca código de produto num repo que ainda não
      lançou release; avisa, nunca bloqueia (quem promove é o usuário)
...
from ancora import verificar as verificar_ancora  # noqa: E402
...
# Espelho do vocabulário de motores.tickets — dono real em skills/handoff/scripts/projecao.py
# (VOCAB_TICKETS). Duplicado aqui, não importado: check_cycle.py não depende de outra skill
# por sys.path (a classe de acoplamento que o DT-116 já registra como débito).
VOCAB_TICKETS = ("none", "github-issues", "jira")
...
LINHA_INDICE_TRUTH = re.compile(r"^\|\s*[^|]+?\s*\|\s*\[[^\]]+\]\(truth/([^)]+)\)\s*\|\s*(\d+)\s*\|\s*$", re.M)


def c15_indice_truth(root: Path, v: list) -> None:
    """Coluna 'Requisitos' do índice specs/TRUTH.md é projeção do disco (DT-076): conta
    '### R' em cada partição e confronta com o número da tabela — mesma leitura que o C5
    já faz do TRUTH.md, mas sobre a contagem, não o custo de contexto."""
    truth = root / "specs" / "TRUTH.md"
    particoes_dir = root / "specs" / "truth"
    if not truth.is_file() or not particoes_dir.is_dir():
        return
    tabela = {m.group(1): int(m.group(2)) for m in LINHA_INDICE_TRUTH.finditer(truth.read_text(encoding="utf-8"))}
    disco = {p.name: len(re.findall(r"^### R", p.read_text(encoding="utf-8"), re.M))
             for p in particoes_dir.glob("*.md")}
    for nome in sorted(tabela.keys() | disco.keys()):
        if nome not in disco:
            v.append(("ALTO", "specs/TRUTH.md", f"tabela cita 'truth/{nome}' sem arquivo correspondente", "remover a linha ou criar a partição"))
        elif nome not in tabela:
            v.append(("ALTO", f"specs/truth/{nome}", "partição em disco sem linha no índice", "acrescentar a linha em specs/TRUTH.md"))
        elif tabela[nome] != disco[nome]:
            v.append(("ALTO", "specs/TRUTH.md", f"'{nome}': índice diz {tabela[nome]}, disco tem {disco[nome]}", "corrigir a contagem na tabela"))


SECAO_AGORA = re.compile(r"^##\s+Agora\s*$(.*?)(?=^##\s|\Z)", re.M | re.S)
ATUALIZADO_EM = re.compile(r"^\*\*Atualizado em:\*\*\s*(\d{4}-\d{2}-\d{2})", re.M)
VERSAO_TAG = re.compile(r"\bv(\d+\.\d+\.\d+)\b")


def primeiro_bullet(secao: str) -> str:
    """Primeiro item '- ' de uma seção — só o topo do 'Agora' descreve o estado atual;
    bullets mais antigos citam versão velha por natureza (histórico), não por drift."""
    m = re.search(r"^-\s.*?(?=^-\s|\Z)", secao, re.M | re.S)
    return m.group(0) if m else ""


def c17_ancora(root: Path, v: list) -> None:
    """Âncora declarada × plugin realmente em execução (delta-104). ALTO: fora da âncora,
    o ciclo estaria rodando contra um framework que não é o contratado, e o resultado
    não é reproduzível. Diferente do C16, que é informativo por desenho — aqui a
    divergência não é staleness esperada, é execução contra a versão errada.

    Toda a lógica (e as duas omissões: chave ausente, plugin root ausente) é do
    ancora.py, dono canônico — este check só traduz o veredito para a tabela."""
    _, achado = verificar_ancora(root)
    if achado:
        v.append(("ALTO", "doc-profile.yaml", achado,
                  "reancore ou volte o plugin: `ancora.py verificar .` mostra as duas versões"))


def c16_handoff(root: Path, v: list) -> None:
    """HANDOFF.md × verdade local do git (DT-045): topo do 'Agora' vs. tag mais recente,
    'Atualizado em' vs. data do último commit. Sempre BAIXO (molde do C7) — staleness é
    esperada numa delta em progresso (a atualização é tarefa de fim de sessão, não de
    cada commit); o check informa, o humano decide. Sem git/tag/HANDOFF.md, se omite."""
    handoff = root / "HANDOFF.md"
    if not handoff.is_file():
        return
    tag = subprocess.run(["git", "-C", str(root), "describe", "--tags", "--abbrev=0"],
                         capture_output=True, text=True)
    commit = subprocess.run(["git", "-C", str(root), "log", "-1", "--format=%cs"],
                            capture_output=True, text=True)
    if tag.returncode != 0 or commit.returncode != 0:
        print("aviso: sem git ou sem tag — C16 (HANDOFF × git) se omite", file=sys.stderr)
        return
    tag_atual, data_commit = tag.stdout.strip(), commit.stdout.strip()
    texto = handoff.read_text(encoding="utf-8")

    m_data = ATUALIZADO_EM.search(texto)
    if m_data and m_data.group(1) != data_commit:
        v.append(("BAIXO", "HANDOFF.md",
                  f"'Atualizado em: {m_data.group(1)}' diverge do último commit ({data_commit})",
                  "confirmar se o índice está desatualizado e atualizar ao fechar a sessão"))

    m_agora = SECAO_AGORA.search(texto)
    if m_agora:
        versoes = VERSAO_TAG.findall(primeiro_bullet(m_agora.group(1)))
        if versoes:
            citada = "v" + max(versoes, key=lambda s: tuple(map(int, s.split("."))))
            if citada != tag_atual:
                v.append(("BAIXO", "HANDOFF.md",
                          f"topo do 'Agora' cita {citada}, tag mais recente é {tag_atual}",
                          "confirmar se o Agora está desatualizado"))
...
def c11_perfil(root: Path, v: list) -> None:
    ...
    tickets = mot.get("tickets")
    if tickets is not None and tickets not in VOCAB_TICKETS:
        v.append(("ALTO", "doc-profile.yaml", f"motores.tickets fora do vocabulário: {tickets!r}",
                  "usar none | github-issues | jira (delta-099)"))
    jira = mot.get("jira")
    jira_ligado = jira not in (None, False)  # ligado: true ou dict (mesmo vazio) — ausente/false é desligado
    if jira_ligado or tickets == "jira":
        projeto = jira.get("projeto") if isinstance(jira, dict) else None
        if not str(projeto or "").strip():
            v.append(("ALTO", "doc-profile.yaml",
                      "motores.jira ligado (ou motores.tickets: jira) sem motores.jira.projeto",
                      "declarar a chave do projeto Jira, ou desligar/trocar o destino"))
...
# Prefixos que são FERRAMENTA, não produto: script que fabrica documento, hook,
# CI, doc. Medido em 31/08/2026 em 6 repos consumidores — o código vivo deles mora
# todo aqui. Lista de negação, não de afirmação: produto novo aparece em caminho
# imprevisto e o aviso deve disparar; ferramenta nova é rara e sai com 1 linha.
PREFIXOS_FERRAMENTA = ("scripts/", "docs/", ".claude/", ".github/", ".githooks/",
                       "tools/", "specs/", "debts/")
EXT_CODIGO = (".py", ".js", ".ts", ".jsx", ".tsx", ".mjs", ".go", ".rs", ".rb", ".java")


def c18_promove(caminhos, promovido: bool) -> bool:
    """True quando a delta toca código de PRODUTO num repo que ainda não publica
    release nenhum. Sinal para o humano decidir, nunca bloqueio: quem promove é o
    usuário rodando o projeto-infra (delta-105)."""
    if promovido:
        return False
    return any(c.endswith(EXT_CODIGO) and not c.startswith(PREFIXOS_FERRAMENTA) for c in caminhos)


def c18_promocao(root: Path, v: list) -> None:
    """Sinal de promoção de perfil (topologia; delta-105). BAIXO, molde do C16: o check informa,
    o humano decide — promover é ato do usuário, e um ALTO travaria o archive por
    uma decisão que não é do gate. O estado NÃO é declarado em arquivo: ele se lê do
    repo (tag existente ou .release-please-config.json legado = já promovido),
    porque estado declarado diverge do real e estado lido não pode."""
    # Sem repo git, sai calado (molde do C16, que retorna em HANDOFF.md ausente):
    # o selftest roda checar() em diretório temporário e um aviso aqui viraria
    # ruído em cada caso, escondendo o que importa.
    if not (root / ".git").exists():
        return
    tocados = subprocess.run(["git", "-C", str(root), "diff", "--name-only", "origin/main...HEAD"],
                             capture_output=True, text=True)
    if tocados.returncode != 0:
        print("aviso: sem origin/main — C18 (promoção de perfil) se omite", file=sys.stderr)
        return
    # "promovido" = o repo JÁ publica release, por qualquer rota. Tag é a rota
    # das skills de release (delta-117); o .release-please-config.json é o
    # marcador legado de repo consumidor que ainda não migrou — sem aceitar os
    # dois, o C18 acusaria um deles em todo archive, para sempre. Sobra o caso que interessa: repo que nunca
    # lançou nada e começou a ter código de produto.
    tags = subprocess.run(["git", "-C", str(root), "tag"], capture_output=True, text=True)
    ja_lanca = ((root / ".release-please-config.json").is_file()
                or bool(tags.returncode == 0 and tags.stdout.strip()))
    caminhos = [c for c in tocados.stdout.splitlines() if c]
    if c18_promove(caminhos, promovido=ja_lanca):
        v.append(("BAIXO", "—", "delta toca código de produto e o repo ainda não lançou release",
                  "considere promover: corte a 1ª release com /deltaspec:criar-release + /deltaspec:release"))
...
def selftest_c11() -> None:
    ...
    casos = [
        ...
        (nucleo + "motores: { tickets: none }\n", 0, "tickets: none é válido"),
        (nucleo + "motores: { tickets: github-issues }\n", 0, "tickets: github-issues é válido"),
        (nucleo + "motores: { tickets: jira, jira: { projeto: SBX } }\n", 0,
         "tickets: jira com projeto declarado é válido"),
        (nucleo + "motores: { tickets: jira }\n", 1, "tickets: jira sem motores.jira.projeto acusa ALTO"),
        (nucleo + "motores: { tickets: jira, jira: {} }\n", 1,
         "tickets: jira com jira: {} (sem projeto) acusa só 1 ALTO — não duplica com o achado de jira ligado"),
        (nucleo + "motores: { tickets: trello }\n", 1, "tickets fora do vocabulário acusa ALTO"),
        (nucleo + "motores: { chave_desconhecida: true }\n", 0,
         "chave desconhecida no bloco motores segue tolerada (cauda livre, delta-099 não muda isso)"),
...
def selftest_c15() -> None:
    """C15: contagem do índice do TRUTH confrontada com '### R' de cada partição no disco."""
    import tempfile

    def rodar(truth: str, particoes: dict[str, str] | None = None):
        """TRUTH.md com tabela real + partições opcionais num repo temporário; roda só o C15."""
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            (root / "specs" / "truth").mkdir(parents=True)
            (root / "specs" / "TRUTH.md").write_text(truth, encoding="utf-8")
            for nome, txt in (particoes or {}).items():
                (root / "specs" / "truth" / nome).write_text(txt, encoding="utf-8")
            v: list = []
            c15_indice_truth(root, v)
            return v

    # 0. repositório sem TRUTH.md ou sem specs/truth/ (greenfield): o C15 se omite
    with tempfile.TemporaryDirectory() as d:
        vazio: list = []
        c15_indice_truth(Path(d), vazio)
        assert vazio == [], f"C15 acusou repo sem TRUTH.md: {vazio}"

    tabela = "| Domínio | Partição | Requisitos |\n|---|---|---|\n| X | [x.md](truth/x.md) | 2 |\n"
    par_ok = {"x.md": "### R1 (delta-001) — a\n### R2 (delta-001) — b\n"}
    assert rodar(tabela, par_ok) == [], "índice em paridade com o disco não deveria acusar"

    par_defasado = {"x.md": "### R1 (delta-001) — a\n"}
    achado = rodar(tabela, par_defasado)
    assert len(achado) == 1 and achado[0][0] == "ALTO" and "2" in achado[0][2] and "1" in achado[0][2], \
        f"índice divergente do disco deveria acusar ALTO citando os dois valores: {achado}"

    par_orfao = {"x.md": "### R1 (delta-001) — a\n### R2 (delta-001) — b\n", "y.md": "### R9 (delta-002) — c\n"}
    achado_orfao = rodar(tabela, par_orfao)
    assert len(achado_orfao) == 1 and "y.md" in achado_orfao[0][1], \
        f"partição em disco sem linha na tabela deveria acusar: {achado_orfao}"

    tabela_orfa = "| Domínio | Partição | Requisitos |\n|---|---|---|\n| X | [x.md](truth/x.md) | 2 |\n| Y | [y.md](truth/y.md) | 1 |\n"
    achado_tabela = rodar(tabela_orfa, par_ok)
    assert len(achado_tabela) == 1 and "y.md" in achado_tabela[0][2], \
        f"linha da tabela sem arquivo correspondente deveria acusar: {achado_tabela}"

    print("selftest C15: OK (paridade, defasagem, partição órfã, linha órfã)")


def selftest_c18() -> None:
    """C18 (delta-105) — sinal de promoção de perfil (topologia). Só a lógica pura: a fiação
    (git diff + presença do .release-please-config.json) é I/O e não entra aqui."""
    assert c18_promove(["src/main.py", "docs/x.md"], promovido=False)
    assert not c18_promove(["scripts/gerar.py", "docs/x.md", ".github/workflows/ci.yml"], promovido=False)
    assert not c18_promove(["src/main.py"], promovido=True), "repo que já lança não reavisa"
    assert not c18_promove([], promovido=False)
    assert c18_promove(["dashboard/src/App.jsx"], promovido=False), "código fora de src/ na raiz conta"
    assert not c18_promove([".claude/hooks/guarda-git.py"], promovido=False), "hook é ferramenta"
    assert not c18_promove(["README.md", "CHANGELOG.md"], promovido=False), "markdown nunca é produto"
    print("selftest C18: OK (código de produto avisa, ferramenta cala, promovido não reavisa)")


def selftest_c17() -> None:
    """C17: a FIAÇÃO, não a lógica — o ancora.py já cobre normalização, comparação e as
    duas omissões nos próprios selftests. Aqui se prova só que o veredito dele chega à
    tabela do analyze com severidade ALTO, e que projeto sem âncora não gera linha."""
    import json
    import os
    import tempfile
    ...
    try:
        with tempfile.TemporaryDirectory() as d:
            tmp = Path(d)
            plugin = tmp / "plugin" / ".claude-plugin"
            plugin.mkdir(parents=True)
            (plugin / "plugin.json").write_text(
                json.dumps({"name": "deltaspec", "version": "1.52.0"}), encoding="utf-8")
            os.environ["CLAUDE_PLUGIN_ROOT"] = str(tmp / "plugin")

            def rodar(ancora: str | None):
                root = tmp / f"r{ancora}"
                root.mkdir(exist_ok=True)
                perfil = PERFIL_NUCLEO
                if ancora:
                    perfil += f'deltaspec:\n  ancora: "{ancora}"\n'
                (root / "doc-profile.yaml").write_text(perfil, encoding="utf-8")
                v: list = []
                c17_ancora(root, v)
                return v

            achado = rodar("v1.51.1")
            assert len(achado) == 1 and achado[0][0] == "ALTO" \
                and "1.51.1" in achado[0][2] and "1.52.0" in achado[0][2], \
                f"deriva deveria virar ALTO nomeando as duas versões: {achado}"

            assert rodar(None) == [], "projeto sem âncora não pode gerar linha na tabela"
    finally:
        os.environ.pop("CLAUDE_PLUGIN_ROOT", None)
        if guardado is not None:
            os.environ["CLAUDE_PLUGIN_ROOT"] = guardado
    print("selftest C17: OK (deriva vira ALTO na tabela, projeto sem âncora se omite)")


def selftest_c16() -> None:
    """C16: HANDOFF.md × git real — cenário histórico do DT-045 (topo do Agora atrasado
    em relação à tag) acusa BAIXO; paridade libera; sem git/tag/arquivo se omite."""
    import os
    import tempfile

    def repo_git(root: Path):
        # Data de autoria E de commit fixadas (--date só afeta a de autoria; %cs lê a de
        # commit, que sem GIT_COMMITTER_DATE cairia no relógio real do runner) — senão a
        # comparação com git log -1 --format=%cs nunca bate no cenário "em dia".
        env = {**os.environ, "GIT_COMMITTER_DATE": "2026-08-13T12:00:00"}
    ...
    def rodar(handoff_txt: str, tags: list[str]):
        """Repo git real com uma tag por commit em `tags`; HANDOFF.md com `handoff_txt`
        commitado por último (assim 'Atualizado em' pode casar ou não com o commit)."""
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            git = repo_git(root)
            for i, tag in enumerate(tags):
                (root / f"f{i}.txt").write_text(tag, encoding="utf-8")
                git("add", "-A")
                git("commit", "-qm", tag, "--date", "2026-08-13T12:00:00")
                git("tag", tag)
            (root / "HANDOFF.md").write_text(handoff_txt, encoding="utf-8")
            git("add", "-A")
            git("commit", "-qm", "handoff", "--date", "2026-08-13T12:00:00")
            v: list = []
            c16_handoff(root, v)
            return v
    ...
    try:
        ...
        print("selftest C16: PULADO (git indisponível)")
        return
    # git presente: daqui em diante toda falha é ruidosa — PULADO não mascara regressão
    atrasado = ("**Atualizado em:** 2026-08-10\n\n## Agora\n"
                "- trabalho concluído, v1.24.0 publicada\n"
                "- item mais antigo, v1.20.0\n")
    achado = rodar(atrasado, ["v1.24.0", "v1.25.0"])
    assert any(s == "BAIXO" and "v1.24.0" in q and "v1.25.0" in q for s, _, q, _ in achado), \
        f"C16 não acusou o topo do Agora atrasado em relação à tag: {achado}"
    assert any(s == "BAIXO" and "Atualizado em" in q for s, _, q, _ in achado), \
        f"C16 não acusou 'Atualizado em' divergente do último commit: {achado}"

    em_dia = ("**Atualizado em:** 2026-08-13\n\n## Agora\n- trabalho concluído, v1.25.0 publicada\n")
    assert rodar(em_dia, ["v1.24.0", "v1.25.0"]) == [], "HANDOFF em paridade não deveria acusar"

    with tempfile.TemporaryDirectory() as d:
        v = []
        c16_handoff(Path(d), v)
        assert v == [], f"sem HANDOFF.md/git/tag o C16 deve se omitir: {v}"

    print("selftest C16: OK (git real; topo do Agora atrasado acusado, paridade liberada, "
          "ausência de HANDOFF/git/tag omitida)")
