...
continuam com o modelo — são juízo, não regex.
  ...
  C18 promoção de fase — delta toca código de produto num repo ainda sem
      release-please; avisa, nunca bloqueia (quem promove é o usuário)
...
def c18_promocao(root: Path, v: list) -> None:
    """Sinal de promoção de fase (delta-105). BAIXO, molde do C16: o check informa,
    o humano decide — promover é ato do usuário, e um ALTO travaria o archive por
    uma decisão que não é do gate. A fase NÃO é declarada em arquivo: ela se lê do
    repo (.release-please-config.json presente = já promovido), porque estado
    declarado diverge do real e estado lido não pode."""
    ...
    if tocados.returncode != 0:
        print("aviso: sem origin/main — C18 (promoção de fase) se omite", file=sys.stderr)
        return
    # "promovido" = o repo JÁ publica release, por qualquer rota. release-please é
    # uma; tag é a outra — a deltaspec tem 93 tags e nenhum release-please, porque
    # publica pelo publica-dist.sh, e sem este segundo marcador o C18 a acusaria
    # em todo archive, para sempre. Sobra o caso que interessa: repo que nunca
    ...
    if c18_promove(caminhos, promovido=ja_lanca):
        v.append(("BAIXO", "—", "delta toca código de produto e o repo segue em fase de especificação",
                  "considere promover: /deltaspec:projeto-infra instala release-please"))
...
def selftest_c18() -> None:
    """C18 (delta-105) — sinal de promoção de fase. Só a lógica pura: a fiação
