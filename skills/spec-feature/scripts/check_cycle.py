...
def numero_delta_livre(root: Path) -> int:
    ...
    CHANGELOG.md. Não cobre o primeiro (concorrência entre checkouts via origin/main/PRs
    abertos) — precisa de rede, fica para uma delta futura se reincidir de novo.
...
def main() -> None:
    ...
    print("\nParcial: cobre C1–C13; os checks 3 e 5 do analyze.md (scope creep, regra canônica) são juízo humano e não rodaram.")
    sevs = {f[0] for f in v}
...
def selftest_c4() -> None:
    ...
    print("selftest C4: OK (git real; perda acusada em Rn, RF-NN e requisito anotado, "
          "MUDA/sufixo/anotação/partição liberados)")
