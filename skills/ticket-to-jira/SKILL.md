...
- **Transição no merge (delta-119):** o item do DT quitado vai para concluído pela **regra de automação do projeto Jira**, não pelo repositório — gatilho *Pull request merged* (integração GitHub for Jira), condição: a chave do item no título da PR ou na branch; ação: transicionar para o status de concluído. A PR carrega a chave porque o `debito.py quitar` a imprime e o `debito.py vinculo` a confere no CI — no título ou na branch; chave só no corpo não vincula e reprova. É configuração de cada projeto Jira — o repositório nunca a aplica —, e o comentário de fechamento em nível de negócio segue manual: a automação não escreve o resultado para o negócio.
