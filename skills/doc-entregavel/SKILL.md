...
Exporta o **entregável congelado para o cliente**: renderiza os diagramas declarados no `doc-profile.yaml`, monta o documento final (PRD/spec + diagramas embutidos + capa de assinatura) e exporta **PDF e/ou DOCX** em `docs/entregaveis/`. Generalização do pipeline validado nos PRDs e contratos de um projeto consumidor real. Documentação cliente é **isenta da economia de tokens** (exceção registrada na ADR-0009 do deltaspec) — completude e fidelidade visual dominam; a renderização é via CLI, o custo de tokens é marginal.
...
`/deltaspec:doc-entregavel <formato> <linguagem>` — ambos opcionais, nesta ordem.

| Argumento | Valores | Default | Efeito |
|---|---|---|---|
| `formato` | `pdf` · `docx` · `html` · `md` · `todos` | o que `entregaveis.formato` declara no `doc-profile.yaml` | `pdf`/`docx` → `exporta_entregavel.py`. `html` → o gerador do próprio projeto; **o exportador não faz html**. `md` → grava só o markdown recortado. |
| `linguagem` | `executiva` · `tecnica` · `completa` | `completa` | Corta pelos H1 `# Parte I — ` e `# Parte II — `. Documento sem esses H1 sai inteiro, em qualquer valor. |

**A linguagem seleciona seções; não reescreve prosa.** Documento que precisa de linguagem executiva ganha as duas Partes no fonte, editadas à mão uma vez: Parte I com o que o sistema faz, requisitos, cronograma, diagramas e visão de protótipo; Parte II com o detalhe técnico e os anexos. Se pedirem "deixa mais executivo" num documento sem Partes, **pare e proponha a reestruturação** — não resuma no export.

**O artefato assinado é o `completa`.** O recorte deixa âncoras de anexo penduradas, então `--linguagem` diferente de `completa` recusa `--anexo` e `--assinatura`: recorte é para leitura e apresentação, não para assinatura.

**Política de link é do renderizador, não do fonte.** O markdown continua linkando normalmente — é o que serve o repo e o HTML.

| Saída | Política |
|---|---|
| `pdf` · `docx` | Autocontido: `[texto](alvo)` vira `texto`, âncora interna e imagem ficam, `.svg` vira `.png` quando o derivado existe ao lado. |
| `html` | Link relativo vira `<a>` para a página irmã publicada; diagrama entra inline. |

De-linkar **não** transcreve: o conteúdo que o documento precisa carregar tem de estar nele. Anexo é conteúdo, nunca ponteiro — transcreva o recorte usado e diga isso na abertura do anexo.
...
4. **Exportar** com `${CLAUDE_PLUGIN_ROOT}/skills/doc-entregavel/scripts/exporta_entregavel.py`, uma chamada por formato em `entregaveis.formato`, capa vinda de `entregaveis.capa` do perfil:
   ...
   ```
   Tipo confidencial pelo `references/juridico.md` (`juridico-nda`, `requisitos-cliente` Versão B)? Acrescente `--rodape CONFIDENCIAL --marca-dagua CONFIDENCIAL` — rodapé e marca d'água saem do export em todas as páginas, nos dois formatos, sem etapa manual (delta-031).
...
**Camada de apresentação ([ADR-0029](../../docs/adrs/ADR-0029-apresentacao-como-modo-e-html-autocontido-canonico.md), supersede a ADR-0018):** apresentação é a flag `apresentacao: true` no artefato do perfil, materializada no archive como página HTML autocontida pelo motor nativo (ou pelo `diagram-design`, quando o perfil o declara). **Nunca entra direto no caminho do export.** O documento congelado renderiza sempre pelo pipeline CLI desta skill; se o cliente exigir o acabamento da camada no congelado, o caminho reprodutível é exportar o HTML para PNG/SVG e embutir a imagem no pipeline. Contrato e fallback: `spec-feature/references/adapters.md`, seção apresentação.

**Conformidade com a regra de HTML autocontido:** esta skill segue [html-autocontido.md](../spec-feature/references/html-autocontido.md) **parcialmente, por decisão registrada** — o pipeline é print-first (`@page`, unidades em `pt`, tipografia serifada fixa herdada dos contratos), então os invariantes de tela (tokens, tema, `focus-visible`) não se aplicam a um PDF/DOCX assinável. A divergência e o "quando corrigir" vivem na tabela de implementações da própria regra.
