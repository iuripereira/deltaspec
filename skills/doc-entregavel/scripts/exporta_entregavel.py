...
def filtra_linguagem(md, linguagem):
    """Corta o documento pelos dois H1 de Parte. Documento sem Partes passa inteiro.

    O documento declara `# Parte I — ...` (executiva) e `# Parte II — ...` (técnica);
    o que vem antes da Parte I (título, preâmbulo, sumário) acompanha os dois recortes.
    """
    if linguagem == 'completa':
        return md
    partes = re.split(r'(?m)^# Parte (I{1,2}) — ', md)
    if len(partes) < 3:               # nao declara Partes: nada a filtrar
        return md
    cabeca = partes[0]
    blocos = {partes[i]: partes[i + 1] for i in range(1, len(partes) - 1, 2)}
    alvo = 'I' if linguagem == 'executiva' else 'II'
    if alvo not in blocos:
        raise SystemExit(f'--linguagem {linguagem}: o documento nao tem "# Parte {alvo} — "')
    return f'{cabeca}# Parte {alvo} — {blocos[alvo]}'


def desescapa_cifrao(md):
    """`\\$` -> `$`. O md escapa o cifrao para o Obsidian/GitHub nao lerem LaTeX; o
    python-markdown nao tem `$` na lista de escapaveis e imprimiria a contrabarra.
    O pandoc (docx) ja resolve sozinho — aplicar nos dois so normaliza."""
    return md.replace('\\$', '$')


def desliga_links(md):
    """pdf/docx sao autocontidos: [texto](alvo) vira texto. Ancora interna e imagem ficam."""
    return re.sub(r'(?<!!)\[([^\]]+)\]\((?!#)[^)]+\)', r'\1', md)


def svg_para_png(md, base):
    """docx nao rasteriza SVG. Troca por .png quando o derivado existe ao lado.

    ponytail: sem o png, a linha fica como esta e a figura falta — ruidoso de proposito,
    e melhor que um docx que abre sem a figura e sem ninguem notar.
    """
    def troca(m):
        png = pathlib.Path(base) / (m.group(1) + '.png')
        return f'({m.group(1)}.png)' if png.exists() else m.group(0)
    return re.sub(r'\(([^)]+)\.svg\)', troca, md)
...
def exporta_pdf(args, md):
    ...
    mdx = markdown.Markdown(extensions=['tables', 'fenced_code', 'sane_lists', 'md_in_html', 'toc', 'footnotes'],
...
def selftest():
    ...
    # --- corte por Parte (delta-092) ---
    CORTE = ("# T\n\nintro\n\n# Parte I — Requisitos\n\ncorpoexec\n\n"
             "# Parte II — Anexos\n\ncorpotec\n")
    exe, tec = filtra_linguagem(CORTE, 'executiva'), filtra_linguagem(CORTE, 'tecnica')
    assert 'corpoexec' in exe and 'corpotec' not in exe
    assert 'corpotec' in tec and 'corpoexec' not in tec
    assert filtra_linguagem(CORTE, 'completa') == CORTE
    assert 'intro' in filtra_linguagem(CORTE, 'executiva'), 'preâmbulo antes da Parte I fica nas duas'
    assert filtra_linguagem("# T\n\nsó isso\n", 'executiva') == "# T\n\nsó isso\n", 'sem Partes: passa inteiro'
    # --- política de link do pdf/docx (delta-092) ---
    LINKS = "veja [os critérios](criterios.md) e a [issue](https://x.tld/A-1); âncora [§5](#5) e ![f](d/a.svg)\n"
    assert desliga_links(LINKS) == "veja os critérios e a issue; âncora [§5](#5) e ![f](d/a.svg)\n"
    assert desescapa_cifrao('custa R\\$ 10 e US\\$ 2') == 'custa R$ 10 e US$ 2'
    print('selftest: corte por Parte e de-link OK')
    with tempfile.TemporaryDirectory() as tmp:
...
def main(argv):
    ...
    ap.add_argument('--linguagem', choices=['executiva', 'tecnica', 'completa'], default='completa',
                    help='recorte do documento pelos H1 de Parte (default: completa)')
    args = ap.parse_args(argv)
    if args.linguagem != 'completa' and (args.anexo or args.assinatura):
        raise SystemExit('capa de assinatura so sai com --linguagem completa: o recorte deixa '
                         'ancoras de anexo penduradas e nao e o artefato assinado')
    md = desescapa_cifrao(desliga_links(filtra_linguagem(le_markdown(args.entrada), args.linguagem)))
    md = svg_para_png(md, pathlib.Path(args.entrada).resolve().parent)
