...
| Ambiente | Destino |
|---|---|
| Repo com `debts/` | `${CLAUDE_PLUGIN_ROOT}/skills/handoff/scripts/debito.py novo` grava em `debts/ativos/` já validado — ele calcula o `DT-NNN` e escreve o formato; a gramática dos campos é do `debts/README.md` daquele repo |
| Repo com `DEBT.md` legado | `debito.py migrar` converte o registro e o cadastro segue pelo `novo` — cadastrar é pelo comando, escrever à mão não é rota |
| Repo que declara `deltaspec.registro: proprio` | o `novo` cria `debts/` do template e cadastra no mesmo comando — o registro nasce no primeiro item, não no init |
| Sem registro alcançável | mecânica em [references/destinos-fora-de-repo.md](references/destinos-fora-de-repo.md) — rota **local** por padrão (vira `DT-NNN` no registro de captura), **obsidian** só a pedido, bloco colável como último recurso; destino em repo git fecha com commit, sem push |
