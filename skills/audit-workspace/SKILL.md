...
O script `audit_workspace.py` — desta skill — hospeda os **dois** blocos, que rodam disjuntos: sem flag só os W (comportamento histórico, inalterado), com `--apenas-git` só os G. A `git-guard` é dona da doutrina (catálogo de anti-padrões, perfis, padrões de segredo) e não tem motor de varredura próprio, para não duplicar `detect_mode()`/`find_repo_roots()`/formato de relatório — a duplicação que o W10 existe para caçar.
