...
O script `audit_workspace.py` — desta skill — hospeda os **três** blocos, que rodam disjuntos: sem flag só os W (comportamento histórico, inalterado), com `--apenas-git` só os G, com `--apenas-dotclaude` só os D da `project-review-dotclaude` (delta-123) — as duas flags são exclusivas. A `git-guard` é dona da doutrina (catálogo de anti-padrões, perfis, padrões de segredo) e não tem motor de varredura próprio, para não duplicar `detect_mode()`/`find_repo_roots()`/formato de relatório — a duplicação que o W10 existe para caçar.

A flag `--profundidade N` (default 1) é da varredura profunda que a `git-guard` usa. O default preserva o alcance histórico byte a byte: descer é decisão explícita de quem chama, nunca comportamento novo (R56).
...
5. **Registro.** Achado confirmado pelo usuário vira `DT-NNN` em `debts/ativos/` do repo dono, cadastrado por `debito.py novo` no fluxo que `deltaspec:handoff` já cobre — esta skill não escreve em nenhum arquivo do alvo.
...
| W9 | repo/workspace | Gate citado (crase/code-span) num `CLAUDE.md`, sem aparecer em nenhum arquivo **rastreado pelo git** sob `.githooks/`, `scripts/` ou `.github/workflows/` — `.git/hooks/` local não conta, nunca é versionado. **Alçada pela localização, não pela citação:** só é gate o que o git rastreia sob `scripts/`, `.githooks/`, `.github/` ou `hooks/`; módulo-fonte (`lib/db.py`), script na raiz e citação que não resolve ficam de fora (delta-087) |
...
O W9 é **conservador por desenho** (delta-087): ler qualquer code-span terminado em `.py` rendeu **78% de ruído** na primeira auditoria real — sete módulos-fonte citados em prosa de arquitetura cobrados como se fossem gates. O sinal confiável é onde o arquivo mora, não como a prosa o escreve. A troca é explícita: gate legítimo fora daquelas quatro pastas deixa de ser vigiado, o que custa um falso negativo silencioso — mais barato que o ruído que faz o operador parar de ler a saída. Script na raiz do repositório fica fora por decisão: é ambíguo por natureza, e incluir a raiz reabriria o ruído para qualquer `.py` solto.
