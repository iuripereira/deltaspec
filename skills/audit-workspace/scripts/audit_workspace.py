...
Bloco D — `.claude/` só com o que o Claude Code lê, mais handoffs/ (skill
project-review-dotclaude, delta-123). Os checks D1–D8 e o catálogo moram na skill:
skills/project-review-dotclaude/references/catalogo.md — aqui só a fiação e o relatório.

Uso: audit_workspace.py [DIR] [--profundidade N] [--excluir PADRAO]... [--apenas-git | --apenas-dotclaude]
  ...
  --apenas-dotclaude roda só o bloco D. Exclusiva com --apenas-git: os blocos são disjuntos.
...
Exit 0 = sem achado; 1 = achado(s); 2 = alvo não é repo nem workspace reconhecível, ou
--apenas-git junto de --apenas-dotclaude.
...
from segredos import arquivo_ignorado, bundle_gerado, casar  # noqa: E402
# Mesma justificativa: a project-review-dotclaude é dona dos checks D (delta-123) e este
# script só descobre repositórios e imprime — os blocos seguem disjuntos.
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "project-review-dotclaude" / "scripts"))
from dotclaude import auditar as auditar_dotclaude, ORDEM as ORDEM_D, SEVERIDADE as SEVERIDADE_D  # noqa: E402
...
# W9: o sinal de que um `.py`/`.sh` citado é gate — e não módulo-fonte citado em prosa de
# arquitetura — é ONDE ELE MORA, nunca a forma da citação (delta-087, DT-088: ler qualquer
# code-span rendeu 78% de ruído na primeira auditoria real). Raiz do repo fica de fora por
# decisão: script solto na raiz é ambíguo, e incluí-la reabriria o ruído.
PASTAS_GATE = ("scripts/", ".githooks/", ".github/", "hooks/")
...
def _resolve_gate(citado: str, rastreados: list[str]) -> str | None:
    """Devolve o caminho rastreado do gate citado, ou None se ele não está sob alçada do W9.

    Pura (sem I/O): `rastreados` são caminhos relativos à raiz do repo, como o git os lista.
    Citação com barra casa por sufixo de caminho; sem barra, por nome de arquivo. Quando o
    mesmo nome mora em dois lugares, basta UMA morada sob pasta de gate para tratar como
    gate — conservador na direção de vigiar, nunca na de calar.
    """
    if "/" in citado:
        moradas = [f for f in rastreados if f == citado or f.endswith("/" + citado)]
    else:
        moradas = [f for f in rastreados if f.rsplit("/", 1)[-1] == citado]
    for f in moradas:
        # comparação por segmento: "lib/scripts/x.py" conta, "descriptions/x.py" não
        if any(("/" + pasta) in ("/" + f) for pasta in PASTAS_GATE):
            return f
    return None
...
def check_w9_gate_without_hook(repos: list[Path], root: Path) -> list[str]:
    ...
    for repo in repos:
        ...
        for script_path in sorted(cited):
            gate = _resolve_gate(script_path, rastreados)
            if gate is None:
                continue  # módulo-fonte, script da raiz ou citação que não resolve (delta-087)
            if Path(gate).name not in hook_text:
                violations.append(
...
COBRANCA = {
    "cliente":  {"G1", "G2", "G3", "G4", "G5", "G6", "G7", "G8", "G9", "G10"},
    "rigido":   {"G1", "G2", "G3", "G4", "G5", "G6", "G7", "G8", "G9", "G10"},
    "padrao":   {"G1", "G2", "G3", "G5", "G6", "G7", "G8", "G9", "G10"},
    "rascunho": {"G1"},
}
# Os três lugares em que o GitHub lê CODEOWNERS; a projeto-infra cria em .github/.
LOCAIS_CODEOWNERS = ("CODEOWNERS", ".github/CODEOWNERS", "docs/CODEOWNERS")
    ...
    for repo in repos:
        ...
        for rel in rastreados:
            ...
            try:
                ...
                continue
            for nome, linha in casar(texto, incluir_generico=not bundle_gerado(rel)):
...
def check_g8_atributos_e_donos(repos: list[Path], root: Path) -> list[tuple[Path, str]]:
    """G8 (delta-090): sem .gitattributes o diff sofre com fim de linha; sem CODEOWNERS o
    review não tem dono. Nasce junto com o `instalar`, que é o que cria os dois."""
    achados = []
    for repo in repos:
        faltas = []
        if not (repo / ".gitattributes").is_file():
            faltas.append(".gitattributes")
        if not any((repo / p).is_file() for p in LOCAIS_CODEOWNERS):
            faltas.append("CODEOWNERS")
        if faltas:
            achados.append((repo, f"[G8] sem {' nem '.join(faltas)} — {_rotulo(repo, root)}: "
                                  "`git-guard instalar` cria os dois"))
    return achados


def _branch_default(repo: Path) -> str:
    ref = _git(repo, "symbolic-ref", "--short", "refs/remotes/origin/HEAD")
    if ref and "/" in ref:
        return ref.split("/", 1)[1]
    for nome in ("main", "master"):
        if _git(repo, "rev-parse", "--verify", "-q", f"refs/heads/{nome}"):
            return nome
    return _git(repo, "rev-parse", "--abbrev-ref", "HEAD")


def check_g9_branch_integrada(repos: list[Path], root: Path) -> list[tuple[Path, str]]:
    """G9 (delta-090): branch já integrada na default e não apagada. SÓ LISTA — apagar
    branch é destrutivo e já custou um PR dependente fechado por engano neste projeto."""
    achados = []
    for repo in repos:
        default = _branch_default(repo)
        if not default or default == "HEAD":
            continue
        atual = _git(repo, "rev-parse", "--abbrev-ref", "HEAD")
        locais = [b for b in _git(repo, "branch", "--merged", default,
                                  "--format=%(refname:short)").splitlines()
                  if b and b not in (default, atual)]
        # refname completo: `%(refname:short)` encurta origin/HEAD para "origin" e o filtro
        # por sufixo /HEAD não o vê — medido neste repositório na primeira execução.
        remotas = [b.removeprefix("refs/remotes/")
                   for b in _git(repo, "branch", "-r", "--merged", default,
                                 "--format=%(refname)").splitlines()
                   if b and not b.endswith("/HEAD") and b.split("/")[-1] != default]
        todas = locais + remotas
        if todas:
            amostra = ", ".join(todas[:3]) + (f" e mais {len(todas) - 3}" if len(todas) > 3 else "")
            achados.append((repo, f"[G9] {len(todas)} branch(es) já integrada(s) em {default} e não "
                                  f"apagada(s) — {_rotulo(repo, root)}: {amostra} (só lista; apagar é "
                                  "decisão humana)"))
    return achados


def check_g10_gitignore_sujo(repos: list[Path], root: Path) -> list[tuple[Path, str]]:
    """G10 (delta-090): .gitignore modificado e não commitado — a correção foi feita e
    nunca salva, e o próximo `git add -A` versiona o que ela ia excluir."""
    achados = []
    for repo in repos:
        if _git(repo, "status", "--porcelain", "--", ".gitignore"):
            achados.append((repo, f"[G10] .gitignore modificado e não commitado — "
                                  f"{_rotulo(repo, root)}: a correção foi feita e nunca salva"))
    return achados
...
def rodar_bloco_g(repos: list[Path], root: Path) -> tuple[list[str], list[str], int]:
    ...
    for nome, achados in (
        ...
        ("G8", check_g8_atributos_e_donos(repos, root)),
        ("G9", check_g9_branch_integrada(repos, root)),
        ("G10", check_g10_gitignore_sujo(repos, root)),
    ):
...
def rodar_bloco_d(repos: list[Path], root: Path) -> tuple[list[str], list[str], int]:
    """Bloco D (delta-123): mesmo contrato do bloco G — agrupa por achado, ordena por
    severidade e conta achados, nunca linhas de cabeçalho (R108)."""
    por_check: dict[str, list[tuple[Path, str]]] = {}
    for repo in repos:
        for check, detalhe in auditar_dotclaude(repo):
            por_check.setdefault(check, []).append((repo, f"[{check}] {_rotulo(repo, root)}: {detalhe}"))
    cobrados: list[str] = []
    informativos: list[str] = []
    n_achados = 0
    for check in ORDEM_D:
        achados = por_check.get(check, [])
        if not achados:
            continue
        if SEVERIDADE_D[check] == "informativo":
            informativos.extend(f"  {linha}" for _, linha in achados)
            continue
        cobrados.append(f"[{check}] {len({r for r, _ in achados})} repositório(s) afetado(s):")
        cobrados.extend(f"  {linha}" for _, linha in achados)
...
def main() -> None:
    ...
    bloco = parser.add_mutually_exclusive_group()
    bloco.add_argument("--apenas-git", action="store_true", dest="apenas_git")
    bloco.add_argument("--apenas-dotclaude", action="store_true", dest="apenas_dotclaude")
    ...
    # Os três blocos são DISJUNTOS por desenho, não por economia: a audit-workspace audita
    # consistência de referência entre repos, a git-guard audita higiene de git e a
    # project-review-dotclaude audita o que mora em .claude/. Misturar as saídas apagaria a
    # fronteira que as três SKILL.md declaram.
    n_achados = 0
    if args.apenas_git:
        cobrados_g, info_g, n_achados = rodar_bloco_g(repos, target)
        violations += cobrados_g
        info += info_g
    elif args.apenas_dotclaude:
        cobrados_d, info_d, n_achados = rodar_bloco_d(repos, target)
        violations += cobrados_d
        info += info_d
    else:
        if mode == "repo":
    ...
    print(f"audit-workspace: modo {mode} — {target}"
          ...
          + (" — bloco dotclaude (project-review-dotclaude)" if args.apenas_dotclaude else ""))
    ...
    if info:
        ...
        print("\n".join(info))
    if violations:
        total = n_achados if (args.apenas_git or args.apenas_dotclaude) else len(violations)
...
def selftest() -> None:
    ...
    # W9 — alçada pela localização real do arquivo (CT1-CT6, delta-087)
    # A função é pura: recebe os caminhos rastreados e devolve o caminho do gate ou None.
    rastreados_fix = [
        "scripts/gate.py", ".githooks/pre-commit", ".github/workflows/ci.yml",
        ".claude/hooks/guarda.py", ".github/scripts/versao.py",
        "lib/db.py", "pages/N_Nome.py", "deploy.sh",
        "descriptions/relatorio.py", "skills/x/scripts/gate.py",
    ]
    # CT1 — as quatro pastas de gate resolvem
    assert _resolve_gate("scripts/gate.py", rastreados_fix) == "scripts/gate.py"
    assert _resolve_gate("pre-commit", rastreados_fix) == ".githooks/pre-commit"
    assert _resolve_gate("versao.py", rastreados_fix) == ".github/scripts/versao.py"
    assert _resolve_gate("guarda.py", rastreados_fix) == ".claude/hooks/guarda.py"
    # CT2 — módulo-fonte fica fora (o ruído de 78% medido em 2026-08-24)
    assert _resolve_gate("lib/db.py", rastreados_fix) is None
    assert _resolve_gate("pages/N_Nome.py", rastreados_fix) is None
    # CT3 — citação que não resolve para arquivo rastreado fica fora (DT-091 é dono da classe)
    assert _resolve_gate("lib/filtros.py", rastreados_fix) is None
    assert _resolve_gate("inexistente.py", rastreados_fix) is None
    # CT4 — script na raiz fica fora, por decisão registrada na spec
    assert _resolve_gate("deploy.sh", rastreados_fix) is None
    # CT5 — sufixo de caminho não confunde 'descriptions/' com 'scripts/'
    assert _resolve_gate("descriptions/relatorio.py", rastreados_fix) is None
    assert _resolve_gate("relatorio.py", rastreados_fix) is None
    # CT6 — mesmo nome em duas moradas, uma sob pasta de gate: resolve como gate
    assert _resolve_gate("gate.py", ["lib/gate.py", "scripts/gate.py"]) is not None
    assert _resolve_gate("gate.py", ["lib/gate.py", "outro/gate.py"]) is None
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        _write(root, {
            "repo-a/CLAUDE.md": "Gate: `scripts/gate.py`. Módulo: `lib/db.py`. Solto: `deploy.sh`.\n",
            "repo-a/scripts/gate.py": "pass\n",
            "repo-a/lib/db.py": "pass\n",
            "repo-a/deploy.sh": "echo\n",
        })
        # a alçada do W9 se apura contra o que o git RASTREIA: antes do commit, nada resolve
        r_sem_commit = _run([str(repo)])
        assert "[W9]" not in r_sem_commit.stdout, \
            f"citação que não resolve para arquivo rastreado não é achado (delta-087):\n{r_sem_commit.stdout}"

        subprocess.run(["git", "-C", str(repo), "add", "."], check=True)
        subprocess.run(["git", "-C", str(repo), "commit", "-q", "-m", "x"], check=True)
        r = _run([str(repo)])
        assert "[W9]" in r.stdout, r.stdout
        # CT7 — o achado que o check existe para pegar sobrevive; o ruído medido não entra
        assert "scripts/gate.py" in r.stdout, r.stdout
        assert "lib/db.py" not in r.stdout, f"módulo-fonte não é gate (DT-088):\n{r.stdout}"
        assert "deploy.sh" not in r.stdout, f"script da raiz fica fora por decisão:\n{r.stdout}"
    ...
    # G1 com bundle gerado (DT-162): a variável `token` do viewer do archify em
    # docs/diagrams/*.html não é segredo; o mesmo valor fora do bundle é, e uma chave
    # real dentro do bundle continua acusada — o recorte desliga só o padrão genérico
    with tempfile.TemporaryDirectory() as d:
        root = Path(d)
        repo = root / "repo-a"
        _git_init(repo, remote="git@github.com:example/repo-a.git")
        _identidade(repo)
        (repo / "docs" / "diagrams" / "x").mkdir(parents=True, exist_ok=True)
        # montados em runtime pelo mesmo motivo do caso acima
        opaco = "Zq9" + "Lm2Vx7Pk4Rt8Nw3Bs6Hy"
        aws = "AK" + "IA" + "7QW3ZKJ9PLMN2XVD"
        _commit(repo, "docs: a", "docs/diagrams/x/p.html", f'var token = "{opaco}";\n')
        _commit(repo, "feat: b", "conf.js", f'var token = "{opaco}";\n')
        _commit(repo, "docs: c", "docs/diagrams/x/q.html", f"AWS_KEY={aws}\n")
        textos = [a[1] for a in check_g1_segredo([repo], root)]
        assert not any("p.html" in x for x in textos), f"variável do viewer não é segredo: {textos}"
        assert any("conf.js:1" in x for x in textos), f"fora do bundle o genérico vale: {textos}"
        assert any("q.html:1" in x and "chave-aws" in x for x in textos), \
            f"segredo real dentro do bundle continua acusado: {textos}"
    ...
    # G8 — .gitattributes e CODEOWNERS: falta acusa, e CODEOWNERS em .github/ vale
    with tempfile.TemporaryDirectory() as d:
        root = Path(d)
        repo = root / "repo-a"
        _git_init(repo, remote="git@github.com:example/a.git")
        achado = check_g8_atributos_e_donos([repo], root)
        assert achado and ".gitattributes" in achado[0][1] and "CODEOWNERS" in achado[0][1], achado
        _write(root, {"repo-a/.gitattributes": "* text=auto\n", "repo-a/.github/CODEOWNERS": "* @x\n"})
        assert check_g8_atributos_e_donos([repo], root) == [], \
            "CODEOWNERS em .github/ é onde a projeto-infra o cria — tem de valer"

    # G9 — branch já integrada e não apagada: lista, nunca apaga
    with tempfile.TemporaryDirectory() as d:
        root = Path(d)
        repo = root / "repo-a"
        _git_init(repo)
        _identidade(repo)
        _commit(repo, "feat: base", "a.txt", "x\n")
        subprocess.run(["git", "-C", str(repo), "branch", "feat/velha"], check=True)
        achado = check_g9_branch_integrada([repo], root)
        assert achado and "feat/velha" in achado[0][1], achado
        assert _git(repo, "branch", "--list", "feat/velha"), "o check lista, nunca apaga"
        subprocess.run(["git", "-C", str(repo), "branch", "-d", "feat/velha"], check=True,
                       capture_output=True)
        assert check_g9_branch_integrada([repo], root) == []

    # G10 — .gitignore modificado e não commitado: a correção foi feita e nunca salva
    with tempfile.TemporaryDirectory() as d:
        root = Path(d)
        repo = root / "repo-a"
        _git_init(repo)
        _identidade(repo)
        _commit(repo, "chore: ignore", ".gitignore", ".env\n")
        assert check_g10_gitignore_sujo([repo], root) == []
        _write(root, {"repo-a/.gitignore": ".env\n.venv\n"})
        achado = check_g10_gitignore_sujo([repo], root)
        assert achado and ".gitignore" in achado[0][1], achado

    # Bloco D — só a fiação; os checks têm selftest próprio em dotclaude.py (delta-123)
    with tempfile.TemporaryDirectory() as d:
        root = Path(d)
        repo = root / "repo-a"
        _git_init(repo)
        _write(root, {"repo-a/.claude/plans/p.md": "x\n"})
        subprocess.run(["git", "-C", str(repo), "add", ".claude/plans/p.md"], check=True)
        # D1 (ALTO), D2 (MÉDIO) e D6 (informativo): ordem por severidade, D6 fora da conta
        _write(root, {"repo-a/.claude/commands/c.md": "x\n", "repo-a/HANDOFF.md": "índice\n",
                      "repo-a/.claude/handoffs/HANDOFF_velho_2020_01_01.md": "x\n"})
        subprocess.run(["git", "-C", str(repo), "add", "."], check=True)
        r = _run([str(repo), "--apenas-dotclaude"])
        assert r.returncode == 1 and "bloco dotclaude" in r.stdout, r.stdout + r.stderr
        assert "[D1] 1 repositório(s) afetado(s):" in r.stdout, r.stdout  # contrato do R108, igual ao G
        assert r.stdout.index("[D1]") < r.stdout.index("[D2]") < r.stdout.index("informativo") \
            < r.stdout.index("[D6]"), r.stdout
        assert "FAIL (2 achado(s))" in r.stdout, r.stdout
        assert "[W" not in r.stdout and "[G" not in r.stdout, r.stdout
        # só D6 sobrando: informativo não derruba o resultado
        subprocess.run(["git", "-C", str(repo), "rm", "-rq", "--cached", ".claude/plans", ".claude/commands"], check=True)
        (repo / ".gitignore").write_text(".claude/plans/\n.claude/commands/\n", encoding="utf-8")
        subprocess.run(["git", "-C", str(repo), "add", ".gitignore"], check=True)
        r3 = _run([str(repo), "--apenas-dotclaude"])
        assert r3.returncode == 0 and "RESULTADO: PASS" in r3.stdout and "[D6]" in r3.stdout, r3.stdout
        r2 = _run([str(repo), "--apenas-dotclaude", "--apenas-git"])
    ...
    if doutrina.is_file():
        ...
        for linha in doutrina.read_text(encoding="utf-8").splitlines():
            ...
            if len(celulas) == 11 and celulas[0] in COBRANCA:
    ...
    print("selftest: OK (9 fixtures W + 8 fixtures G — W1-W3 e W5-W10, derivação de perfil "
          "com override, G1 sem vazar valor e com o recorte de bundle gerado, G2 resolvendo o caminho, G5 ignorando merge, "
          "a sincronia entre perfis.md e COBRANCA e a fiação do bloco D; W4 removido, ver plan.md)")
