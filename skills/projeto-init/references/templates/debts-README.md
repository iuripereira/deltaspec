...
Próximo `DT-NNN` = maior ID existente + 1, contando **`ativos/` e `_archive/`** — numeração global, IDs **nunca reutilizados**. Item `débito`/`pendência` nasce com `fila`, Local, Gatilho e Origem; sem a triagem dos eixos, a fila sai com erro cobrando (nunca invente valor).
