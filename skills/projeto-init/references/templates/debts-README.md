...
**As três naturezas.** `débito` é problema técnico a corrigir quando o gatilho disparar. `pendência` é trabalho ou decisão que sobrou de uma delta arquivada. `guarda` é um aviso para **não** "consertar" histórico imutável — não é trabalho, é proteção. Na dúvida, o critério é este: **se existe o que fazer, não é guarda** — item com conserto proposto é `débito`, ainda que pequeno; e risco medido, revogável e com gatilho de reavaliação é `débito` no estado `aceito`, não guarda, porque `vigente` promete permanência a algo que se desfaz com um comando.
...
| `vigente` | guarda permanente; nunca será "resolvida" | nada além da descrição — e é **exclusivo da guarda**: o script recusa guarda em outro estado e natureza pontuável em `vigente` |
...
Ele injeta `encerrado: AAAA-MM-DD`, escreve `#### Como foi quitado` (linguagem amigável — detalhe técnico fica no commit/PR e no comentário do issue), troca `- **Ticket:**` por `- **Encerrado:**` (com o Ticket como referência quando não há `--ticket-ref`) e move com `git mv` para `_archive/`. Regenere o índice: `python3 ${CLAUDE_PLUGIN_ROOT}/skills/handoff/scripts/debito.py indice .` (o `fila` avisa se esquecer).

O item **muda de lugar, nunca some**: o arquivo arquivado não é podado. A PR que quita DT com Ticket carrega o vínculo que o `debito.py quitar` imprime — `Closes #N` no GitHub; no Jira, a chave no título da PR (ou na branch) mais a regra de automação do projeto, com o comentário de fechamento **em nível de negócio** (resultado e impacto, sem jargão). Ticket que ficou aberto? Feche citando a quitação.
