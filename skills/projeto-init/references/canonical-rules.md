...
Placeholders `{{assim}}` são preenchidos a partir da inspeção do projeto. Os toggles `{{se houver develop: ...}}` e `{{se não houver develop: ...}}` resolvem-se pela **topologia planejada** — a decidida na detecção e confirmada no plano —, não pela existência da branch naquele momento: a `develop` só nasce depois, no `projeto-infra` (regra no passo 3 da `SKILL.md`). Texto em PT-BR — é o idioma padrão de todos os projetos do usuário; o cabeçalho boilerplate fica em inglês por convenção.
...
- **Keep a Changelog 1.0.0** — toda mudança notável entra no `CHANGELOG.md` (na raiz), agrupada em `Adicionado / Mudado / Corrigido / Removido / Obsoleto / Segurança`. **A PR não edita o `CHANGELOG.md`**: ela deixa um fragmento em `changelog.d/<slug>.<categoria>.md` com uma linha, e `montar_changelog.py` acrescenta os fragmentos ao `## [Não lançado]` no release (mescla: o bullet que já estava lá fica) e os apaga. Duas PRs vivas editando a mesma seção conflitam a cada merge; dois arquivos com nomes diferentes, nunca. No corte, `montar_changelog.py --liberar X.Y.Z` (chamado por `/deltaspec:criar-release`) renomeia `[Não lançado]` → `## [X.Y.Z] - AAAA-MM-DD` e abre um `[Não lançado]` novo — mesma contagem de bullets antes e depois.
- **O dono do CHANGELOG é um só: o fragmento curado.** A versão é input humano — o argumento de `/deltaspec:criar-release X.Y.Z`, validado por `versao_corte.py`: tem de ser o **sucessor imediato** da última tag no nível do bump que os commits desde ela exigem (hotfix = PATCH) — e nenhuma ferramenta gera texto de CHANGELOG a partir de commits. **Nunca dois escrevendo o CHANGELOG.**
...
- **Correlação commit → bump:** `fix` = PATCH · `feat` = MINOR · `!`/`BREAKING CHANGE` = MAJOR. O maior vence. **Tag e Release criadas pela skill `/deltaspec:release` no merge da PR de release/hotfix{{se houver develop: , e a sync `main → develop` pela mesma skill, sempre por `sync/X.Y.Z` com a guarda do CHANGELOG por conjunto de bullets}}; nunca à mão — salvo a versão lançada antes da adoção, sem PR de release, que o dono taggeia no commit que a lançou (a `/deltaspec:criar-release` documenta).** PRs só de documentação não geram tag.
{{se houver develop: - **Origem do corte:** `/deltaspec:criar-release X.Y.Z` corta `release/X.Y.Z` da `develop`; `--hotfix` corta da `main`.}}
{{se não houver develop: - **Origem do corte:** `/deltaspec:criar-release X.Y.Z` corta da `main`; a PR de release entra por squash.}}
...
{{se houver develop: - **Topologia com `develop`:** `feature/*`, `fix/*` e `bugfix/*` saem da `develop` e voltam a ela por PR; `release/X.Y.Z` sai da `develop` e entra na `main`; `hotfix/*` sai da `main`. A volta `main → develop` (sync) é da skill `/deltaspec:release`, nunca à mão — só ela mantém `main ⊆ develop`.}}
{{se não houver develop: - **Topologia sem `develop`:** PR de escopo direto na `main`, por squash; o corte de release parte da `main`.}}
...
- **Merge por squash na PR de escopo** — histórico da base = 1 commit por PR; a mensagem do squash segue Conventional Commits.{{se houver develop:  A PR de release/hotfix e a de sync entram por **merge commit** (ADR-0045 do deltaspec) — só ele preserva `main ⊆ develop`.}}
