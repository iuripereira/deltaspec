...
{{se houver CI: - **Valide no CI:** um job reprova PR com commits fora do padrão, e `check_changelog.py` reprova entrada fora do formato (categoria, tamanho, referência de PR, ordenação). Repositório cujo fluxo não passa por PR roda o gate com `--sem-pr`.}}
```
