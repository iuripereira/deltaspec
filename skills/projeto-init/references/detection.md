...
| docs-sdd · site-estatico | ADRs, HANDOFF.md, debts/ | specs, IDs RN/RNF/DEP, regra de propagação |
| docs-sdd · tooling | ADRs, IDs estáveis, HANDOFF.md, debts/, doc em camadas | specs estáticos (o ciclo sdd cobre), regra de propagação (sem DATA_DICTIONARY) |
...
| `debts/` · site-estatico e tooling | `debts/README.md` do template `debts-README.md` + pasta `ativos/` | nada (⚠️ = criar só no 1º débito/pendência é aceitável se o usuário preferir; `_archive/`, `licoes/` e `LICOES.md` nascem no primeiro uso) |
...
| --- | --- | --- | --- |
| app-web | completo | duro | completo (rulesets main+develop+release/hotfix, husky/commitlint, CI com `release-check`/`sync-check`/`fechar-issues-no-merge`, corte da `develop`, CodeRabbit) |
| backend | completo | duro | completo |
| tooling | completo | recomendado (dispensável por task com justificativa no plan.md) | mínimo (ruleset main, CI lint/test; sem develop — corte da `main`) |
...
| `CLAUDE.md` | ✅ | ✅ | ✅ | ✅ | ✅ |
| `CHANGELOG.md` | ✅ | ✅ | ⚠️ | ❌ | ✅ |
| `changelog.d/` + README (montador por fragmento — template `changelog.d-README.md`; espelha a linha `CHANGELOG.md`) | ✅ | ✅ | ⚠️ | ❌ | ✅ |
| `HANDOFF.md` (índice; `.claude/handoffs/` nasce com o 1º handoff, sem scaffold) | ✅ | ✅ | ✅ | ✅ | ✅ |
| `.gitignore` (append secrets/PII) | ✅ | ✅ | ✅ | ✅ | ✅ |
| `docs/adrs/` + `ADR-TEMPLATE.md` | ✅ | ✅ | ⚠️ | ❌ | ⚠️ |
| `debts/` (espelha a linha de `docs/adrs/`) | ✅ | ✅ | ⚠️ | ❌ | ⚠️ |
| `specs/` + `TRUTH.md` (ciclo — substitui `docs/specs/` estático) | ✅ | ✅ | ⚠️ | ❌ | ✅ |
| `GLOSSARY.md` | ✅ | ✅ | ⚠️ | ⚠️ | ❌ |
| `DATA_DICTIONARY.md` | ✅ | ✅ | ❌ | ✅ | ❌ |
| `doc-profile.yaml` (decisão de documentação visual — ADR-0009) | ✅ | ✅ | ❌ | ❌ | ⚠️ |

Regra prática (alinhada à matriz acima): **workspace-dados** recebe `CLAUDE.md` (com módulo `data-workspace`) + `HANDOFF.md` + `DATA_DICTIONARY.md` (dicionário semântico por campo — definição, domínio, steward, sensibilidade, origem, qualidade) + `.gitignore` de PII — **sem** CHANGELOG, docs SDD (adrs/specs) ou testes. `GLOSSARY.md` só se houver termos de domínio além do schema. **site-estatico** recebe a tríade leve, sem `testing`/`architecture`.

Nos tipos com ciclo, o scaffold de specs é `specs/` + `specs/TRUTH.md` (copie o template de `${CLAUDE_PLUGIN_ROOT}/skills/spec-feature/references/templates/TRUTH.md`; o template delta-spec assume o papel do antigo SPEC-TEMPLATE). `docs/adrs/`, debts/, CHANGELOG e GLOSSARY seguem inalterados — o `plan.md` do ciclo gera ADR em `docs/adrs/` quando a decisão for durável. **Não crie** `docs/specs/` + `SPEC-TEMPLATE.md` nesses tipos (repos existentes com `docs/specs/` ficam como estão — não migre sem pedido).
...
## Sinais de modelo de dados existente (governa o passo 5 da SKILL.md)

Procure sinais **só** nos tipos com `DATA_DICTIONARY.md` = ✅ na matriz de scaffold (app-web, backend, workspace-dados). A oferta lista o que encontrou; um sinal basta.

| Sinal | Exemplos |
| --- | --- |
| Migrations | `migrations/`, `alembic/`, `db/migrate/` |
| Schema declarativo | `prisma/schema.prisma`, `drizzle.config.*`, `schema.dbml` já existente |
| DDL solto | `*.sql` com `CREATE TABLE` (fora de vendored/`node_modules`) |
| Models de ORM | `models.py`/`models/` (Django, SQLAlchemy, Sequelize, Eloquent) |
| Artefato deltaspec prévio | `docs/data-model.md`, `DATA_DICTIONARY.md` com linhas de campo preenchidas |
| Dados tabulares (workspace-dados) | `.csv`/`.xlsx` com cabeçalho estável — o cabeçalho é o schema |

Sinal encontrado → a SKILL.md oferece invocar `modelo-dados` (gate próprio, opcional). Nenhum sinal → relato de 1 linha, sem invocação.
