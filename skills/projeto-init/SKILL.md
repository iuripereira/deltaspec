...
2. **Confirmar** um plano curto com o usuário — tipo detectado, topologia (com ou sem `develop`), módulos que entram, arquivos a criar, e o toggle de co-autoria (`Co-Authored-By`, opcional). Um único gate; espere o "ok".
3. **Montar o `CLAUDE.md`**: leia `references/canonical-rules.md`, selecione os módulos que a matriz de `detection.md` marca para o tipo, na ordem do arquivo, preenchendo `{{placeholders}}`. Os toggles `{{se houver develop: ...}}` / `{{se não houver develop: ...}}` resolvem-se pela **topologia planejada** — a decidida na detecção e confirmada no passo 2: com `develop` quando o perfil `infra` do tipo em `detection.md` é `completo` (app-web, backend) ou a `develop` já existe no remoto; sem, nos demais; o override do plano vence —, **não** pela existência da branch neste momento: num repo novo a `develop` só nasce no passo 6, pelo `projeto-infra`, e lê-la aqui daria o texto sem `develop` a quem vai tê-la. Inclua sempre os módulos `header` + `core`. Copie o texto das regras canônicas; adapte só os exemplos ao stack. **Não substitua uma regra canônica por uma versão sua "equivalente".**
...
6. **Oferecer o módulo infra** (gate próprio, opcional): apresente o perfil da coluna `infra` de detection.md e, se o usuário aceitar, invoque a skill **projeto-infra** com o tipo detectado. Falha de infra (sem rede, `gh` não autenticado) **não trava o init** — reporte e siga; a skill é invocável avulsa depois. Recusa ou falha com topologia com `develop`: avise que o `CLAUDE.md` já saiu com o texto da `develop` (passo 3) — crie-a pelo `projeto-infra` ou remonte o `CLAUDE.md` sem ela.
