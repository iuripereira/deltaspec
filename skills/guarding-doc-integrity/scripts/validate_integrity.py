...
Uso:
  validate_integrity.py [DIR | caminho/deps.toml]   (default: ./deps.toml) — C1+C2+C3
  validate_integrity.py --links-only DIR            — só C3, sem exigir deps.toml (delta-036,
                                                       reaproveitado pela skill audit-workspace
                                                       em repo sem manifesto)
...
# classe que apodrece em rename (achado ALTO do review da delta-027). A profundidade é
# livre (`(../)+`): o mesmo atalho escrito em arquivo aninhado sobe mais níveis — caso
# real dos débitos arquivados em .claude/debts/, `../../../../issues/N` (delta-041).
...
def _raiz_principal(root: Path) -> Path:
    """Raiz do repositório git principal — não a do worktree em que `root` está checado
    (delta-085). `--git-common-dir` aponta pro `.git` do repo principal mesmo dentro de
    um worktree linked (`.claude/worktrees/<nome>/`), o que `--show-toplevel` não faz (ele
    devolve o topo do PRÓPRIO worktree). Sem git no PATH ou `root` fora de qualquer repo
    (fixtures do selftest, `tempfile.TemporaryDirectory()` puro) → cai no próprio `root`,
    ...
    try:
        ...
        if saida.returncode == 0 and saida.stdout.strip():
            return Path(saida.stdout.strip()).parent
    except (OSError, subprocess.SubprocessError):
        pass
    return root


def scan_links_c3(root: Path, scan_links: set[Path]) -> tuple[int, list[tuple[Path, int, str]]]:
    """Núcleo puro do C3 — passa uma vez pelos links vivos de scan_links, devolve
    (checados, mortos) com o achado morto já estruturado (path, linha, target), sem
    formatar nem imprimir. Único dono do laço e da regra de skip (http/mailto/#//
    atalho do GitHub/placeholder) — `check_links()` formata pro CLI; `audit-workspace`
    importa esta função direto (delta-036) em vez de reimplementar ou de fazer parse
    de texto formatado de outro processo.

    Link que ESCAPA da raiz do repo (sobe mais níveis que a profundidade do arquivo em
    relação a `root`) resolve a partir da raiz do repo PRINCIPAL, não do parent físico do
    arquivo — em worktree aninhado os dois divergem (delta-085, achado num repo
    consumidor). Link que fica dentro do repo é imune: resolve igual em qualquer nesting."""
    checked = 0
    dead: list[tuple[Path, int, str]] = []
    raiz_principal = None  # calculada preguiçosamente — só quando algum link escapa
    for path in sorted(scan_links):
        profundidade = len(path.relative_to(root).parent.parts)
        for i, line in linhas_vivas(path.read_text(encoding="utf-8")):
            for target in MD_LINK.findall(line):
                ...
                alvo = target.split("#")[0]
                subidas = 0
                while alvo[subidas * 3:subidas * 3 + 3] == "../":
                    subidas += 1
                if subidas > profundidade:
                    if raiz_principal is None:
                        raiz_principal = _raiz_principal(root)
                    dest = raiz_principal / path.relative_to(root).parent / alvo
                else:
                    dest = path.parent / alvo
                if not dest.exists():
                    dead.append((path, i, target))
    return checked, dead


def check_links(root: Path, scan_links: set[Path]) -> tuple[list[str], int]:
    """C3 formatado pro CLI. main() (com deps.toml) e links_only() (sem manifesto)
    chamam esta função — nenhuma reimplementa a resolução."""
    checked, dead = scan_links_c3(root, scan_links)
    violations = [f"[C3] link morto — {p.relative_to(root)}:{i} → {t}" for p, i, t in dead]
    return violations, checked


def links_only(dir_arg: str) -> None:
    """--links-only DIR: só C3, sem deps.toml — delta-036."""
    root = Path(dir_arg)
    if not root.is_dir():
        die(f"diretório não encontrado: {root}")
    root = root.resolve()
    varrido = collect(root, ["**/*.md"])
    scan_links = varrido - collect(root, EXCLUDE_LINKS_PADRAO)
    violations, checked = check_links(root, scan_links)
...
def main() -> None:
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--links-only":
        if len(sys.argv) < 3:
            die("--links-only exige um diretório: validate_integrity.py --links-only DIR")
        links_only(sys.argv[2])
        return
    ...
    link_violations, checked = check_links(root, scan_links)
...
"""
    ...
    # delta-059: espelho declarado no manifesto que não existe no disco é violação C1,
    # não silêncio — o ramo existia sem fixture (achado residual do P2-6 da auditoria 01)
    sem_espelho = rodar({
        "PRD.md": "Limite de R$ 2.000.\n",
        # CLAUDE.md (espelho declarado) deliberadamente ausente
    })
    assert sem_espelho.returncode == 1, f"espelho inexistente deveria falhar:\n{sem_espelho.stdout}"
    assert "[C1] arquivo do manifesto não existe: CLAUDE.md" in sem_espelho.stdout, \
        f"o C1 tem de nomear o arquivo ausente:\n{sem_espelho.stdout}"
    ...
    # delta-041: o mesmo atalho em arquivo aninhado sobe mais níveis — a forma casa em
    # qualquer profundidade (caso real: débitos arquivados em .claude/debts/)
    atalho_fundo = rodar({
        "PRD.md": "Limite de R$ 2.000.\n",
        "CLAUDE.md": "Limite: R$ 2.000.\n",
        "a/b/n.md": "Ticket [#88](../../../../issues/88) e PR [#9](../../../../pull/9).\n",
    }, manifesto.replace('scan_globs = ["*.md"]', 'scan_globs = ["**/*.md"]'))
    assert atalho_fundo.returncode == 0, \
        f"atalho do GitHub em arquivo aninhado virou link morto:\n{atalho_fundo.stdout}"
    ...
    # delta-036: --links-only roda C3 sem deps.toml — nenhum manifesto é escrito na fixture
    def rodar_links_only(arquivos: dict) -> "subprocess.CompletedProcess":
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            for nome, txt in arquivos.items():
                caminho = root / nome
                ...
                caminho.write_text(txt, encoding="utf-8")
            return subprocess.run(
                [sys.executable, __file__, "--links-only", str(root)],
                capture_output=True, text=True,
            )

    links_limpo = rodar_links_only({
        "PRD.md": "Ver [vizinho](CLAUDE.md).\n",
        "CLAUDE.md": "Sem link nenhum.\n",
    })
    assert links_limpo.returncode == 0, f"--links-only limpo deveria passar:\n{links_limpo.stdout}"
    assert "RESULTADO: PASS" in links_limpo.stdout

    links_sujo = rodar_links_only({
        "PRD.md": "Ver [sumiu](nao-existe.md).\n",
        "specs/_archive/001-x/plan.md": "Ver [extinto](sumiu.md).\n",  # default protege histórico
    })
    assert links_sujo.returncode == 1, f"--links-only sujo deveria falhar:\n{links_sujo.stdout}"
    assert links_sujo.stdout.count("[C3] link morto") == 1, \
        f"só o de PRD.md — _archive/ fica fora do C3 pelo default, mesmo sem deps.toml:\n{links_sujo.stdout}"

    # delta-085: link que escapa da raiz do repo resolve a partir da raiz do repo
    # PRINCIPAL, não do parent físico do arquivo — worktree aninhado (.claude/worktrees/)
    # muda a profundidade física sem mudar a intenção do link (achado num repo consumidor)
    with tempfile.TemporaryDirectory() as d:
        base = Path(d)
        repo = base / "repo"
        repo.mkdir()
        subprocess.run(["git", "init", "-q"], cwd=repo, check=True, capture_output=True)
        subprocess.run(["git", "config", "user.email", "t@t.io"], cwd=repo, check=True, capture_output=True)
        subprocess.run(["git", "config", "user.name", "t"], cwd=repo, check=True, capture_output=True)
        alvo_dir = base / "irmao"
        alvo_dir.mkdir()
        (alvo_dir / "y.md").write_text("alvo\n", encoding="utf-8")
        (repo / "debts" / "ativos").mkdir(parents=True)
        (repo / "debts" / "ativos" / "x.md").write_text(
            "Ver [y](../../../irmao/y.md).\n", encoding="utf-8")
        subprocess.run(["git", "add", "-A"], cwd=repo, check=True, capture_output=True)
        subprocess.run(["git", "commit", "-q", "-m", "x"], cwd=repo, check=True, capture_output=True)

        # checkout principal: já passava antes da correção — não pode regredir
        direto = subprocess.run(
            [sys.executable, __file__, "--links-only", str(repo)], capture_output=True, text=True,
        )
        assert direto.returncode == 0, f"link de escape no checkout principal:\n{direto.stdout}"

        wt = repo / ".claude" / "worktrees" / "w1"
        subprocess.run(["git", "worktree", "add", "-q", "-b", "w1", str(wt), "HEAD"],
                        cwd=repo, check=True, capture_output=True)
        aninhado = subprocess.run(
            [sys.executable, __file__, "--links-only", str(wt)], capture_output=True, text=True,
        )
        assert aninhado.returncode == 0, \
            f"link de escape virou falso positivo em worktree aninhado (DT-062):\n{aninhado.stdout}"
        assert "RESULTADO: PASS" in aninhado.stdout, aninhado.stdout

    print(f"selftest: OK ({12} fixtures, C1/C2/C3 detectados; C3 com conjunto próprio, recorte, sintaxe citada, modo --links-only e raiz do repo principal em worktree aninhado)")
