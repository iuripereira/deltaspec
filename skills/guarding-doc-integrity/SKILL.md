...
### 5. Âncora do framework (`ancora.py`, delta-104)

O projeto declara em `doc-profile.yaml` a versão do deltaspec que consome, e o gate a confronta com o plugin **realmente em execução**:

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/guarding-doc-integrity/scripts/ancora.py verificar .
python3 ${CLAUDE_PLUGIN_ROOT}/skills/guarding-doc-integrity/scripts/ancora.py fixar . v1.51.1
```

`fixar` escreve a chave e **relê o arquivo** antes de devolvê-lo, recusando o que não for SemVer — nunca edite a chave à mão (mesma doutrina do `debito.py novo`). A verificação roda em três pontos: este comando, o **C17** do `check_cycle.py` e o bloco 5 do hook de pré-commit da `git-guard`.

**O teto é a detecção.** O plugin roda de onde o harness o instalou e nenhum arquivo no repositório muda isso; `/plugin install` não aceita versão. O que a âncora entrega é que a deriva pare de depender de alguém lembrar. Duas omissões deliberadas, ambas exit 0: **chave ausente** (estado de todo projeto já inicializado — a chave não propaga retroativamente, como a cauda do perfil no C11) e **`$CLAUDE_PLUGIN_ROOT` ausente** (o CI de um consumidor não tem plugin; reprovar ali quebraria todo consumidor — RNF2).

Âncora é **pino exato**: plugin mais novo deriva tanto quanto mais velho. Quem quer faixa não quer âncora.
...
| Fora da âncora (C17) | `ancora.py verificar .` mostra as duas versões; reancore com `fixar` **ou** volte o plugin — lendo o CHANGELOG entre elas antes |
| Projeto sem âncora e você quer ancorar | `ancora.py fixar . vX.Y.Z`; sem a chave o gate se omite e nada muda |
