...
1. **Rotear os achados novos.** Débito, pendência ou guarda descoberto na sessão e ainda sem registro → arquivo novo `debts/ativos/DEBT_DT-NNN-<topico>.md` (próximo número livre — contando `ativos/`, `_archive/` e, em repo em migração, `.claude/debts/`), com frontmatter (`id`, `natureza`, `estado`, `descricao`, `aberto`; `fila` se pontuável) e os campos no corpo. Item de natureza `débito`/`pendência` nasce com `fila` (`P·J·Pr`), **Local** e **Gatilho** — gramática em [references/debito.md](references/debito.md). **Item que muda para `quitado`/`descartado` arquiva no mesmo commit** (ADR-0030): frontmatter ganha o estado final e `encerrado: AAAA-MM-DD`, o corpo ganha a seção `#### Como foi quitado` (2–4 frases amigáveis — o que doía, o que foi feito, o que muda; técnico fica no commit/PR e no comentário do issue GitHub; fechamento no Jira em nível de negócio) e troca **Ticket** por **Encerrado**, e o arquivo move com `git mv` para `debts/_archive/DEBT_DT-NNN-<topico>_<AAAA>_<MM>_<DD>.md` (data do encerramento) — links **não mudam** (mesma profundidade); o arquivo arquivado **nunca é podado**. Post-mortem sem ação pendente → `debts/LICOES.md`, com data e desfecho. Depois de rotear ou quitar, **regenere o índice**: `python3 ${CLAUDE_PLUGIN_ROOT}/skills/handoff/scripts/debito.py indice .` (o `DEBT.md` é projeção — ADR-0031). Projeto sem `debts/` → crie do template da `projeto-init` (ou `debito.py migrar` num registro legado).
2. **Escrever o handoff da sessão** em `.claude/handoffs/HANDOFF_<topico>_<AAAA>_<MM>_<DD>.md` (crie a pasta se não existir), a partir do template `handoff-sessao.md` da `projeto-init` — cabeçalho (Data · Status · Branch/Commit) + as 5 seções; tópico em kebab-case curto que desambigua sessões do mesmo dia; seção sem dado real sai. Se houver delta em curso em `specs/NNN-*/`, cite número, fase em que parou e o veredito do último gate — em dúvida, rode `python3 ${CLAUDE_PLUGIN_ROOT}/skills/spec-feature/scripts/check_cycle.py specs/NNN-nome`.
3. **Atualizar o índice `HANDOFF.md`** (teto ~30 linhas): `Agora` com o estado corrente em 2–4 linhas, a sessão nova no topo de `Sessões recentes` (link + resumo de 1 linha, ~10 entradas — as antigas saem da lista, os arquivos ficam) e o campo "Atualizado em".
...
5. **Imprimir o prompt de retomada.** Encerre com o prompt que inicia a próxima sessão — uma linha, apontando o `HANDOFF.md` (o índice leva ao handoff mais recente):
...
Regra completa em [references/debito.md](references/debito.md) — ida, volta, idempotência e degradação vivem lá; o script **não acessa a rede**, quem executa os comandos é você.
...
| Esquecer a delta em curso | Passo 2 a cita quando existe `specs/NNN-*/` |
