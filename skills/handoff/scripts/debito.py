...
de deprecação apontando o `migrar`.
  ...
  vinculo   lista o vínculo PR → ticket dos DTs quitados num intervalo git e o confere no texto da PR
...
Uso: debito.py novo [ROOT] --natureza N --descricao T [--fila P·J·Pr] [--local L]
     ...
     debito.py vinculo [ROOT] --base REF [--head REF] [--pr-arquivo F] [--alvo B --default B] [--branch B]
     debito.py --selftest
...
from projecao import (ARQUIVO_PERFIL, LINK_MD, alvo_local, corpo_ticket, emitir_sh_acli,
                      etiquetas, ler_destino_tickets, ler_registro)
...
def die(msg: str) -> None:
    # stderr: o stdout do `vinculo` é colado pelo criar-release no corpo da PR (delta-119)
...
# Template do README do registro — vive na projeto-init, que é a dona do scaffold.
# Caminho relativo ao próprio script (skills/handoff/scripts/ -> skills/), nunca
# absoluto de máquina: o step "Portabilidade (RNF5)" do job `ci` reprova o contrário.
TEMPLATE_README = (Path(__file__).resolve().parents[2]
                   / "projeto-init/references/templates/debts-README.md")


# Sondas que o `git check-ignore` confere antes de criar o registro: a pasta, o README e
# um nome de item. Conferir só a pasta deixava passar `debts/**/*.md` — pasta visível,
# conteúdo ignorado.
SONDAS_REGISTRO = ("debts/README.md", f"{DIR_ATIVOS}/", f"{DIR_ATIVOS}/DEBT_DT-000-sonda.md")


def sondas_ignoradas(root: Path):
    """Saída do `git check-ignore -v` para as sondas ignoradas, ou None se nenhuma é.

    Usa o `git()` do script, e com ele a mesma degradação (RNF2): fora de repositório
    git, ou sem o binário, não há o que checar e o cadastro segue. O caso que a checagem
    existe para pegar é o `.gitignore` por lista branca (`/*` ignora tudo, exceções entram
    uma a uma): ali o registro nasceria no disco e invisível ao git, e nada no script volta
    a olhar para ele — o `quitar` só falharia meses depois, num `git mv` de arquivo não
    rastreado.
    """
    return git(root, "check-ignore", "-v", *SONDAS_REGISTRO)


def _normaliza(texto: str) -> str:
    """Sem espaço repetido, caixa nem acento — a chave de comparação da idempotência."""
    return " ".join(_ascii(texto or "").lower().split())


def chave_do_item(texto: str) -> tuple:
    """(sintoma, origem) normalizados, lidos do arquivo **como gravado**.

    Os dois lados da comparação passam por aqui — o item candidato é renderizado antes de
    ser comparado —, então o link reescrito da Origem e o título sem link do frontmatter
    nunca fazem a mesma pendência parecer duas. O sintoma sai só do frontmatter e a Origem
    do bloco de campos, pelos mesmos padrões que o parser do arquivo usa.
    """
    fm = FRONTMATTER.match(texto)
    sintoma = dict(FM_CHAVE.findall(fm.group(1))).get("descricao", "") if fm else ""
    origem = dict(CAMPO.findall(texto)).get("Origem", "")
    return _normaliza(sintoma), _normaliza(origem)


def duplicata(root: Path, candidato: str):
    """(caminho do ativo igual, id do encerrado igual) para o texto de um item candidato.

    A chave é sintoma + origem. Ativo igual bloqueia o cadastro; encerrado igual **não**
    bloqueia: item que volta é reincidência, e abrir de novo é o registro honesto — o ID
    arquivado sai junto só para virar aviso. O ID vem do **nome**, como o R121 exige para
    a união de IDs: item malformado não pode derrubar a conta.
    """
    alvo = chave_do_item(candidato)
    igual = reincide = None
    for pasta in (DIR_ATIVOS, DIR_ARCHIVE):
        for arquivo_md in sorted((root / pasta).glob("*.md")):
            nome = ARQUIVADO.match(arquivo_md.name)
            if not nome or chave_do_item(arquivo_md.read_text(encoding="utf-8",
                                                              errors="replace")) != alvo:
                continue
            if pasta == DIR_ATIVOS:
                igual = igual or arquivo_md
            else:
                reincide = reincide or nome.group(1)
    return igual, reincide


def criar_registro(root: Path) -> list:
    """Materializa `debts/README.md` + `debts/ativos/` do template e devolve o que criou.

    Só é chamada com `deltaspec.registro: proprio` declarado (delta-120, R2): criar
    registro por dedução transformaria um `novo` errado — raiz trocada, diretório
    enganado — em pasta nova no lugar errado.

    README existente não é sobrescrito: repositório que já escreveu o seu não perde o
    texto por um cadastro, a mesma proteção que o `migrar` já dá. A lista devolvida é o
    que o `cmd_novo` desfaz se o item for recusado — registro pela metade não fica.
    """
    if not TEMPLATE_README.is_file():
        die(f"template do registro não encontrado em {TEMPLATE_README.name} — a "
            f"instalação do plugin está incompleta; nada foi criado")
    # DEBT.md legado com itens: o registro vazio daria DT-001 por cima dos IDs dele
    # (ids_em_disco não lê o legado) e travaria o migrar, que é no-op com ativos/
    # povoado — o legado nunca mais entraria no registro.
    legado = root / "DEBT.md"
    if legado.is_file() and itens_legados(legado.read_text(encoding="utf-8"))[0]:
        die("DEBT.md legado com itens — converta com `debito.py migrar` antes de "
            "cadastrar: o registro criado agora reutilizaria os IDs dele e o migrar "
            "não rodaria mais; nada foi criado")
    ignoradas = sondas_ignoradas(root)
    if ignoradas:
        die("o registro nasceria ignorado pelo .gitignore, e nada voltaria a olhar para "
            f"ele:\n{ignoradas.rstrip()}\nlibere esses caminhos (em .gitignore de lista "
            "branca: `!/debts/`, `!/debts/README.md`, `!/debts/ativos/`, "
            "`!/debts/ativos/*.md`) e rode de novo; nada foi criado")
    criados = [p for p in (root / "debts", root / DIR_ATIVOS) if not p.exists()]
    (root / DIR_ATIVOS).mkdir(parents=True, exist_ok=True)
    readme = root / "debts/README.md"
    if not readme.exists():
        readme.write_text(TEMPLATE_README.read_text(encoding="utf-8"), encoding="utf-8")
        criados.append(readme)
    nomes = ", ".join(f"{c.relative_to(root)}{'/' if c.is_dir() else ''}" for c in criados)
    print(f"[registro] criado {nomes} ({ARQUIVO_PERFIL} declara deltaspec.registro: proprio)")
    return criados
...
def validar(itens: list, root: Path) -> list:
    ...
    for item in itens:
        ...
        # guarda ⟺ vigente (R2, delta-116). Bicondicional de propósito: guarda 'aberta'
        # promete um trabalho que não existe, e item pontuável 'vigente' promete
        # permanência a algo que se paga. Estado final é isento — a guarda cai quando a
        # premissa cai, e o arquivo encerrado é o registro disso.
        if st not in ESTADOS_FINAIS and (item.get("natureza") == "guarda") != (st == "vigente"):
            erros.append(f"{ident}: natureza '{item.get('natureza')}' com estado '{st}' — "
                         f"'vigente' é o estado da guarda permanente e só dela")
...
# Legenda da fila no cabeçalho do índice (R1, delta-115): resume os eixos para quem
# lê os rótulos das seções (J9/J3/J1) sem sair do arquivo. **Sem link de propósito** —
# o cabeçalho só linka o que existe (lição da delta-045) e o consumidor pode não ter
# `debts/README.md`, que segue dono canônico da regra completa.
LEGENDA_FILA = (
    "> Fila `P·J·Pr` — **P**rincipal (o que custa pagar) · **J**uros (o atrito já "
    "observado) · **Pr**obabilidade (chance de incidir de novo), cada eixo em 1 · 3 · 9; "
    "o score `(J × Pr) / P` ordena a fila e é derivado na leitura, nunca gravado."
)
...
def render_item(item: dict, reescrever_corpo: bool = True):
    ...
    `reescrever_corpo=False` desliga os 2 níveis **na prosa** quando o artefato citado
    não está no repositório do registro (delta-120, R2): ali `x` → `../../x` produz
    caminho que resolve dentro do registro e aponta arquivo que só existe no repo da
    sessão — link morto e silencioso, porque a prosa não passa pelo `validar`.

    Os **campos** seguem reescritos mesmo assim, e não é inconsistência: ali os 2
    níveis são metade de um protocolo — `parse_arquivo_ativo` os desfaz na leitura, e
    é essa volta que faz o `validar` resolver o `Local` a partir da raiz. Desligá-los
    mudaria o significado do caminho gravado, e todo `Local` cross-repo passaria a
    reprovar como inexistente. Prosa não tem essa volta; por isso só ela muda.
    """
    rl = reescreve_links if reescrever_corpo else (lambda t: t)
    ident, pend = item["id"], []
...
def monta_item(ident, natureza, descricao, fila, local, gatilho, origem, ticket,
    ...
    """
    # Estado inicial deriva da natureza (R1, delta-116) e só aqui: guarda é permanente
    # por definição, então nasce 'vigente'. Literal fixo era o defeito — o comando
    # decidia num lugar e nada conferia no outro, e toda guarda nascia 'aberto'.
    item = {"id": ident, "natureza": natureza,
            "status": "vigente" if natureza == "guarda" else "aberto",
...
def na_mesma_raiz(root: Path) -> bool:
    """O comando roda de dentro da árvore do próprio registro?

    Proxy de "o artefato citado pertence a este repositório" (delta-120, R2): exato para
    `debito.py novo .`, para `novo ..` de um subdiretório e para a captura que aponta o
    registro de outro repositório. Fica fora do `cmd_novo` porque quem sabe disso é quem
    leu a linha de comando — passar o booleano mantém a função testável.
    """
    # ponytail: proxy pelo diretório corrente — chamada ao próprio repositório feita de
    # fora da árvore dele perde a reescrita da prosa; se esse uso aparecer, trocar por
    # flag explícita de fronteira na linha de comando.
...
def cmd_novo(root: Path, natureza, descricao, fila, local, gatilho, origem, ticket,
             corpo, hoje=None, reescrever_corpo: bool = True) -> int:
    ...
    """
    # Recusas que só dependem da entrada vêm antes de qualquer escrita: depois do
    # criar_registro, recusar deixaria registro pela metade no disco.
    ...
    if len(descricao) > DESCRICAO_LIMITE:
        ...
        return 1
    registro = ler_registro(root)
    if registro == "externo":
        # Declaração explícita vence descoberta por disco (R4): o repositório pode ter
        # debts/ativos/ e ainda assim mandar a captura para fora. Sem esta guarda, a
        # presença da pasta silenciaria a declaração — e o item nasceria no lugar que
        # o dono do repositório disse que não queria.
        die(f"{ARQUIVO_PERFIL} declara deltaspec.registro: externo — a pendência deste "
            f"diretório é cadastrada no registro de captura declarado no CLAUDE.md do "
            f"usuário; aponte o comando para a raiz dele")
    criados = []
    if not modo_novo(root):
        if registro != "proprio":
            die(f"{DIR_ATIVOS}/ não encontrado — declare `deltaspec.registro: proprio` "
                f"no {ARQUIVO_PERFIL} para o comando criar o registro, converta o legado "
                f"com `debito.py migrar`, ou cadastre no registro de captura apontando o "
                f"comando para a raiz dele")
        criados = criar_registro(root)
    hoje = hoje or date.today()
    _, previa, _ = render_item(monta_item("DT-000", natureza, descricao, fila, local, gatilho,
                                          origem, ticket, corpo, hoje),
                               reescrever_corpo=reescrever_corpo)
    igual, reincide = duplicata(root, previa)
    if igual:
        print(f"[idempotente] {ARQUIVADO.match(igual.name).group(1)} já registra esta "
              f"descrição com esta origem — nada foi criado")
        print(igual.relative_to(root))
        return 0
    if reincide:
        print(f"[reincidência] {reincide} registrou isto e foi encerrado — item novo, "
              f"porque o que volta não é duplicata")
    ...
    item = monta_item(proximo_id(disco | remotos), natureza, descricao, fila, local,
                      gatilho, origem, ticket, corpo, hoje)
    if not reescrever_corpo:
        print("[fronteira] registro fora do diretório da sessão — caminhos citados na prosa "
              "ficam como texto, sem os 2 níveis de debts/ativos/")
    nome, conteudo, pendencias = render_item(item, reescrever_corpo=reescrever_corpo)
    ...
    if pendencias or erros:
        ...
        for c in reversed(criados):  # nem o registro que esta chamada criou para ele
            if c.is_file():
                c.unlink()
            else:
                c.rmdir()
...
# Chave que o campo Ticket grava (R52): GitHub como `[#N](…/issues/N)`, `#N`, `gh#N` ou a
# URL do issue; Jira como `PROJ-NNN`, solta ou dentro de link. O GitHub é tentado antes —
# valor do Jira nunca tem `#N` nem `issues/N`.
# ESPELHO: o molde fechar-issues-no-merge.yml da projeto-infra repete CHAVE_GH em shell
# (grep -oE), porque o CI do consumidor não tem o framework. Mudou um, mude o outro.
CHAVE_GH = re.compile(r"(?:^|\W)(?:gh)?#(\d+)\b|issues/(\d+)\b")
CHAVE_JIRA = re.compile(r"\b([A-Z][A-Z0-9_]+-\d+)\b")
# Palavras que o GitHub interpreta como fechamento no corpo da PR, seguidas ou não de
# dois-pontos (docs "Linking a pull request to an issue") — só agem em PR contra a default.
PALAVRAS_FECHAMENTO = ("close", "closes", "closed", "fix", "fixes", "fixed",
                       "resolve", "resolves", "resolved")


def valor_do_ticket(texto: str) -> str:
    """Valor do campo `- **Ticket:**` (âncora de início de linha); '' se ausente ou vazio."""
    valor = next((m.group(2).strip() for m in CAMPO.finditer(texto)
                  if m.group(1).strip() == "Ticket"), "")
    return "" if valor in VAZIO else valor


def chave_ticket(valor: str):
    """('github', 'N') · ('jira', 'PROJ-N') · None — a chave de um valor do campo Ticket."""
    if m := CHAVE_GH.search(valor):
        return "github", m.group(1) or m.group(2)
    if m := CHAVE_JIRA.search(valor):
        return "jira", m.group(1)
    return None


def linha_vinculo(chave) -> str:
    """A linha que a PR carrega: palavra de fechamento no GitHub, a chave literal no Jira."""
    tipo, valor = chave
    return f"Closes #{valor}" if tipo == "github" else valor


def vinculo_presente(chave, texto: str) -> bool:
    """O texto da PR (título + corpo) carrega o vínculo desta chave?"""
    tipo, valor = chave
    if tipo == "github":
        palavras = "|".join(PALAVRAS_FECHAMENTO)
        return re.search(rf"\b(?:{palavras}):?\s+#{valor}\b", texto, re.I) is not None
    return re.search(rf"(?<![A-Za-z0-9_-]){re.escape(valor)}(?!\d)", texto) is not None
...
def cmd_quitar(root: Path, dt_id: str, como: str, ticket_ref: str, hoje=None) -> int:
    ...
    valor_ticket = valor_do_ticket(corpo)
    # A referência leva o Ticket junto (depois do --ticket-ref, quando há): o arquivado
    # mantém o elo com o ticket que o projetou — antes o valor sumia na troca (delta-119)
    referencia = " · ".join(v for v in (ticket_ref.strip(), valor_ticket) if v)
    encerrado_linha = (f"- **Encerrado:** {data_iso} · {referencia}\n" if referencia
    ...
    if r.returncode != 0:
        ...
        return 1
    # `git mv` sobre um arquivo já sujo (o write_text acima) stagia a rename com o blob
    # antigo do índice, não o conteúdo que acabou de ser escrito — sem este `add`, quem
    # commitar sem tocar o arquivo de novo grava a versão de antes da quitação (achado
    # real: DT-130 foi ao _archive/ do main com o estado anterior ao quitar).
    subprocess.run(["git", "-C", str(root), "add", str(destino.relative_to(root))], check=True)
    print(destino.relative_to(root))
    print(f"regenere o índice: python3 {Path(__file__).name} indice .")
    if valor_ticket:
        chave = chave_ticket(valor_ticket)
        if fm.get("estado") == "descartado":
            print(f"ticket {valor_ticket}: descarte fecha à mão, como 'não planejado' — "
                  f"a palavra de fechamento fecharia como concluído")
        elif chave:
            onde = " (no título da PR ou na branch)" if chave[0] == "jira" else " (no corpo da PR)"
            print(f"vínculo para a PR: {linha_vinculo(chave)}{onde}")
        else:
            print(f"[aviso] Ticket '{valor_ticket}' sem chave reconhecível — confira o vínculo à mão")
    return 0


DATA_NO_NOME = re.compile(r"_\d{4}_\d{2}_\d{2}(?=\.md$)")  # sufixo de data do arquivado (R18)


def dts_quitados_no_intervalo(root: Path, base: str, head: str):
    """[(DT-NNN, valor do Ticket, estado final)] dos itens arquivados no intervalo.

    O Ticket vem da ÚLTIMA versão em ativos/ — o pai do commit que a apagou —, o que
    cobre também o item criado e quitado dentro do intervalo, que o diff não mostra
    como remoção. None = intervalo ilegível (ref inexistente, clone raso).
    """
    saida = git(root, "diff", "--name-status", "--no-renames", f"{base}...{head}",
                "--", DIR_ARCHIVE)
    if saida is None:
        return None
    itens = []
    for linha in saida.splitlines():
        status, _, caminho = linha.partition("\t")
        nome = PurePosixPath(caminho).name
        m = ARQUIVADO.match(nome)
        if status != "A" or not m:
            continue
        m_fm = FRONTMATTER.match(git(root, "show", f"{head}:{caminho}") or "")
        estado_final = dict(FM_CHAVE.findall(m_fm.group(1))).get("estado", "") if m_fm else ""
        em_ativos = f"{DIR_ATIVOS}/{DATA_NO_NOME.sub('', nome)}"
        # --full-history: no CI o head é o merge ref sintético, cujo 1º pai é a base; o item
        # criado e quitado na PR não existe em nenhum dos pais, e a simplificação padrão do
        # `git log -- <path>` descartaria o lado da PR (review da delta-119)
        apagou = (git(root, "log", "-1", "--full-history", "--format=%H", "--diff-filter=D",
                      f"{base}..{head}", "--", em_ativos) or "").strip()
        anterior = git(root, "show", f"{apagou}^:{em_ativos}") if apagou else None
        itens.append((m.group(1), valor_do_ticket(anterior or ""), estado_final))
    return sorted(itens, key=lambda i: int(i[0][3:]))


PREFIXO_RELEASE = "release/"  # branch do criar-release: a chave do Jira já vinculou na PR de escopo


def cmd_vinculo(root: Path, base: str, head: str, texto_pr, alvo: str, default: str,
                branch: str = "") -> int:
    """Vínculo PR → ticket dos DTs quitados no intervalo (delta-119).

    Sem `texto_pr`, imprime só as linhas — é o que o criar-release cola no corpo da PR de
    release. Com ele, confere e sai 1 nomeando cada vínculo ausente. O `Closes #N` vale em
    qualquer ponto do texto; a chave do Jira, só no título (1ª linha) ou na branch, que é
    onde a integração GitHub↔Jira vincula a PR. Avisos vão para o stderr, para o stdout
    continuar colável.
    """
    # Sem guarda de debts/ativos/: a PR que quita o ÚLTIMO ativo deixa a pasta vazia, que
    # some do git — a guarda deixaria passar justo essa PR. Repositório sem registro não
    # tem entrada em _archive/, e o diff vazio já responde "nada a vincular".
    itens = dts_quitados_no_intervalo(root, base, head)
    if itens is None:
        die(f"intervalo {base}...{head} ilegível — ref inexistente ou clone raso (fetch-depth: 0)")
    so_jira = bool(alvo and default and alvo != default)
    if so_jira:
        print(f"aviso: PR contra '{alvo}', não a default '{default}' — o GitHub ignora a palavra "
              f"de fechamento aqui; quem fecha é o fechar-issues-no-merge no merge e, sem ele, a PR "
              f"de release", file=sys.stderr)
    release = branch.startswith(PREFIXO_RELEASE)
    if release:
        print(f"aviso: PR de release ({branch}) — a chave do Jira já vinculou na PR que quitou "
              f"o DT; só os Closes #N são exigidos", file=sys.stderr)
    exigidos = []
    for ident, valor, estado_final in itens:
        if estado_final != "quitado" or not valor:
            continue
        chave = chave_ticket(valor)
        if chave is None:
            print(f"aviso: {ident}: Ticket '{valor}' sem chave reconhecível — confira à mão",
                  file=sys.stderr)
        elif not (so_jira and chave[0] == "github") and not (release and chave[0] == "jira"):
            exigidos.append((ident, chave))
    if not exigidos:
        print("nada a vincular nesta PR", file=sys.stderr)
    if texto_pr is None:
        for _, chave in exigidos:
            print(linha_vinculo(chave))
        return 0
    onde = {"github": texto_pr, "jira": texto_pr.split("\n", 1)[0] + "\n" + branch}
    faltou = False
    for ident, chave in exigidos:
        presente = vinculo_presente(chave, onde[chave[0]])
        faltou |= not presente
        dica = "" if presente or chave[0] == "github" else " — no título da PR ou na branch"
        print(f"[{'ok' if presente else 'ausente'}] {ident}: {linha_vinculo(chave)}{dica}")
    if faltou:
        print("> acrescente o vínculo marcado 'ausente' — `Closes #N` no corpo, a chave do Jira no "
              "título; sem ele, o merge não fecha o ticket (R18)")
        return 1
    return 0


def itens_legados(texto: str) -> tuple:
    """(itens, formato) de um DEBT.md legado — blocos primeiro, tabela antiga depois."""
    itens = parse_blocos(texto)
    if not itens and TABELA_ANTIGA.search(texto):
        return parse_tabela(texto), "tabela"
    return itens, "blocos"
...
def migrar(root: Path) -> int:
    ...
    itens, formato = itens_legados(caminho_debt.read_text(encoding="utf-8"))
    if not itens:
...
def main() -> None:
    ...
    p.add_argument("--base", default="", help="vinculo: ref de base do intervalo (ex.: origin/main)")
    p.add_argument("--head", default="HEAD", help="vinculo: ref de ponta do intervalo (default: HEAD)")
    p.add_argument("--pr-arquivo", help="vinculo: título + corpo da PR; sem ele, só imprime as linhas")
    p.add_argument("--alvo", default="", help="vinculo: branch alvo da PR")
    p.add_argument("--default", dest="branch_default", default="",
                   help="vinculo: branch default do repositório")
    p.add_argument("--branch", default="",
                   help="vinculo: branch da PR (head) — a chave do Jira também vale ali")
    a = p.parse_args()
    ...
    if a.comando == "novo":
        ...
        sys.exit(cmd_novo(root, a.natureza, a.descricao, a.fila, a.local, a.gatilho,
                          a.origem, a.ticket, corpo, reescrever_corpo=na_mesma_raiz(root)))
    if a.comando == "quitar":
        if not a.dt_id:
            die("quitar exige DT-NNN: debito.py quitar [ROOT] DT-NNN --como '...'")
        sys.exit(cmd_quitar(root, a.dt_id, a.como, a.ticket_ref))
    if a.comando == "vinculo":
        if not a.base:
            die("vinculo exige --base REF (ex.: origin/main)")
        if a.pr_arquivo and not Path(a.pr_arquivo).is_file():
            die(f"--pr-arquivo {a.pr_arquivo} não existe")
        texto = Path(a.pr_arquivo).read_text(encoding="utf-8") if a.pr_arquivo else None
        sys.exit(cmd_vinculo(root, a.base, a.head, texto, a.alvo, a.branch_default, a.branch))
...
def selftest_novo() -> None:
    ...
    # natureza × estado (R2, delta-116): bicondicional guarda ⟺ vigente, com os dois
    # lados vigiados — o defeito real era guarda nascendo 'aberto' e ninguém acusar
    assert "guarda" in erros_de(arquivo(raiz(), natureza="guarda", estado_item="aberto",
                                        local="", gatilho="", fila="")), \
        "guarda fora de vigente não acusada"
    assert "vigente" in erros_de(arquivo(raiz(), natureza="débito", estado_item="vigente",
                                         fila="P1·J1·Pr1")), \
        "vigente em natureza pontuável não acusada"
    r_g_fim = arquivo(raiz(), natureza="guarda", estado_item="quitado", local="", gatilho="",
                      fila="", encerrado="2026-01-02 · premissa caiu")
    assert carregar(r_g_fim) is not None, \
        "guarda encerrada acusada — a guarda cai quando a premissa cai (isenção do R2)"
    ...
    # o corte é o 1º heading: o cabeçalho explica o vocabulário (legenda, R1), as linhas
    # de item é que não podem materializar valor derivado (ADR-0031)
    cabecalho, projecao = idx.split("\n## ", 1)
    assert "score" not in projecao.lower() and "stale" not in projecao.lower(), \
        "índice materializou score/stale — só valor gravado e ordem entram (ADR-0031)"
    assert "**P**rincipal" in cabecalho and "**J**uros" in cabecalho and "**Pr**obabilidade" in cabecalho, \
        "cabeçalho sem a legenda dos três eixos da fila (R1)"
    assert "1 · 3 · 9" in cabecalho and "(J × Pr) / P" in cabecalho, \
        "legenda sem os degraus ou sem o score derivado (R1)"
    assert "](" not in LEGENDA_FILA, \
        "legenda com link: o cabeçalho só linka o que existe, e o consumidor pode não ter debts/README.md"
    ...
    # ---- delta-120 (R2, R4): registro declarado nasce no cadastro ----

    def recusou(root, descricao="Sintoma qualquer"):
        """(código de saída, saída impressa) de um cmd_novo que termina em die().

        Captura as duas saídas: o die() foi para stderr na delta-119, e só stdout
        aqui devolveria mensagem vazia com o código certo — o teste passaria a
        conferir a recusa sem conferir o que ela diz.
        """
        buf = io.StringIO()
        codigo = None
        with contextlib.redirect_stdout(buf), contextlib.redirect_stderr(buf):
            try:
                cmd_novo(root, "pendência", descricao, "P1·J1·Pr1", "[alvo](alvo.py)",
                         "quando doer", "sessão de teste", "", "Prosa.")
            except SystemExit as e:
                codigo = e.code
        return codigo, buf.getvalue()

    # (a) chave `proprio` sem debts/: o comando cria o registro e segue o cadastro
    r_cria = raiz()
    (r_cria / "doc-profile.yaml").write_text("deltaspec:\n  registro: proprio\n", encoding="utf-8")
    cod, voz = falado(cmd_novo, r_cria, "pendência", "Nasce com o registro", "P1·J1·Pr1",
                      "[alvo](alvo.py)", "quando doer", "sessão de teste", "", "Prosa.")
    assert cod == 0, f"com a chave declarada o cadastro deveria criar e seguir: {voz}"
    assert (r_cria / "debts/README.md").is_file(), "README do registro não nasceu"
    assert (r_cria / "debts/ativos").is_dir(), "pasta ativos/ não nasceu"
    assert len(list((r_cria / "debts/ativos").glob("DEBT_DT-001-*.md"))) == 1, \
        "registro recém-criado deveria começar em DT-001"
    assert voz.index("debts/README.md") < voz.index("DEBT_DT-001"), \
        "o que foi criado tem de ser dito antes do caminho do item"
    assert "# debts/" in (r_cria / "debts/README.md").read_text(encoding="utf-8"), \
        "README não veio do template da projeto-init"

    # (b) README autoral sem ativos/: a criação completa o registro sem sobrescrever o
    # texto que o dono já escreveu. Fixture própria — num registro já criado o
    # criar_registro nem roda, e o teste passaria sem conferir nada.
    r_autoral = raiz()
    (r_autoral / "doc-profile.yaml").write_text("deltaspec:\n  registro: proprio\n", encoding="utf-8")
    (r_autoral / "debts").mkdir()
    (r_autoral / "debts/README.md").write_text("# autoral\n", encoding="utf-8")
    assert quieto(cmd_novo, r_autoral, "pendência", "Segundo item", "P1·J1·Pr1",
                  "[alvo](alvo.py)", "quando doer", "sessão de teste", "", "Prosa.") == 0
    assert (r_autoral / "debts/ativos").is_dir(), "a criação não completou o registro"
    assert (r_autoral / "debts/README.md").read_text(encoding="utf-8") == "# autoral\n", \
        "a criação do registro sobrescreveu o README que o dono escreveu"

    # (c) sem a chave e sem debts/: recusa como antes, e a mensagem dá as três saídas
    r_sem_chave = raiz()
    cod, voz = recusou(r_sem_chave)
    assert cod == 2, "sem a chave o cadastro deveria recusar com código 2"
    assert not (r_sem_chave / "debts").exists(), "recusa deixou registro pela metade"
    for saida in ("deltaspec.registro", "migrar", "captura"):
        assert saida in voz, f"a recusa não nomeia a saída {saida!r}: {voz}"

    # (c2) recusa depois da criação não deixa registro pela metade: sem Local, ou com a
    # descrição acima do teto, o comando sai ≠ 0 e o disco fica como estava
    for extra, rotulo in (({"local": ""}, "sem Local"),
                          ({"descricao": "x" * (DESCRICAO_LIMITE + 1)}, "descrição longa")):
        r_meia = raiz()
        (r_meia / "doc-profile.yaml").write_text("deltaspec:\n  registro: proprio\n", encoding="utf-8")
        args = {"natureza": "pendência", "descricao": "Recusa depois de criar",
                "fila": "P1·J1·Pr1", "local": "[alvo](alvo.py)", "gatilho": "quando doer",
                "origem": "sessão", "ticket": "", "corpo": "Prosa."} | extra
        assert quieto(cmd_novo, r_meia, **args) != 0, f"{rotulo}: deveria recusar"
        assert not (r_meia / "debts").exists(), f"{rotulo}: recusa deixou registro pela metade"

    # (c3) chave `proprio` com DEBT.md legado povoado: criar o registro reutilizaria os
    # IDs do legado e travaria o migrar — recusa antes de escrever, apontando o migrar
    r_legado = raiz()
    (r_legado / "doc-profile.yaml").write_text("deltaspec:\n  registro: proprio\n", encoding="utf-8")
    (r_legado / "DEBT.md").write_text(CABECALHO_FIXTURE + linha("DT-007"), encoding="utf-8")
    cod, voz = recusou(r_legado, descricao="Colidiria com o legado")
    assert cod == 2, "DEBT.md legado com itens deveria recusar a criação do registro"
    assert not (r_legado / "debts").exists(), "recusou mas criou o registro"
    assert "migrar" in voz, f"a recusa não aponta o migrar: {voz}"

    # (d) chave `externo` vence a descoberta por disco: nem com debts/ativos/ cadastra aqui
    r_externo = arquivo(raiz())
    (r_externo / "doc-profile.yaml").write_text("deltaspec:\n  registro: externo\n", encoding="utf-8")
    antes = sorted(p.name for p in (r_externo / "debts/ativos").iterdir())
    cod, voz = recusou(r_externo, descricao="Vai para a captura")
    assert cod == 2, "com registro externo o cadastro local deveria recusar"
    assert "captura" in voz, f"a recusa não aponta o registro de captura: {voz}"
    assert sorted(p.name for p in (r_externo / "debts/ativos").iterdir()) == antes, \
        "recusa por registro externo não pode escrever no registro local"

    # (e) T3: registro que nasceria ignorado pelo git recusa ANTES de escrever. É o caso
    # do .gitignore por lista branca (`/*`), onde o registro ficaria no disco e invisível
    # ao git para sempre — e o script não volta a olhar para ele.
    r_ign = raiz()
    subprocess.run(["git", "-C", str(r_ign), "init", "-q"], check=True)
    (r_ign / ".gitignore").write_text("/*\n!/.gitignore\n", encoding="utf-8")
    (r_ign / "doc-profile.yaml").write_text("deltaspec:\n  registro: proprio\n", encoding="utf-8")
    cod, voz = recusou(r_ign, descricao="Nasceria ignorado")
    assert cod == 2, "registro que cai no gitignore deveria recusar"
    assert not (r_ign / "debts").exists(), "recusou mas escreveu assim mesmo"
    assert "gitignore" in voz.lower(), f"a recusa não nomeia o gitignore: {voz}"

    # (e2) conteúdo ignorado com a pasta visível também recusa — `debts/**/*.md` deixava
    # passar a checagem que olhava só debts/ativos/
    r_ign2 = raiz()
    subprocess.run(["git", "-C", str(r_ign2), "init", "-q"], check=True)
    (r_ign2 / ".gitignore").write_text("debts/**/*.md\n", encoding="utf-8")
    (r_ign2 / "doc-profile.yaml").write_text("deltaspec:\n  registro: proprio\n", encoding="utf-8")
    cod, voz = recusou(r_ign2, descricao="Conteudo ignorado")
    assert cod == 2 and not (r_ign2 / "debts").exists(), \
        f"registro com conteúdo ignorado deveria recusar sem escrever: {voz}"
    assert "debts/README.md" in voz, f"a recusa não nomeia o caminho ignorado: {voz}"

    # (f) T3: sem git a checagem se omite e o registro nasce — degradação (RNF2)
    r_semgit = raiz()
    (r_semgit / "doc-profile.yaml").write_text("deltaspec:\n  registro: proprio\n", encoding="utf-8")
    assert quieto(cmd_novo, r_semgit, "pendência", "Sem git nenhum", "P1·J1·Pr1",
                  "[alvo](alvo.py)", "quando doer", "sessão", "", "Prosa.") == 0
    assert (r_semgit / "debts/ativos").is_dir(), "sem git a criação deveria proceder"

    # (g) T5: dois gatilhos capturando a mesma coisa convergem para um item só
    r_idem = raiz()
    (r_idem / "doc-profile.yaml").write_text("deltaspec:\n  registro: proprio\n", encoding="utf-8")
    mesmo = ("pendência", "Cache falha na sessão", "P1·J1·Pr1", "[alvo](alvo.py)",
             "quando doer", "sessão de teste", "", "Prosa.")
    assert quieto(cmd_novo, r_idem, *mesmo) == 0
    cod, voz = falado(cmd_novo, r_idem, *mesmo)
    assert cod == 0, "repetição idempotente sai 0 — não é recusa, é convergência"
    assert len(list((r_idem / "debts/ativos").glob("*.md"))) == 1, "criou item duplicado"
    assert "DT-001" in voz, f"a repetição deveria nomear o DT existente: {voz}"

    # (h) T5: diferença cosmética não é pendência nova
    assert quieto(cmd_novo, r_idem, "pendência", "  cache falha na SESSAO  ", "P1·J1·Pr1",
                  "[alvo](alvo.py)", "quando doer", "sessão de teste", "", "Prosa.") == 0
    assert len(list((r_idem / "debts/ativos").glob("*.md"))) == 1, \
        "normalização de espaço, caixa e acento não pegou"

    # (i) T5: mesma frase vinda de outro lugar são dois fatos
    assert quieto(cmd_novo, r_idem, "pendência", "Cache falha na sessão", "P1·J1·Pr1",
                  "[alvo](alvo.py)", "quando doer", "outra sessão", "", "Prosa.") == 0
    assert len(list((r_idem / "debts/ativos").glob("*.md"))) == 2, \
        "origem diferente deveria abrir item novo"

    # (i2) link na Origem ou na descrição não esconde a repetição: a comparação é contra o
    # item como fica gravado (Origem com os 2 níveis, título sem o alvo do link)
    r_link2 = raiz()
    (r_link2 / "specs").mkdir()
    (r_link2 / "specs/x.md").write_text("x\n", encoding="utf-8")
    (r_link2 / "doc-profile.yaml").write_text("deltaspec:\n  registro: proprio\n", encoding="utf-8")
    for _ in range(2):
        assert quieto(cmd_novo, r_link2, "pendência", "Falha [no cache](specs/x.md)",
                      "P1·J1·Pr1", "[alvo](alvo.py)", "quando doer",
                      "[delta-120](specs/x.md)", "", "Prosa.") == 0
    assert len(list((r_link2 / "debts/ativos").glob("*.md"))) == 1, \
        "link na Origem ou na descrição fez a mesma pendência virar duas"

    # (j) T5: item idêntico já encerrado é reincidência, não duplicata — abre item novo
    # e nomeia o arquivado, senão quem lê não sabe que aquilo já voltou.
    ativo = next((r_idem / "debts/ativos").glob("DEBT_DT-001-*.md"))
    texto = ativo.read_text(encoding="utf-8").replace("estado: aberto", "estado: quitado")
    (r_idem / "debts/_archive").mkdir(parents=True, exist_ok=True)
    (r_idem / "debts/_archive" / f"{ativo.stem}_2026_01_01.md").write_text(texto, encoding="utf-8")
    ativo.unlink()
    cod, voz = falado(cmd_novo, r_idem, *mesmo)
    assert cod == 0 and len(list((r_idem / "debts/ativos").glob("*.md"))) == 2, \
        "item arquivado não pode bloquear o cadastro de uma reincidência"
    assert "DT-001" in voz, f"a reincidência deveria nomear o DT arquivado: {voz}"

    # (k) raiz do registro != diretório da sessão desliga a reescrita de links (R2).
    # O corpo é onde o defeito é silencioso: `[x](docs/adrs/X.md)` virava
    # `../../docs/adrs/X.md`, que resolve DENTRO do registro e aponta arquivo que só
    # existe no repo da sessão — e corpo não passa pelo validar, então nada acusava.
    # a decisão: invocado de dentro do registro reescreve; apontado para outro, não
    r_reg = arquivo(raiz())
    with contextlib.chdir(r_reg):
        assert na_mesma_raiz(r_reg) and na_mesma_raiz(Path(".")), \
            "dentro do próprio registro a reescrita tem de continuar ligada"
        assert not na_mesma_raiz(raiz()), "registro apontado noutro lugar não é a mesma raiz"
    (r_reg / "sub").mkdir()
    with contextlib.chdir(r_reg / "sub"):
        assert na_mesma_raiz(r_reg), "de um subdiretório do registro a reescrita segue ligada"

    # o efeito: prosa fica literal, campo continua reescrito. A assimetria é o achado
    # da T4 — os 2 níveis do campo são metade de um protocolo que o parse desfaz na
    # leitura, e é essa volta que o validar usa para resolver o Local a partir da raiz.
    cod, voz = falado(cmd_novo, r_reg, "pendência", "Veio de outro repo", "P1·J1·Pr1",
                      "[alvo](alvo.py)", "quando doer", "sessão noutro repo", "",
                      "Corpo com [x](docs/adrs/X.md).", reescrever_corpo=False)
    assert cod == 0, f"cadastro cross-repo deveria passar: {voz}"
    escrito = (r_reg / "debts/ativos/DEBT_DT-002-veio-de-outro-repo.md").read_text(encoding="utf-8")
    assert "(docs/adrs/X.md)" in escrito and "../../docs/adrs/X.md" not in escrito, \
        "prosa cross-repo reescrita para caminho que resolve dentro do registro"
    assert "- **Local:** [alvo](../../alvo.py)" in escrito, \
        "campo perdeu os 2 níveis — o parse deixaria de resolvê-lo e o validar reprovaria"
    assert "texto" in voz, f"faltou o aviso de que os caminhos da prosa ficaram literais: {voz}"

    # e o default continua reescrevendo: o desligamento é da fronteira, não do recurso
    r_mesma = arquivo(raiz())
    assert quieto(cmd_novo, r_mesma, "pendência", "Mesma raiz", "P1·J1·Pr1",
                  "[alvo](alvo.py)", "quando doer", "sessão local", "", "Prosa.") == 0
    assert "../../alvo.py" in (r_mesma / "debts/ativos/DEBT_DT-002-mesma-raiz.md").read_text(
        encoding="utf-8"), "reescrita sumiu no caso normal — a guarda pegou demais"

    # captura vinda de outro repositório aponta o artefato no Local como texto puro: o
    # validar exige o campo e só confere alvo de link — sem link, passa (R2)
    cod, voz = falado(cmd_novo, r_reg, "pendência", "Artefato de outro repo", "P1·J1·Pr1",
                      "outro-repo/src/foo.py:42", "quando doer", "sessão noutro repo", "",
                      "Prosa.", reescrever_corpo=False)
    assert cod == 0, f"Local em texto puro deveria passar no validar: {voz}"
    gravado = next((r_reg / "debts/ativos").glob("DEBT_DT-*-artefato-de-outro-repo.md"))
    assert "- **Local:** outro-repo/src/foo.py:42" in gravado.read_text(encoding="utf-8"), \
        "o Local em texto puro não foi gravado como veio"

    # (l) RNF1: repositório que não declara nada sai como saía antes desta delta. A saída
    # do `novo` fica congelada aqui como literal — sem a versão anterior em disco é o
    # único jeito de comparar com "antes", e é o que garante que os consumidores sem a
    # chave não mudem de comportamento num /plugin update. Para `fila` e `indice` a
    # comparação é por ausência das linhas novas, não byte a byte: a saída do `fila`
    # depende da data corrente (marca stale) e do churn do git, então congelá-la
    # reprovaria por passagem de tempo em vez de por regressão.
    r_rnf = arquivo(raiz())
    (r_rnf / "doc-profile.yaml").write_text("motores: {}\n", encoding="utf-8")  # perfil sem a chave: o caso real
    cod, voz = falado(cmd_novo, r_rnf, "pendência", "Sem chave declarada", "P1·J1·Pr1",
                      "[alvo](alvo.py)", "quando doer", "sessão", "", "Prosa.",
                      hoje=date(2026, 1, 1))
    assert cod == 0, f"cadastro sem a chave deveria seguir igual: {voz}"
    assert voz == ("[id] só o disco — nenhum ref remoto buscado (git fetch --prune)\n"
                   "debts/ativos/DEBT_DT-002-sem-chave-declarada.md\n"
                   "regenere o índice: python3 debito.py indice .\n"), \
        f"a saída do `novo` mudou para repositório sem a chave: {voz!r}"

    linhas_novas = ("[registro]", "[idempotente]", "[reincidência]", "[fronteira]")
    for comando, rotulo in ((cmd_fila, "fila"), (cmd_indice, "indice")):
        cod_c, voz_c = falado(comando, r_rnf)
        assert cod_c == 0, f"`{rotulo}` falhou em repositório sem a chave: {voz_c}"
        for marca in linhas_novas:
            assert marca not in voz_c, f"`{rotulo}` ganhou linha desta delta: {marca}"

    arvore = {str(p.relative_to(r_rnf)) for p in r_rnf.rglob("*") if p.is_file()}
    assert arvore == {"alvo.py", "DEBT.md", "doc-profile.yaml",
                      "debts/ativos/DEBT_DT-001-caso-teste.md",
                      "debts/ativos/DEBT_DT-002-sem-chave-declarada.md"}, \
        f"árvore ganhou ou perdeu arquivo em repositório sem a chave: {sorted(arvore)}"

    print("selftest novo: OK (frontmatter, paridade, precedência, malformações, união, diff, migrar, índice, registro declarado, gitignore, idempotência, fronteira do repo, RNF1)")


def selftest_vinculo_puro() -> None:
    """Chave do ticket e presença do vínculo — funções puras, sem git (delta-119)."""
    # valor do campo: âncora de início de linha; campo citado na prosa não é campo
    assert valor_do_ticket("x\n- **Ticket:** [#7](../../../../issues/7)\n") == "[#7](../../../../issues/7)"
    assert valor_do_ticket("- **Ticket:** —\n") == ""
    assert valor_do_ticket("prosa que cita - **Ticket:** #1 no meio\n") == ""
    # chave: os formatos que o campo Ticket grava (R52) e o que não é chave
    assert chave_ticket("[#95](../../../../issues/95)") == ("github", "95")
    assert chave_ticket("#7") == ("github", "7")
    assert chave_ticket("gh#12") == ("github", "12")
    assert chave_ticket("https://github.com/o/r/issues/31") == ("github", "31")
    assert chave_ticket("PROJ-12") == ("jira", "PROJ-12")
    assert chave_ticket("[SBX-3](https://x.atlassian.net/browse/SBX-3)") == ("jira", "SBX-3")
    assert chave_ticket("a combinar") is None
    # a linha que a PR carrega
    assert linha_vinculo(("github", "95")) == "Closes #95"
    assert linha_vinculo(("jira", "PROJ-12")) == "PROJ-12"
    # GitHub: toda palavra de fechamento documentada, sem diferenciar maiúsculas
    for palavra in PALAVRAS_FECHAMENTO:
        assert vinculo_presente(("github", "95"), f"corpo\n{palavra.upper()} #95\n"), palavra
    assert vinculo_presente(("github", "95"), "Fixes: #95"), "a forma com dois-pontos é documentada pelo GitHub"
    assert not vinculo_presente(("github", "95"), "Refs #95"), "menção sem palavra não fecha"
    assert not vinculo_presente(("github", "95"), "Closes #950"), "#95 não casa #950"
    assert not vinculo_presente(("github", "9"), "Closes #95"), "#9 não casa #95"
    # Jira: chave literal em qualquer ponto; prefixo ou sufixo alheio não conta
    assert vinculo_presente(("jira", "PROJ-12"), "feat(x): algo (PROJ-12)")
    assert not vinculo_presente(("jira", "PROJ-12"), "PROJ-123")
    assert not vinculo_presente(("jira", "PROJ-12"), "XPROJ-12")
    print("selftest vinculo (puro): OK (valor do campo, chave GitHub/Jira, linha, palavras de fechamento, bordas)")
...
def selftest_quitar() -> None:
    ...
    assert "- **Encerrado:** 2026-03-01 · delta-999 · #1 · JIRA-9" in texto_novo, \
    ...
    assert "vínculo para a PR: JIRA-9" in voz, f"quitado com Ticket devia imprimir o vínculo: {voz}"

    # regressão: o `git mv` interno tem que deixar o ÍNDICE com o conteúdo novo, não só o
    # disco — commitar sem um `git add` manual depois não pode reverter a quitação (achado
    # real: DT-130 foi ao _archive/ do main com "estado: aberto", conteúdo de antes do quitar,
    # porque `git mv` sozinho stagia a rename com o blob antigo quando o arquivo estava sujo)
    staged = subprocess.run(["git", "-C", str(r1), "show",
                            f":debts/_archive/DEBT_DT-001-caso-teste_2026_03_01.md"],
                            capture_output=True, text=True).stdout
    assert staged == texto_novo, \
        ("git mv deixou o índice com conteúdo diferente do disco — a próxima pessoa a "
         f"commitar sem `git add` manual gravaria a versão de antes do quitar.\nstaged={staged!r}")
    ...
    assert "vínculo" not in voz2, voz2
    ...
    # delta-119: sem --ticket-ref, o Ticket vira a referência do Encerrado e o vínculo sai impresso
    r6 = repo()
    ativo(r6, ident="DT-006", ticket="[#7](../../../../issues/7)")
    codigo6, voz6 = falado(cmd_quitar, r6, "DT-006", "Prosa do conserto.", "", date(2026, 3, 6))
    arq6 = r6 / "debts/_archive/DEBT_DT-006-caso-teste_2026_03_06.md"
    assert codigo6 == 0 and "vínculo para a PR: Closes #7" in voz6, voz6
    assert "- **Encerrado:** 2026-03-06 · [#7](../../../../issues/7)" in arq6.read_text(encoding="utf-8"), \
        "sem --ticket-ref, o Ticket devia virar a referência do Encerrado"

    # delta-119: descarte com Ticket não imprime vínculo — lembra de fechar como 'não planejado'
    r7 = repo()
    ativo(r7, ident="DT-007", estado_item="descartado", ticket="#8")
    codigo7, voz7 = falado(cmd_quitar, r7, "DT-007", "Motivo do descarte.", "", date(2026, 3, 7))
    assert codigo7 == 0 and "não planejado" in voz7 and "vínculo para a PR" not in voz7, voz7

    # delta-119: vinculo sobre um intervalo git real — DTs quitados na branch contra a main
    r8 = repo()
    ativo(r8, ident="DT-010", estado_item="aberto", ticket="[#7](../../../../issues/7)")
    ativo(r8, ident="DT-011", topico="jira", estado_item="aberto", ticket="PROJ-12")
    ativo(r8, ident="DT-012", topico="sem-ticket", estado_item="aberto")
    ativo(r8, ident="DT-013", topico="descarte", estado_item="aberto", ticket="#8")
    ativo(r8, ident="DT-014", topico="lixo", estado_item="aberto", ticket="a combinar")
    subprocess.run(["git", "-C", str(r8), "switch", "-qc", "quita"], check=True, capture_output=True)
    # criado e quitado dentro do próprio intervalo: não existe na base, o diff não o mostra
    # como remoção — o Ticket tem de vir do commit que o apagou
    ativo(r8, ident="DT-015", topico="novo", estado_item="aberto", ticket="#9")
    for ident, topico, final in (("DT-010", "caso-teste", "quitado"), ("DT-011", "jira", "quitado"),
                                 ("DT-012", "sem-ticket", "quitado"), ("DT-013", "descarte", "descartado"),
                                 ("DT-014", "lixo", "quitado"), ("DT-015", "novo", "quitado")):
        caminho = r8 / f"debts/ativos/DEBT_{ident}-{topico}.md"
        caminho.write_text(caminho.read_text(encoding="utf-8").replace("estado: aberto", f"estado: {final}"),
                           encoding="utf-8")
        assert falado(cmd_quitar, r8, ident, "Prosa do conserto.", "", date(2026, 4, 1))[0] == 0, ident
    subprocess.run(["git", "-C", str(r8), "commit", "-qam", "quita"], check=True, capture_output=True)

    def falado_err(fn, *args):
        """Como o `falado`, mas separa o stderr — o vinculo avisa por lá, e o stdout é colável."""
        out, err = io.StringIO(), io.StringIO()
        with contextlib.redirect_stdout(out), contextlib.redirect_stderr(err):
            r = fn(*args)
        return r, out.getvalue(), err.getvalue()

    codigo8, voz8, err8 = falado_err(cmd_vinculo, r8, "main", "HEAD", None, "", "")
    assert codigo8 == 0 and voz8.splitlines() == ["Closes #7", "PROJ-12", "Closes #9"], \
        f"linhas do vínculo erradas (sem ticket, descartado e lixo ficam de fora): {voz8!r}"
    assert "DT-014" in err8 and "a combinar" in err8, f"Ticket sem chave devia avisar no stderr: {err8!r}"
    assert falado_err(cmd_vinculo, r8, "main", "HEAD", "feat: x (PROJ-12)\n\nCloses #7\nfixes #9\n",
                      "main", "main")[0] == 0, "texto com os três vínculos devia passar"
    codigo9, voz9, _ = falado_err(cmd_vinculo, r8, "main", "HEAD", "Refs #7\nPROJ-12", "main", "main")
    assert codigo9 == 1 and "[ausente] DT-010: Closes #7" in voz9 and "[ausente] DT-015: Closes #9" in voz9, voz9
    assert "[ausente] DT-011: PROJ-12" in voz9, f"chave do Jira só no corpo não vincula: {voz9}"
    # chave do Jira no nome da branch vale — é onde a integração GitHub↔Jira também lê
    assert falado_err(cmd_vinculo, r8, "main", "HEAD", "feat: x\n\nCloses #7\nCloses #9\n",
                      "main", "main", "feat/PROJ-12-quita")[0] == 0, "chave na branch devia valer"
    # PR de release: a chave do Jira é dispensada (vinculou na PR de escopo); os Closes seguem
    voz_rel = falado_err(cmd_vinculo, r8, "main", "HEAD", None, "", "", "release/1.2.0")[1]
    assert voz_rel.splitlines() == ["Closes #7", "Closes #9"], f"release devia listar só os Closes: {voz_rel!r}"
    assert falado_err(cmd_vinculo, r8, "main", "HEAD", "chore(release): 1.2.0\n\nCloses #7\nCloses #9\n",
                      "main", "main", "release/1.2.0")[0] == 0, "release com os Closes devia passar"
    # PR contra branch que não é a default: GitHub se omite, Jira segue exigido
    codigo10, voz10, err10 = falado_err(cmd_vinculo, r8, "main", "HEAD", "nada", "develop", "main")
    assert codigo10 == 1 and "PROJ-12" in voz10 and "Closes #7" not in voz10, voz10
    assert "develop" in err10, f"alvo ≠ default devia avisar no stderr: {err10!r}"
    # ref inexistente e --pr-arquivo inexistente: erro de uso (2), com o ERRO no stderr — o
    # stdout é o que o criar-release cola no corpo da PR e não pode receber a mensagem
    for extra in (["--base", "nao-existe"], ["--base", "main", "--pr-arquivo", str(r8 / "nao-existe.txt")]):
        cli = subprocess.run([sys.executable, __file__, "vinculo", str(r8), *extra],
                             capture_output=True, text=True)
        assert cli.returncode == 2 and cli.stdout == "" and "ERRO" in cli.stderr, \
            (extra, cli.returncode, cli.stdout, cli.stderr)
    # CI faz checkout do merge ref sintético (1º pai = base): o DT criado e quitado na PR não
    # existe em nenhum dos dois pais, e sem --full-history o git log o perde (review da delta-119)
    for passo in (["switch", "-q", "main"], ["switch", "-qc", "merge-ref"],
                  ["merge", "-q", "--no-ff", "--no-edit", "quita"]):
        subprocess.run(["git", "-C", str(r8), *passo], check=True, capture_output=True)
    voz_m = falado_err(cmd_vinculo, r8, "main", "HEAD", None, "", "")[1]
    assert voz_m.splitlines() == ["Closes #7", "PROJ-12", "Closes #9"], \
        f"merge ref perdeu o DT criado na PR: {voz_m!r}"
    # repositório sem debts/ (mas com histórico): diff vazio, aviso no stderr e 0 (RNF2).
    # O r8 já cobre a borda oposta: todos os ativos saíram, e a pasta vazia não derruba o check
    r9 = repo()
    subprocess.run(["git", "-C", str(r9), "add", "-A"], check=True, capture_output=True)
    subprocess.run(["git", "-C", str(r9), "commit", "-qm", "base"], check=True, capture_output=True)
    codigo_r9, voz_r9, err_r9 = falado_err(cmd_vinculo, r9, "main", "HEAD", None, "", "")
    assert (codigo_r9, voz_r9) == (0, "") and "nada a vincular" in err_r9, (codigo_r9, voz_r9, err_r9)

    print("selftest quitar: OK (feliz com/sem ticket-ref, Ticket vira Encerrado e referência, vínculo "
          "impresso, descarte lembra 'não planejado', recusas) · vinculo: OK (intervalo, item criado no "
          "intervalo, merge ref, conferência, Jira no título/branch, release dispensa o Jira, "
          "alvo ≠ default, erro de uso no stderr, sem debts/)")
