...
VOCAB_REGISTRO = ("proprio", "externo")  # delta-120, R4
...
def _ler_perfil(root: Path, sem_yaml: str, ilegivel: str) -> dict:
    """Conteúdo do `doc-profile.yaml` como dict — dono único da leitura do arquivo.

    Ler nunca derruba quem chama (RNF2): arquivo ausente, PyYAML faltando e YAML
    inválido devolvem `{}`. Os dois avisos recebem a consequência pronta em vez de
    montá-la aqui porque cada chamador perde uma coisa diferente, e é o aviso que
    separa "não declarei" de "não consigo ler" — a família de falha que o DT-033
    fechou, e que uma mensagem genérica traria de volta.
    """
    perfil = root / ARQUIVO_PERFIL
    if not perfil.is_file():
        return {}
    if yaml is None:
        print(f"aviso: PyYAML ausente — {ARQUIVO_PERFIL} não lido, {sem_yaml} "
              "(pip install pyyaml, ADR-0023)")
        return {}
    try:
        return yaml.safe_load(perfil.read_text(encoding="utf-8")) or {}
    except yaml.YAMLError:
        print(f"aviso: {ARQUIVO_PERFIL} ilegível — {ilegivel}")
        return {}


def ler_registro(root: Path) -> str | None:
    """`'proprio'`, `'externo'` ou `None` — dono único da leitura de `deltaspec.registro`.

    Declara onde a pendência daquele repositório é cadastrada: `proprio` autoriza o
    `debito.py novo` a **criar** `debts/` quando ele não existe; `externo` manda a
    captura para o registro declarado no `CLAUDE.md` do usuário mesmo havendo
    `debts/ativos/` no disco (delta-120, R4).

    `None` é o caso comum e silencioso: nenhum consumidor tem a chave hoje, e a
    ausência precisa preservar o comportamento vigente byte a byte (RNF1). Valor fora
    do vocabulário também devolve `None`, mas **com aviso**: nenhum gate confere essa
    chave (o C11 cobra só o núcleo do perfil), então é aqui que "declarei errado" deixa
    de parecer "não declarei" — `externa` no lugar de `externo` cadastraria no registro
    local, contra a declaração do dono.
    """
    dados = _ler_perfil(
        root,
        sem_yaml="registro não declarado",
        ilegivel="registro cai em não declarado",
    )
    bloco = dados.get("deltaspec") or {}
    valor = bloco.get("registro") if isinstance(bloco, dict) else None
    if valor is not None and valor not in VOCAB_REGISTRO:
        print(f"aviso: deltaspec.registro com valor {valor!r} fora do vocabulário "
              f"({' | '.join(VOCAB_REGISTRO)}) — tratado como não declarado")
        return None
    return valor
...
def ler_destino_tickets(root: Path) -> tuple[str, str | None]:
    ...
    dados = _ler_perfil(
        root,
        sem_yaml="destino cai no default",
        ilegivel="destino do ticket cai no default",
    )
...
def selftest():
    ...
    # (i) ler_registro (delta-120, R4): dono único de deltaspec.registro. A ausência é
    # o caso comum — nenhum dos consumidores tem a chave — e precisa devolver None sem
    # ruído, para que o comportamento vigente do debito.py não mude (RNF1).
    with tempfile.TemporaryDirectory() as td:
        raiz = Path(td)
        assert ler_registro(raiz) is None, "sem perfil, nada é declarado"
        perfil = raiz / ARQUIVO_PERFIL
        perfil.write_text("deltaspec:\n  registro: proprio\n", encoding="utf-8")
        assert ler_registro(raiz) == "proprio"
        perfil.write_text("deltaspec:\n  registro: externo\n", encoding="utf-8")
        assert ler_registro(raiz) == "externo"
        # a chave convive com as outras do bloco deltaspec, sem atrapalhar
        perfil.write_text("deltaspec:\n  ancora: 1.55.1\n  registro: proprio\n", encoding="utf-8")
        assert ler_registro(raiz) == "proprio", "registro convive com ancora no mesmo bloco"
        # desligado de todas as formas que um perfil real usa
        for ausente in ("deltaspec:\n  ancora: 1.55.1\n", "deltaspec: {}\n",
                        "deltaspec: false\n", "motores: {}\n", "{}\n"):
            perfil.write_text(ausente, encoding="utf-8")
            assert ler_registro(raiz) is None, f"deveria devolver None: {ausente!r}"
        # valor fora do vocabulário vira None **com aviso**: nenhum gate confere a chave, e
        # "próprio" com acento é a grafia natural em português
        import contextlib
        import io
        for errado in ("talvez", "próprio", "externa"):
            perfil.write_text(f"deltaspec:\n  registro: {errado}\n", encoding="utf-8")
            aviso = io.StringIO()
            with contextlib.redirect_stdout(aviso):
                assert ler_registro(raiz) is None, f"fora do vocabulário deveria virar None: {errado}"
            assert "fora do vocabulário" in aviso.getvalue(), f"faltou o aviso para {errado!r}"
        # perfil quebrado não derruba o cadastro — o registro vale sozinho (RNF2)
        perfil.write_text("deltaspec:\n  registro: [\n", encoding="utf-8")
        assert ler_registro(raiz) is None, "YAML inválido deveria degradar, não estourar"
