...
Grava `encerrado: {AAAA-MM-DD}` no frontmatter, injeta a seção **`#### Como foi quitado`** no corpo (linguagem amigável, sem jargão de implementação — isso fica no commit/PR e no comentário do issue), troca **Ticket** por `- **Encerrado:** data · referência` (a de `--ticket-ref` ou, sem ela, o próprio valor do Ticket — o arquivado não perde o elo com o ticket que o projetou), e move o arquivo com `git mv` para `debts/_archive/DEBT_DT-NNN-<topico>_<AAAA>_<MM>_<DD>.md`. Mesma garantia de "nada é inventado" do `novo`: recusa sem `--como`, recusa se o estado não é final ainda, recusa se o `DT-NNN` não existe em `ativos/` — e a edição só fica no disco depois de reler com o próprio parser e o `validar` da fila (edição inválida nunca persiste). O `debito.py` lê os IDs arquivados do **nome** dos arquivos, na união de `debts/_archive/` e `.claude/debts/` (legado): a fila avisa candidato a arquivamento esquecido em `ativos/`, e o `diff` trata ID arquivado como estado final. A legenda completa dos estados vive em `debts/README.md`, não aqui. Item `quitado` com Ticket sai com a linha de vínculo impressa (`vínculo para a PR: Closes #N`, ou a chave do Jira); `descartado` com Ticket sai com o lembrete de fechar à mão como "não planejado".
...
**Fechamento de ticket na quitação (delta-049, delta-119, delta-124):** a PR que quita DT com Ticket carrega o vínculo que o `debito.py quitar` imprime. **No GitHub**, é a linha `Closes #N` no corpo da PR: o merge na branch default fecha o issue, e a PR vira a citação técnica da quitação. **No Jira**, é a chave do item no título da PR (ou na branch): a regra de automação do projeto Jira move o item para concluído no merge (receita na `ticket-to-jira`), e o comentário em **nível de negócio** — o que foi resolvido e o impacto, sem detalhe de implementação — segue à mão. O *como* em linguagem amigável vive na seção `#### Como foi quitado` do arquivo arquivado. DT `descartado` fecha como "não planejado", pelo workflow abaixo ou à mão onde ele não está instalado: a palavra de fechamento fecharia como concluído. Na topologia com `develop`, o GitHub ignora a palavra de fechamento na PR contra a `develop` (só a interpreta na branch default) — quem fecha é o workflow `fechar-issues-no-merge` que o `/deltaspec:projeto-infra` instala: no merge na `develop`, fecha as issues que a PR resolve (palavra de fechamento no corpo e DT que ela arquivou, achado pelo Ticket ou pelo título `[DT-NNN]` ou `DT-NNN — `); no merge da `release/*` ou `hotfix/*` na `main`, carimba "lançada na vX.Y.Z" nelas e fecha o que sobrou. Sem o workflow, quem fecha é a PR de release, que o `/deltaspec:criar-release` monta com as mesmas linhas.

**Reincidência depois da release (delta-124):** issue concluída e carimbada nunca é reaberta. O mesmo defeito ou débito que volta pela mesma causa abre issue nova, citando a antiga e a versão em que ela saiu, com a label `reincidencia` — é falha de processo, não só de código. Afeta o cliente no ambiente publicado a partir da `main` → hotfix (`/deltaspec:criar-release X.Y.Z --hotfix`). Não afeta → DT novo pelo `debito.py novo`, com **Origem** citando a issue antiga, e volta à fila.

**Conferência no CI (delta-119).** `debito.py vinculo . --base <ref> [--head <ref>] [--pr-arquivo pr.txt --alvo <branch> --default <branch> --branch <head>]` lista o vínculo exigido pelos DTs quitados no intervalo — Ticket lido da última versão em `ativos/` — e, com o texto da PR, sai 1 nomeando o que falta. O `Closes #N` vale em qualquer ponto do texto; a chave do Jira, só no título ou na branch (é onde a integração GitHub↔Jira lê — no corpo, não vincula), e na PR de `release/*` ela é dispensada, porque vinculou na PR que quitou o DT. O job `ci` do deltaspec roda o step em toda PR; o consumidor cola o mesmo job no CI dele (a `projeto-init` não distribui CI), com o `ref:` na tag do deltaspec que o projeto consome:

```yaml
on:
  pull_request:
    types: [opened, synchronize, reopened, edited]   # edited: o conserto do corpo re-roda o check
jobs:
  vinculo:
    ...
    steps:
      ...
      - uses: actions/checkout@fbc6f3992d24b796d5a048ff273f7fcc4a7b6c09 # v5
        with:
          repository: iuripereira/deltaspec
          ref: vX.Y.Z   # a versão do deltaspec que o projeto consome
          path: .deltaspec
      - env:   # texto de PR é entrada não confiável: por env:, nunca dentro do run:
          PR_TITULO: ${{ github.event.pull_request.title }}
          PR_CORPO: ${{ github.event.pull_request.body }}
          PR_BRANCH: ${{ github.head_ref }}
          PR_ALVO: ${{ github.base_ref }}
          PR_DEFAULT: ${{ github.event.repository.default_branch }}
        run: |
          printf '%s\n\n%s\n' "$PR_TITULO" "$PR_CORPO" > "$RUNNER_TEMP/pr.txt"
          python3 .deltaspec/skills/handoff/scripts/debito.py vinculo . --base "origin/$PR_ALVO" \
            --head HEAD --pr-arquivo "$RUNNER_TEMP/pr.txt" --alvo "$PR_ALVO" --default "$PR_DEFAULT" \
            --branch "$PR_BRANCH"
```

O job acima espelha o step "Vinculo da PR com o ticket dos DTs quitados" do `.github/workflows/ci.yml` do deltaspec — duplicação consciente (o consumidor não herda o workflow): mudou um, mude o outro.
