...
[Versão corrente: tags](https://github.com/iuripereira/deltaspec/tags) · 23 skills · Licença MIT
...
Pronto: as 23 skills aparecem com o prefixo `deltaspec:`. Para atualizar depois, `/plugin update deltaspec@deltaspec` — sem o `@deltaspec` do marketplace, o plugin não é encontrado.
...
Branch protection, CI, checagem de Conventional Commits, review assistido. Rodar de novo não estraga nada — a segunda vez só relata o que já estava lá.

O release é de duas skills, sem nada à mão: `/deltaspec:criar-release X.Y.Z` valida a versão contra a última tag, corta a branch de release, libera o `CHANGELOG.md` e abre a PR para a `main`; `/deltaspec:release` mergeia com checks verdes, cria tag e Release numa chamada no commit do merge e dispara o deploy pela tag, quando o projeto declara um. Onde houver `develop`, ela sincroniza `main → develop` por uma PR `sync/X.Y.Z` com o CHANGELOG conferido por conjunto de bullets — nenhum some, nenhum não lançado vai para baixo da versão — e confirma que a `main` é ancestral da `develop`; no próprio framework, publica também o derivado público. Parou no meio? Rode de novo: ela confere o que já foi feito e continua do primeiro passo que falta. A versão é a única decisão humana.
...
flowchart LR
    ...
    r -->|não| l("registro de captura<br>rota do seu CLAUDE.md")
...
A **lição** é o terceiro habitante do `debts/`: post-mortem sem ação pendente, um arquivo `debts/licoes/LICAO_L-NNN-<topico>.md` com a regra que fica como título, o que aconteceu, a causa (lacuna do sistema ou do processo, nunca "erro humano"), o desfecho e a prevenção — cada item apontando o check, o `DT-NNN` ou a disciplina que impede a reincidência. Nasce por `licao.py nova`, nunca à mão; `debts/LICOES.md` é o índice gerado, e o CI reprova índice divergente ([ADR-0042](docs/adrs/ADR-0042-licoes-em-arquivo-por-item.md)).

```bash
# cadastra uma lição já validada — calcula o L-NNN e recusa campo faltando
python3 skills/handoff/scripts/licao.py nova . \
  ...
  --origem "delta-NNN · PR #N" --corpo-arquivo corpo.md

# regenera debts/LICOES.md a partir de debts/licoes/ (projeção, ADR-0042)
python3 skills/handoff/scripts/licao.py indice .
```

**Fora de um projeto com registro alcançável, nada se perde.** A skill [`eu-tenho-tdah`](skills/eu-tenho-tdah/SKILL.md) cadastra a pendência como `DT-NNN` num **registro de captura** — um repositório com `debts/`, cuja **rota** você declara no seu próprio `CLAUDE.md`; o framework nomeia rotas, jamais caminhos de máquina —, com fila e envelhecimento como qualquer item, fecha com commit e sem push quando o destino é versionado, e migra o item para o `debts/` do repo dono assim que ele virar trabalho de verdade. Repositório que quer registro próprio declara `deltaspec.registro: proprio` no `doc-profile.yaml`, e o `debito.py novo` o cria no primeiro cadastro. É o par do [`handoff`](skills/handoff/SKILL.md), que faz o mesmo roteamento no fim da sessão.
...
Vinte e dois comandos do ciclo, na ordem em que você os encontra. Clique no nome para ler o detalhe da skill.
...
| [`project-review-dotclaude`](skills/project-review-dotclaude/SKILL.md) | Mede se `.claude/` guarda só o que o Claude Code lê, mais `handoffs/`: plano e prompt estacionados, `commands/` legado, artefato de execução sem regra no `.gitignore` versionado, hook citado e não versionado, documento canônico no lugar errado | Auditar a pasta `.claude/` de um repositório ou de um workspace inteiro |
...
| [`replan-doc`](skills/replan-doc/SKILL.md) | Replaneja um plano em execução contra o que o repo aprendeu depois dele: coleta dirigida por mapa de alvos, fatos com fonte, plano novo datado com tarefas checáveis — o anterior fica intacto | O plano ficou velho: chegou dossiê, resposta, grilling, handoff ou débito depois dele |
| [`lote-debitos`](skills/lote-debitos/SKILL.md) | Corrige vários `DT-NNN` pequenos e independentes numa passada: um subagente e uma worktree por débito, gate local por item, e parada obrigatória antes de qualquer push ou PR | A fila acumulou itens pequenos e fechar um por vez não escala |
| [`inicio-de-sessao`](skills/inicio-de-sessao/SKILL.md) | Abre o dia com o estado real de cada delta derivado do git — nunca de um campo de status —, a bancada de worktrees e branches que guardam trabalho não salvo ou parado, e UM próximo passo | Retomar um projeto configurado e precisar saber onde parou |
| [`criar-release`](skills/criar-release/SKILL.md) | Corta a release sem receita à mão: valida a versão pedida contra a última tag e os commits (`versao_corte.py` — só o sucessor imediato passa), corta `release/X.Y.Z` da `develop` (ou da `main`, sem `develop`), libera o `[Não lançado]` com `--liberar`, abre a PR e espera os checks. Hotfix em duas fases: `--hotfix` cria `hotfix/X.Y.Z` da `main` e para; a segunda chamada, com o conserto, corta | Quando o `[Não lançado]` tem o que lançar e você decidiu a versão |
| [`release`](skills/release/SKILL.md) | Publica a versão cortada: merge da PR de release/hotfix (merge commit com `develop`, squash sem), tag + Release a partir da seção do CHANGELOG, deploy pela tag, sync `main→develop` por `sync/X.Y.Z` conferida por conjunto de bullets e `main ⊆ develop` verificada pelo `merge-base --is-ancestor`. Retomável: rodar de novo continua do primeiro passo que falta | Quando a PR `release/*` ou `hotfix/*` está verde — ou para retomar uma publicação que parou no meio |

A vigésima terceira não é comando: [`eu-tenho-tdah`](skills/eu-tenho-tdah/SKILL.md) fica ligada o tempo todo e entrega dois eixos — molda a resposta (ação antes de contexto, listas ranqueadas, próximo passo concreto) e dá destino à tangente: toda pendência vira registro no sistema da [seção 6](#6-o-registro-de-débitos), nunca um convite no fim da resposta.
...
# a forma da entrada do CHANGELOG: categoria, tamanho, referência de PR, ordenação, rodapé
python3 skills/spec-feature/scripts/check_changelog.py CHANGELOG.md

# a PR que quita DT com Ticket leva o vínculo (Closes #N ou a chave do Jira);
# o job ci roda o mesmo comando com o texto da PR (--pr-arquivo)
...
| `debts/` | débito, pendência e lição | `DT-NNN`/`L-NNN` fixos, um arquivo por item; resolvido **muda de status, nunca some** — encerrado move para `debts/_archive/`; lição é imutável em `debts/licoes/`; índices gerados em `DEBT.md`/`LICOES.md`; regras em `debts/README.md` — visão geral na [seção 6](#6-o-registro-de-débitos) |
