...
[Versão corrente: tags](https://github.com/iuripereira/deltaspec/tags) · 15 skills · Licença MIT
...
Pronto: as 15 skills aparecem com o prefixo `deltaspec:`. Para atualizar depois, `/plugin update deltaspec@deltaspec` — sem o `@deltaspec` do marketplace, o plugin não é encontrado.
...
Catorze comandos do ciclo, na ordem em que você os encontra. Clique no nome para ler o detalhe da skill.
...
A décima quinta não é comando: [`eu-tenho-tdah`](skills/eu-tenho-tdah/SKILL.md) é o perfil de escrita do Iuri (baseado em [ayghri/i-have-adhd](https://github.com/ayghri/i-have-adhd)), ligado o tempo todo — ação antes de contexto, listas ranqueadas, tangente vira pendência salva.
...
# tamanho do TRUTH, dependências das tasks (checagens C1–C13)
...
| `debts/` | débito, pendência e lição | `DT-NNN` fixo, um arquivo por item; resolvido **muda de status, nunca some** — encerrado move para `debts/_archive/`; índice por urgência gerado em `DEBT.md`; regras em `debts/README.md` |
