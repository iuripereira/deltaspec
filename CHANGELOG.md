...
- **Bloco aninhado em item de lista chega inteiro ao pdf do cliente** (delta-032, bugfix): tabela de decisão dentro de um `- RN-NNN` virava texto com hífen literal, e parágrafo aninhado escapava do item ([DT-009](DEBT.md), mordida da rodada IMEX de 2026-07-20). O `deepen_indents` do `tabela_cliente.py` tinha três furos independentes: só aprofundava bullets; tabela colada no item nunca iniciava bloco (faltava linha em branco); e o `splitlines`/`join` comia a linha em branco final da seção, fazendo o heading seguinte ser engolido como linha da tabela — este terceiro achado saiu do teste fim a fim da própria delta. Bloco cercado (```) permanece intacto. Selftest com os três casos e três mutações cobertas; o contorno manual saiu do "Erros comuns" da SKILL.md.
