...
### Adicionado
- Molde `fechar-issues-no-merge.yml` no `projeto-infra`: fecha a issue no merge da PR na `develop` e carimba a versão na release (#493)
- Skill `project-review-dotclaude`: modo `auditar` com os checks D1–D8 sobre `.claude/`, pelo `audit_workspace.py --apenas-dotclaude` (#489)
- `status-pmo` ganha o one page do projeto (sinais vitais, objetivos, top 5 com detalhe) e o roadmap do portfólio por marcos, com saúde e criticidade por regra (#482)

## [1.60.2] - 2026-09-21

### Adicionado
- DT-182 cunhado: o painel marca archive pendente com a PR mergeada mesmo com tarefa aberta no `tasks.md` — migrado do repositório de origem junto com o código (#480)

### Mudado
- `release-check.yml` e `sync-check.yml` passam a usar `actions/checkout@v5`, que roda no Node 24; o v4 que estava neles roda no Node 20, descontinuado pelo GitHub Actions (#479)

### Corrigido
- A `inicio-de-sessao` grava a nota do dia no vault e processa as anotações do usuário, deixando o rastro na própria nota (#484)

## [1.60.1] - 2026-09-17

### Corrigido
- Gate de segredos reconhece o bundle do archify pelo sufixo `.archify.html` em qualquer pasta, não só sob `docs/diagrams/`, encerrando o falso positivo que forçava `--no-verify` (#475)

## [1.60.0] - 2026-09-17

### Adicionado
- Skill `inicio-de-sessao`: painel de abertura de dia por projeto configurado, com o estado de cada delta derivado do git, bancada de worktrees e um próximo passo (#465)

### Mudado
- O deltaspec passa ao perfil com `develop`: PR de contribuição vai contra a `develop`, e a `main` só recebe release e hotfix, por merge commit (ADR-0048) (#458)

## [1.59.0] - 2026-09-13

### Adicionado
- `debito.py novo` cria o registro `debts/` quando o `doc-profile.yaml` declara `deltaspec.registro: proprio`, e o cadastro repetido devolve o item existente em vez de duplicar (#455)
- PR que quita DT com Ticket carrega o vínculo (`Closes #N` ou a chave do Jira), impresso pelo `quitar` e conferido no CI pelo `debito.py vinculo` (#451)

## [1.58.1] - 2026-09-10

### Corrigido
- `CLAUDE.md` deixa de linkar o registro privado de débitos, cujo link morto reprovava a integridade do snapshot na publicação do derivado (#445)

## [1.58.0] - 2026-09-10

### Adicionado
- `projeto-infra` ganha `release-check.yml` (versão, tag, fork e versão anterior sem tag na PR para a `main`), `sync-check.yml` (CHANGELOG na sync) e o ruleset `release-hotfix.json` (#442)
- `montar_changelog.py --liberar X.Y.Z` libera o `[Não lançado]` como versão (recusa versão ≤ última) e escreve o rodapé de comparação (#442)
- Skills `criar-release` e `release`: o humano decide só a versão (sucessor imediato da última tag); as duas cortam, validam, mergeiam, taggeiam, publicam e, com `develop`, sincronizam (#442)
- Skill `replan-doc`: replaneja um plano em execução contra os fatos novos do repo — coleta dirigida, fatos com fonte, plano novo datado com tarefas checáveis (#424)

### Mudado
- `projeto-infra` aplica os rulesets pela topologia, mesclando os existentes: `commits` exigido na `main` e na `develop`; com `develop`, a `main` aceita só merge commit e exige `release-check` (#442)

### Corrigido
- `montar_changelog.py` mescla os fragmentos no `[Não lançado]` em vez de trocar a seção inteira, que apagava o bullet já presente (#442)
- `segredos.py` e o G1 do `audit_workspace` deixam de acusar o runtime embutido nas páginas do archify em `docs/diagrams/`; os padrões de segredo real seguem valendo ali (#441)

### Removido
- release-please sai do `projeto-infra`: versão e tag passam a ser das skills `criar-release` e `release`, e quem já o tem migra no próximo corte (#442)

## [1.57.0] - 2026-09-09

### Adicionado
- Cabeçalho do `DEBT.md` gerado explica a fila em uma linha: os três eixos, a escala e o score derivado (#435)

### Corrigido
- 28 débitos ativos com descrição acima do teto de 100 voltam a caber no rótulo da fila e do índice (#435)
- `debito.py novo --natureza guarda` deixa de cadastrar a guarda como `aberto`: ela nasce `vigente`, como a regra sempre disse (#437)
- Validação recusa a combinação impossível de natureza e estado — `vigente` é da guarda, e só dela (#437)

## [1.56.1] - 2026-09-08

### Corrigido
- Vista de integração deixa de citar nome real de organização e de repo consumidor, que abortava o gate de confidencialidade na publicação do derivado (#431)

## [1.56.0] - 2026-09-08

### Adicionado
- Vista da fila de integração (`integracao.py`): branch × worktree × sessão × PR × idade × plano, ordem derivada por marco/pilha/`fila:N`, `--json` para consumidores (#428)

## [1.55.1] - 2026-09-05

### Corrigido
- Skills publicadas não linkam mais para `debts/`, que o allowlist nunca publica — o derivado nascia com dois links mortos (#421)

## [1.55.0] - 2026-09-05

### Adicionado
- Lições viram arquivos em `debts/licoes/` com cadastro por `licao.py nova` e índice gerado em `LICOES.md` (#418)

### Mudado
- As 42 entradas de `debts/LICOES.md` viram 43 lições em `debts/licoes/`, com cinco DTs aceitos para os candidatos a mecanizar (#419)

## [1.54.0] - 2026-09-02

### Adicionado
- `projeto-init` cria `changelog.d/` + README nos tipos com release-triad, materializando no scaffold o fluxo por fragmento da delta-105 (#412)

### Mudado
- O template `handoff-sessao.md` passa a viver na skill `handoff`, com as seis referências atualizadas junto (#412)

### Removido
- `docs/epics/` sai do scaffold e do CLAUDE.md gerado, e o `SPEC-TEMPLATE.md` morto é deletado — doc sem produtor é a classe do épico órfão (#412)

## [1.53.0] - 2026-09-02

### Adicionado
- C18 avisa quando o repo comeca a ter codigo de produto e ainda nao publica release (#407)
- CHANGELOG montado de fragmentos em changelog.d, validado pelo gate existente (#407)

### Mudado
- Perfil do projeto-infra passa a ser por fase: release-please so na implementacao (#407)

## [1.52.0] - 2026-08-30

### Adicionado

- `ancora.py`: o projeto declara em `doc-profile.yaml` a versão do deltaspec que consome e o gate reprova quando o plugin em execução diverge (#403)
- C17 do `check_cycle.py` e bloco 5 do hook de pré-commit passam a confrontar a âncora; adoção por `ancora.py fixar` (#403)

## [1.51.1] - 2026-08-30

### Corrigido

- `check_data_model.py`: célula marcada `[lacuna]` era silêncio no gate — o M7 novo a reapresenta como pauta, sempre BAIXO (#400)

## [1.51.0] - 2026-08-30

### Adicionado

- `check_handoff.py`: gate mecânico do handoff de sessão — frontmatter parseável e 5 itens do checklist de reprovação (#366, #367)

### Mudado

- Template `handoff-sessao.md` ganha frontmatter e 8 seções; `handoff` chama o gate no passo 3 e autorrevisa os itens de julgamento no 3.5 (#366, #367)

## [1.50.1] - 2026-08-30

### Corrigido

- `debito.py quitar`: `git mv` sobre arquivo sujo stagia a rename com o blob antigo do índice — commit sem `git add` explícito gravava o item como se não tivesse sido quitado (#394)

## [1.50.0] - 2026-08-30

### Adicionado

- `debito.py novo`: `descricao` ganha teto de 100 caracteres — vira título do índice, da fila e da issue (#372)
- `motores.tickets: none|github-issues|jira` explícito no `doc-profile.yaml`; ausente deriva pela regra legada (#372)
- `debito.py exportar --faixa alta|media|baixa` recorta os dialetos pela mesma faixa de score da fila (#372)

## [1.49.1] - 2026-08-30

### Corrigido

- `lote-debitos`: identificador de terceiro sai do que é publicado — o gate de confidencialidade abortou a v1.49.0 antes de ela sair no derivado (#370)

## [1.49.0] - 2026-08-30

### Adicionado

- `debito.py quitar`: mecaniza a quitação/descarte — `encerrado`, seção `Como foi quitado`, `Ticket`→`Encerrado` e `git mv` para `_archive/` (#363)
- Skill `lote-debitos`: corrige vários `DT-NNN` independentes em paralelo, um subagente e uma worktree por débito, sem push automático (#364)

### Mudado

- `.gitignore`: `.claude/plans/` deixa de ser local-only — planos de sessão passam a ser versionados (#365)

## [1.48.0] - 2026-08-29

### Adicionado

- `git-workflow`: PR aberto é PR vivo — título e descrição acompanham a branch, e o módulo declara o merge por squash (#337)
- `check_cycle.py`: C16 confronta o topo do "Agora" do `HANDOFF.md` com a tag e a data do último commit, fecha o DT-045 (#349)
- `handoff`: passo novo roda `debito.py fila` e cobra decisão (agendar/aceitar/descartar) sobre item `stale` antes de fechar a sessão (#352)

## [1.47.0] - 2026-08-29

### Adicionado

- `check_cycle.py`/`ci.yml`: três checks fecham DT-064, DT-056 e DT-076 — manifestos+READMEs, registro de `--selftest` e índice do TRUTH confrontados com o disco (#340, #341)

### Corrigido

- `pre-commit` da git-guard: sem `deps.toml` na raiz o bloco de integridade se omite com aviso, em vez de bloquear todo commit que toca `.md` (#346)

## [1.46.1] - 2026-08-29

### Corrigido

- `publica-dist.sh`: push recusado na assinatura explica a causa provável — aprovação pendente no agente, agente que não serve a chave, ou deploy key revogada (#344)
- `plugin.json`: manifesto volta a espelhar a tag. A v1.46.0 saiu com `1.45.0` no manifesto, então o derivado publicado anunciava a versão anterior (#344)

## [1.46.0] - 2026-08-29

### Adicionado

- `doc-entregavel`: `--linguagem executiva|tecnica|completa` recorta o documento pelos H1 `# Parte I — ` / `# Parte II — `; sem Partes, sai inteiro (#338)
- `doc-entregavel`: recorte não assina — `--linguagem` diferente de `completa` recusa `--anexo` e `--assinatura`, que deixariam âncora de anexo pendurada (#338)
- `doc-entregavel`: `pdf`/`docx` ficam autocontidos — link relativo e URL degradam para texto; âncora interna e imagem ficam (#338)
- `doc-entregavel`: `.svg` vira `.png` no caminho do docx quando o derivado existe ao lado, porque o pandoc não rasteriza SVG (#338)
- `doc-entregavel`: seção `## Argumentos` no SKILL.md — a linguagem seleciona seções, nunca reescreve prosa (#338)

### Corrigido

- `doc-entregavel`: `R\$` vazava a contrabarra no PDF — o pandoc desescapa, o python-markdown não tem `$` na lista (#338)
- `doc-entregavel`: nota de rodapé não renderizava no PDF; a extensão `footnotes` não estava ligada no python-markdown (#338)

## [1.45.0] - 2026-08-27

### Adicionado

- Hook de agente `guarda-git.py` (git-guard): pede confirmação antes de `--no-verify`, force-push, descarte de árvore, remoção forçada e `git add -A` — o G3 para de acusar o repo (#332)
- `segredos.py --staged`/`--diff`: o mesmo código no `pre-commit` e no CI; templates `pre-commit` (dono único, 4 blocos) e `commit-msg` da git-guard; `.githooks/` do repo vira symlink (#333)
- Checks G8 (`.gitattributes`/CODEOWNERS), G9 (branch integrada, só lista) e G10 (`.gitignore` sujo) no bloco git; o repo ganha `.gitattributes` e CODEOWNERS (#333)
- Modo `instalar` da git-guard (hooks, `.gitattributes`/CODEOWNERS, hook do harness — nunca sobrescreve), gancho no init; a doc-integrity aponta para ela (ADR-0040) (#334)

### Mudado

- **BREAKING** O template `pre-commit` sai da guarding-doc-integrity e passa a ser o da git-guard; instalação antiga com `deltaspec.validator` segue funcionando (#334)

## [1.44.4] - 2026-08-26

### Corrigido

- C5 (`check_changelog.py`) reprovava o `CHANGELOG.md` do repositório derivado, cujo rodapé o `publica-dist.sh` trunca por desenho: sem rodapé nenhum o check se omite com aviso (#328)

## [1.44.3] - 2026-08-26

### Corrigido

- C4 (`check_cycle.py`) acusava ALTO na fase analyze em toda delta que cresce um requisito: a comparação contra o bloco MUDA só roda no archive (#326)

## [1.44.2] - 2026-08-26

### Corrigido

- W9 (`audit-workspace`) cobrava hook de módulo-fonte citado em prosa: a alçada passa a ser a localização do arquivo no repo (#322)
- `publica-dist.sh` abortava quando a chave de publicação vivia num agente SSH, e a mensagem mandava gerar outra fora do cofre (#323)
