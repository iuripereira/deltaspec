...
3. **PR contra a `develop`** com os dois checks verdes (`ci` + `commits`). Merge é por squash; a `main` só recebe release e hotfix, por merge commit. PR acima do limiar canônico de tamanho (ver CLAUDE.md) será pedido para dividir.
...
To contribute: fork → scope branch (`type/short-description`) → [Conventional Commits](https://www.conventionalcommits.org) → PR against `develop` with both CI checks green (`ci` + `commits`); merges are squashed — `main` only takes releases and hotfixes, by merge commit. **Any change under `skills/` must go through the framework's own delta-spec cycle** (this repo dogfoods itself) — open an issue first and the delta will be driven together. Run the self-tests above before opening a PR. Security reports go through [SECURITY.md](SECURITY.md), never public issues; the [Contributor Covenant 2.1](CODE_OF_CONDUCT.md) applies.
