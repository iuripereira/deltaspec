...
- R52 (delta-017, delta-035) — ferramenta de ticket é projeção do registro, com ida mecânica e volta aprovada.
...
- R55 (delta-033) — o formato de item do ciclo tem um parser só, com dono canônico.
  - DADO os scripts do framework que leem item de `tasks.md`/`test-plan.md` (`check_cycle.py` nos C2/C8/C9/C10 e `tickets.py` na projeção) QUANDO qualquer um deles parseia um item ENTÃO todos passam pela **mesma função** do módulo dono (`skills/spec-feature/scripts/itens.py`), e nenhum script reimplementa a âncora da linha — regra de ouro do repositório, medida no review por ausência de padrão duplicado
  - DADO um `tasks.md` com item multi-linha QUANDO a projeção de tickets (R53) roda ENTÃO ela enxerga os mesmos campos que o gate enxerga, incluindo `dep:` na primeira linha — dois parsers divergentes projetariam bloqueio faltando no Jira em silêncio
...
- R12 (delta-033) — a metade mecânica do analyze é um script, e o núcleo que ele exige não inclui campo que ninguém lê.
  ...
  - DADO um item de `tasks.md` ou de `test-plan.md` cujo texto continua em linhas seguintes QUANDO o gate o lê ENTÃO os campos que vêm depois do travessão (`arquivos:`, `cobre:`, `verificação:`, `tipo:`) são reconhecidos em qualquer das linhas do item — que termina na linha em branco, no próximo item ou no próximo heading — e quebra de linha nunca produz achado falso (medição de 2026-08-07: 3 falsos por task quebrada em duas)
  - DADO a aresta `(dep: Tn)` QUANDO o C9 a lê ENTÃO ela continua valendo **apenas colada ao ID da task**, na primeira linha do item — a tolerância a multi-linha não afrouxa essa âncora, porque `(dep: Tn)` escrito na prosa da task não é aresta (decisão do dogfood da delta-016, R40)
  - DADO um heading `###` nas seções que hospedam requisito (`## Mudanças` e `## Requisitos não funcionais`) do `spec.md` que não casa a forma canônica `### Rn|RNFn — ADICIONA|MUDA|REMOVE` QUANDO o C1 roda ENTÃO acusa **ALTO** nomeando a linha e o texto do heading — requisito que o parser não enxerga nunca sai LIBERADO em silêncio (reprodução de 2026-08-07: `### R2 — Adiciona:` em minúscula passava sem nenhum check de cenário)
