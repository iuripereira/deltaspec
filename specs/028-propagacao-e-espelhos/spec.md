...
- Métrica: **exatamente uma** dependência externa admitida nos scripts do framework (`PyYAML`), com a ADR que a admite citada no arquivo dono da política (`CLAUDE.md`) e em **três** espelhos sancionados — `README.md` e `README.en.md`, onde o leitor precisa do comando de instalação, e `specs/TRUTH.md`, onde o requisito cita a decisão que o originou —, dentro do teto que a própria `guarding-doc-integrity` recomenda; `SECURITY.md` afirma o fato sem repetir o identificador, apontando para o dono
- Verificação: entrada `[[owner]]` no `deps.toml` com o padrão da ADR, conferida pelo C1 do `validate_integrity.py` — que roda no CI **e no pré-commit** —, e o C2 garantindo que o identificador não se materialize fora dos sancionados; dependência nova exige ADR própria, nunca herança desta
- Verificação da metade negativa: step no job `ci` com o grep das frases de zero-dep, **declarado como exceção** — o `validate_integrity.py` não tem check de padrão proibido, e criar um está fora do escopo desta delta
...
- O `[[owner]]` novo governa um identificador de ADR, não um valor de negócio como os demais. Se o C2 acusar materialização legítima futura (uma skill que precise citar a ADR), o espelho entra no manifesto por decisão, não por exceção silenciosa.
