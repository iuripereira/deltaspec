...
- **Keep a Changelog 1.0.0** — toda mudança notável entra em `CHANGELOG.md` (na raiz), primeiro sob `## [Não lançado]`, agrupada em `Adicionado / Mudado / Corrigido / Removido / Obsoleto / Segurança`. No release, `montar_changelog.py --liberar X.Y.Z` renomeia `[Não lançado]` → `## [X.Y.Z] - AAAA-MM-DD`, abre um `[Não lançado]` novo e escreve o rodapé de compare (é o que `/deltaspec:criar-release` chama).
...
- **Correlação commit → bump:** `fix` = PATCH · `feat` = MINOR · `!`/`BREAKING CHANGE` = MAJOR. O maior vence. **A tag corta no merge que conclui a delta — normalmente o PR de archive, porque o "pronto" inclui o archive — e quem a cria é `/deltaspec:release`, nunca a mão.** Este repo é o perfil com `develop` desde a v1.59.0 ([ADR-0048](docs/adrs/ADR-0048-deltaspec-no-perfil-com-develop.md)): `/deltaspec:criar-release X.Y.Z` corta `release/X.Y.Z` da `develop`, a PR de release entra na `main` por merge commit, e a `release` taggeia no merge e faz a sync `main→develop` por `sync/X.Y.Z`. PRs de documentação fora do ciclo não geram tag.
...
- **Tag cortada = Release publicada:** depois que a PR de release fica verde, o Iuri roda `/deltaspec:release`, e ela mergeia, cria tag e Release numa chamada (`gh release create --target <sha do merge>` com a seção correspondente do CHANGELOG) e, neste repo, roda o `publica-dist.sh`; se parar no meio, rodar de novo retoma do primeiro passo que falta. A Release do GitHub é **projeção** da tag (a tag continua a fonte), mas é a vitrine que o visitante lê — sem ela a página anuncia versão velha como "Latest" (decisão do Iuri em 2026-08-04, DT-030; disciplina monitorada pelo DT-031).
- **Valide no CI:** o job `commits` reprova PR com commits fora do padrão.

## Fluxo de trabalho Git

- **`main` e `develop` protegidas, `main` sempre lançável.** Merge só via PR com checks verdes (`ci` + `commits`) — imposta pelos rulesets `sdd-protect-main` (só merge commit; `ci` + `commits` + `release-check`) e `sdd-protect-develop` (`ci` + `commits`), sem liberação para admin ([ADR-0048](docs/adrs/ADR-0048-deltaspec-no-perfil-com-develop.md)).
...
- **Merge por squash na `develop`** — a PR de escopo nasce da `develop` e volta a ela por squash: 1 commit por PR, mensagem em Conventional Commits. `release/*`, `hotfix/*` e `sync/*` entram por **merge commit** (ADR-0045); squash delas recria o 83/76, e na `main` o `sdd-protect-main` só aceita merge commit (ADR-0048).
- **PR aberto é PR vivo.** Título e descrição valem para o que está na branch **agora**. Commit que amplia o escopo → releia os dois **antes de pedir revisão ou mergear** (não a cada push). Com mais de um commit, o squash leva o **título do PR** para a `develop` — título velho fica no histórico; descrição velha engana quem revisa. As mensagens de commit sobrevivem (`COMMIT_MESSAGES`).
- **Higiene pós-merge:** apague a branch mergeada local (`git branch -d`) e remota (`git push origin --delete` + `git fetch --prune`). **Nunca apague a `main` nem a `develop`.**
...
- **Comandos do dia a dia:**
  ...
  - A **lista completa** de selftests vive no README (§8, "As checagens automáticas") e é o que o job `ci` executa — não a duplique aqui: a cópia parcial desta seção foi o drift pego pela auditoria 01 (P0-2).
