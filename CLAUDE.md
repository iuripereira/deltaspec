...
- **Correlação commit → bump:** `fix` = PATCH · `feat` = MINOR · `!`/`BREAKING CHANGE` = MAJOR. O maior vence. **A tag corta no merge que conclui a delta — normalmente o PR de archive, porque o "pronto" inclui o archive — e quem a cria é `/deltaspec:release`, nunca a mão.** Este repo é o perfil sem `develop`: `/deltaspec:criar-release X.Y.Z` corta da `main`, a PR de release entra por squash e a `release` taggeia no merge. PRs de documentação fora do ciclo não geram tag.
...
- **`main` protegida e sempre lançável.** Merge só via PR com checks verdes (`ci` + `commits`) — **regra escrita**: o repo é privado sem GitHub Pro, a API de rulesets responde 403 e nada na plataforma a impõe (DT-166).
...
- **Merge por squash** — histórico da `main` = 1 commit por PR; a mensagem do squash segue Conventional Commits. Sem `develop`, até a PR de release entra por squash — o merge commit da ADR-0045 é da topologia com `develop`.
