...
O framework promete, em três lugares vivos, que os gates rodam sem dependência externa: `CLAUDE.md` ("os gates usam stdlib pura — zero pacote externo"), `README.md` ("Sem dependência externa: só a biblioteca padrão do Python 3.11+") e `README.en.md` (idem em inglês). É promessa de primeira linha, ao lado de "tudo roda na sua máquina" (ADR-0001) — parte do que torna o gate auditável e reprodutível.
...
Decidida com o usuário em 2026-08-02 (clarify da delta-026): **alternativa 3.** `PyYAML` passa a ser dependência externa **declarada e admitida** dos gates — a única. Os três espelhos da promessa deixam de dizer "só a biblioteca padrão" e passam a nomear a exceção nos mesmos termos; o job `ci` instala o pacote explicitamente, em vez de contar com o que o runner traz pré-instalado.
