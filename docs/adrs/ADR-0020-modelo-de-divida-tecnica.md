...
- **Data:** 2026-08-01
- **Supersedes:** —
- **Superseded by:** [ADR-0028](ADR-0028-arquivamento-de-debitos-encerrados.md) (2026-08-10, delta-041) — **apenas na renúncia a "arquivo por item"**: itens em estado final (`quitado`/`descartado`) passam a arquivar em `.claude/debts/`. Depois, [ADR-0030](ADR-0030-registro-de-debitos-em-pasta-na-raiz.md) (2026-08-11, delta-043) — as renúncias a "arquivo por item" e a "front-matter" caem **de vez** (ativos inclusive, em `debts/ativos/`). O modelo de fila (score derivado e nunca gravado, três eixos, `stale` calculado) segue integralmente vigente.
