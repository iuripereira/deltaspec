...
A primeira adoção real, em 2026-08-02 num repo consumidor (graphify 0.9.32), produziu três fatos que o contrato escrito da doc não previa:
...
3. **O grafo cita código que não existe.** Dos 27 arquivos de código referenciados, 16 são fantasmas (`apps/api/src/<dominio>/core/policy.py`, ...): as specs descrevem implementação ainda não escrita, e o extrator criou o nó do alvo citado. Não é defeito do motor — é retrato fiel de um projeto documentado à frente do código.
