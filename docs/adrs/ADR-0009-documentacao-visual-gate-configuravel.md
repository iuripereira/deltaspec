...
A documentação visual (arquitetura, modelo de dados, fluxos, casos de uso) ficava fora do ciclo: cada projeto decidia ad hoc se e quando diagramar, e o resultado era inconsistência — projetos com entregável contratual (PRD/contrato assinado pelo cliente, como nos projetos do caso de referência) precisam de diagramas congelados em PDF/DOCX, enquanto projetos internos precisam no máximo de Mermaid inline vivo junto do código. Duas forças em tensão:
...
Há ainda a distinção de natureza: documentação **interna** é viva (mantida a cada mudança relevante, renderiza nativa no GitHub/Obsidian); documentação **cliente** é congelada (exportada em momento definido, versionada, re-assinada a cada baseline — padrão já praticado nos contratos do caso de referência, cláusula 1.3.1).
...
Para o entregável congelado nasce a skill **`doc-entregavel`**: renderiza os diagramas do perfil (mmdc/dbml-renderer → SVG/PNG), monta o documento final com capa de assinatura parametrizada e exporta PDF/DOCX em `docs/entregaveis/` — generalização do pipeline já validado nos 4 PRDs + 4 contratos do caso de referência (pypandoc + python-docx para DOCX; markdown → HTML+CSS → chrome headless para PDF). Nova baseline = novo arquivo versionado = nova assinatura; entregável nunca é sobrescrito.
...
**Promoção (2026-07-28, delta-013):** o piloto doc-profile+doc-entregavel fechou nos 4 repos do caso de referência (8 exports em 2 rodadas, 2026-07-20) e a exceção do RNF1 consolidou via MUDA no TRUTH.md; a `doc-entregavel` saiu de experimental no README. O check mecânico do doc-profile (presença + schema) permanece adiado pelo perímetro do ADR-0006 — pendência roteada na delta-013. A delta externa com gate do specify (num projeto consumidor) não condicionava esta promoção e segue como gatilho do DT-004.
